        ...
        if no_find_request_impl:
            ...
            if stub is None:
                raise MethodNotFoundError(id_,
                                          message='The method {0!r} does not exist or is not available.'.format(method))
...
def raise_if_invalid_id(id_, message='', exception_class=None):
    if not isinstance(id_, (str, int, float, type(None))):
        ...
        raise exception_class(str(message or 'Invalid JSON-RPC `id` {0!r}'.format(id_)))
...
class Response:
    ...
    def __repr__(self):
        if qualname:
            return '<{0}.{1} object at {2} id={3} result={4}>'.format(self.__module__, self.__class__.__qualname__,
                                                                      hex(id(self)), self._id, self.result)
        else:
            return '<{0}.{1} object at {2} id={3} result={4}>'.format(self.__module__, self.__class__.__name__,
                                                                      hex(id(self)), self._id, self.result)
...
class Remote:
    ...
    def decorate(cls, method='', prefix='', is_notification=False):
        def wrapper(_method, _prefix, callable_object):
            ...
            if _prefix:
                _method = '{0}.{1}'.format(_prefix, _method)
...
class Local:
    ...
    def rpc_func(cls, method='', prefix='', is_notification=False):
        def wrapper(_method, _prefix, callable_object):
            ...
            if _method in cls._stubs:
                raise KeyError('JSON-RPC method "{0}" duplicated definition.'.format(_method))
    ...
    def rpc_class(cls, method='', prefix='', is_notification=False):
        def wrapper(_method, _prefix, class_type):
            ...
            if _method in cls._stubs:
                raise KeyError('JSON-RPC method "{0}" has a duplicated definition.'.format(_method))
...
class Error(Exception):
    ...
    def __repr__(self):
        if qualname:
            return '<{0}.{1} object at {2} id={3} code={4} message={5} data={6}>'.format(self.__module__,
