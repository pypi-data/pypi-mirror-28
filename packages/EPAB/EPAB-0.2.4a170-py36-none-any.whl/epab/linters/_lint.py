...
@epab.utils.run_once
@epab.utils.stashed
def _lint(ctx: click.Context, amend: bool, fail: bool):
    epab.utils.info('Running all linters')
    ctx.invoke(pep8, amend=amend)
    if fail and CTX.repo.changed_files():
        epab.utils.error('Linting changed some files')
...
@click.option('-f', '--fail', is_flag=True, help='Amend last commit with changes')
def lint(ctx: click.Context, amend: bool, fail: bool):
    """
    Runs all linters
