...
def _process():
    process = mock()
    subprocess = mock()
    when(subprocess).read_nonblocking(1, None).thenReturn('output').thenRaise(pexpect.exceptions.EOF(None))
...
def test_output(process, input_):
    unstub(process.subprocess)
    when(process.subprocess).read_nonblocking(1, None).thenReturn(input_).thenRaise(pexpect.exceptions.EOF(None))
    out, code = epab.utils.run('test')
    verify(epab.utils).info('RUNNING: test')
    ...
    verify(epab.utils).std_out(input_)
...
def test_mute_output():
    ...
    verify(epab.utils, times=0).error(...)
    verify(epab.utils).cmd_start('RUNNING: test')
...
def test_filter_as_str():
    when(epab.utils._run)._read_subprocess_output(subprocess=any(), mute=True, filters=['test']).thenReturn('output')
    ...
    verify(epab.utils, times=0).std_err(...)
    verify(epab.utils, times=0).std_out(...)
    verify(epab.utils._run)._read_subprocess_output(...)
    assert code == 0
    assert out == 'output'
...
def test_no_output(process):
    unstub(process.subprocess)
    when(process.subprocess).read_nonblocking(1, None).thenReturn('').thenRaise(pexpect.exceptions.EOF(None))
    out, code = epab.utils.run('test')
    verify(epab.utils).info('RUNNING: test')
    verify(epab.utils).info('test.exe -> 0')
    verify(epab.utils, times=0).cmd_start(...)
...
def test_filtered_output(process):
    unstub(process.subprocess)
    when(process.subprocess).read_nonblocking(1, None).thenReturn('output').thenRaise(pexpect.exceptions.EOF(None))
...
def test_error(process):
    ...
    unstub(process.subprocess)
    when(process.subprocess).read_nonblocking(1, None).thenReturn('some error').thenRaise(pexpect.exceptions.EOF(None))
    out, code = epab.utils.run('test', filters=['output'], failure_ok=True)
    verify(epab.utils).info('RUNNING: test')
    verify(epab.utils, times=0).cmd_start(...)
    verify(epab.utils, times=0).cmd_end(...)
    verify(epab.utils).std_err('test.exe some error')
...
def test_failure(process):
    process.return_code = 1
    unstub(process.subprocess)
    when(process.subprocess).read_nonblocking(1, None).thenReturn('error').thenRaise(pexpect.exceptions.EOF(None))
    with pytest.raises(SystemExit):
