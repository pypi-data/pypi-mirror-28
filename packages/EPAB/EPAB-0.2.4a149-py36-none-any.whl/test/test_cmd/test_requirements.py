...
import pytest
from mockito import (ARGS, expect, mock, patch, spy, spy2, unstub, verify, verifyNoMoreInteractions,
                     verifyNoUnwantedInteractions, when, when2)
...
def test_requirements_with_dev_packages():
    ...
    reqs_dev = Path('requirements-dev.txt')
    when(epab.utils).run('pipenv lock -r', mute=True, filters='Courtesy Notice: ').thenReturn(('reqs==0.1.0', 0))
    when(epab.utils).run('pipenv lock -r -d', mute=True, filters='Courtesy Notice: ').thenReturn(('reqs==0.1.1', 0))
    epab.cmd._reqs._write_reqs(False, False)
    assert reqs.exists()
    assert reqs_dev.exists()
    assert reqs.read_text() == 'reqs==0.1.0'
