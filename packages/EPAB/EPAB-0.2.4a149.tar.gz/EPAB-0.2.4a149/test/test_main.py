...
import click
import pytest
from click.testing import CliRunner, EchoingStdin, Result
from mockito import mock, patch, verify, when, when2

import epab.utils
from epab.__main__ import _COMMANDS, _LINTERS, cli
from epab.cmd import _appveyor, _chglog, _install_hooks, _pytest, _release, _reqs, _version
...
def test_appveyor():
    when(_appveyor)._appveyor(...)
    assert succesfull_run2('appveyor')
    verify(_appveyor)._appveyor(...)
