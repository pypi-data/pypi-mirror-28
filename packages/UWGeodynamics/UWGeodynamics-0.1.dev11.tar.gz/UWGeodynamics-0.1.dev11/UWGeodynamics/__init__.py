...
r = requests.get('https://pypi.python.org/pypi/%s/json' % "UWGeodynamics")
upstream_version = r.json()['info']['version']
print(upstream_version)
iv = LooseVersion(installed_version)
uv = LooseVersion(upstream_version)
if iv < uv:
    print(_get_message("UWGeodynamics", iv, uv,
