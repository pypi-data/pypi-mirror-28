...
class TestMVPolyRepr(unittest.TestCase):

    def test_repr_example(self):
        for C in classes:
            x, y = C.variables(2)
            p = (x + 2*y - 4)**3
            s = repr(p)
            self.assertTrue(s, "repr() false")
...
class TestMVPolyEval(unittest.TestCase):
    ...
    def test_eval_return_type(self):
        for C in classes:
            ...
            p = x**2 + y
            self.assertTrue(
                isinstance(p.eval(2, 1), np.int64),
                "int64 return"
            )
    ...
    def test_int_integer_warning(self):
        for C in classes:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                self.assertTrue(
                    len(w),
                    "expected 1 warning, got {0:d}".format((len(w)))
                )
                wcat = w[0].category
                self.assertTrue(
                    issubclass(wcat, RuntimeWarning),
...
class TestMVPolyMaxmodND(unittest.TestCase):
    ...
    def test_maxmodnd_gupta_ray(self):
        ...
        for C in classes:
            ...
            obt = p.maxmodnd(epsilon=eps)[0]
            self.assertTrue(
                np.abs(obt - expected) < eps*expected,
