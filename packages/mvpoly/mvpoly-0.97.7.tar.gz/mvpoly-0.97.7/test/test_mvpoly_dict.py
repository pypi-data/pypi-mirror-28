...
class TestMVPolyDictNonzero(unittest.TestCase):
    ...
    def test_nonzero_enumerate(self):
        ...
        self.assertTrue(
            sorted(obt) == sorted(exp),
            "nonzero"
        )


class TestMVPolyDictInitFromNonzeroTuples(unittest.TestCase):

    def test_init_from_nonzero_tuples(self):
        x, y = MVPolyDict.variables(2)
        p = 3*x + y**2 + 7
        q = MVPolyDict.init_from_nonzero_tuples(p.nonzero, p.dtype)
        self.assertTrue(p == q, "init_from_nonzero_tuples")
