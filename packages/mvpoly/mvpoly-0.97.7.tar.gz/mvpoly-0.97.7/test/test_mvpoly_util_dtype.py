...
class TestMPUtilDtype(unittest.TestCase):
    ...
    def test_dtype_add(self):
        for dat in  testdat:
            ...
            self.assertTrue(obt == exp, "dtype plus {0!s}".format((repr(obt))))

    def test_dtype_minus(self):
        for dat in  testdat:
            t1, t2, exp = dat
            obt = mvpoly.util.dtype.dtype_minus(t1, t2)
            self.assertTrue(obt == exp, "dtype minus {0!s}".format((repr(obt))))

    def test_dtype_mul(self):
        for dat in  testdat:
            t1, t2, exp = dat
            obt = mvpoly.util.dtype.dtype_mul(t1, t2)
            self.assertTrue(obt == exp, "dtype mul {0!s}".format((repr(obt))))
