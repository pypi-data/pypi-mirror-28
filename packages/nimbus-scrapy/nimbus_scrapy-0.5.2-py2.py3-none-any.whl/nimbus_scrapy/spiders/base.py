...
class AbstractSpider(BaseMixin, Spider):
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        try:
            spider.log(message=u"SPIDER NAME: {}".format(spider.name), level=logging.INFO)
            if config is None:
                spider.log(message="config is None", level=logging.ERROR)
            else:
                spider.log(message=config, level=logging.DEBUG)
                spider.setup(config)
                spider.log(u"CONFIG [{sid}]: {sname}".format(sid=spider.sid, sname=spider.sname), level=logging.INFO)
                spider.log(u"CONFIG [{spider_id}]: {spider_name}".format(spider_id=spider.spider_id, spider_name=spider.spider_name), level=logging.INFO)
                spider.log(u"CONFIG [{spider_id}]: {version}".format(spider_id=spider.spider_id, version=spider.version), level=logging.INFO)
