...
class BLS(BanyanBase):
    ...
    Where:
        ...
        append_bp_address = yes or no. If yes a -b commmand line option with
                            the IP adress of this computer is appended to the
                            command string.
    ...
    def __init__(self, subscriber_port='43125', publisher_port='43124',
        ...
        # allow time for zmq connections to establish
        time.sleep(.3)
    ...
    def incoming_message_processing(self, topic, payload):
        ...
        for idx, record in enumerate(self.launch_db):
            # if no launch id has alreary been assigned, update the record and print a message
            if not record['launch_id']:
                if record['launch_id'] == payload['launch_id']:
                    self.launch_db[idx]['process_id'] = payload['process_id']
                    print('{}: {:35} PID = {}'.format(topic, record['command_string'], str(self.proc.pid)))
                    break
...
def bls():
    ...
    parser.add_argument("-p", dest="publisher_port", default='43124',
                        help="Publisher IP port")
    parser.add_argument("-r", dest="relaunch_all", default='False',
                        help="Relaunch All If Backplane Dies")
    parser.add_argument("-s", dest="subscriber_port", default='43125',
                        help="Subscriber IP port")

    args = parser.parse_args()

    if args.relaunch_all == 'True':
        args.relaunch_all = True
    else:
        args.relaunch_all = False
    kw_options = {
