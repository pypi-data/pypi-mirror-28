...
class mypoolin_upi(object):
	...
	def request_upi_single(self,virtual_address,beneficiary_amount):
		resp = self.hmac_requests("POST","/request_upi_single",beneficiary_virtual_address=virtual_address,beneficiary_amount=beneficiary_amount)
		return self.json_validation_and_return(resp.text)

	def request_upi_single_async(self,virtual_address,beneficiary_amount):
		resp = self.hmac_requests("POST","/request_upi_single_async",beneficiary_virtual_address=virtual_address,beneficiary_amount=beneficiary_amount)
		return self.json_validation_and_return(resp.text)
