...
from duckietown_swarm.utils import memoize_simple
import getpass
import json
...
from .utils import yaml_dump_omaps, tmpfile
from .utils import yaml_load_omaps
...
def find_more_information(h):
    ...
    try:
        info_data = get_ipfs_data(h + '/' + INFO)
    except CmdException as e:
        if 'no link named' in e.stderr:
            raise UnexpectedFormat(e.stderr)
        raise
    try:
...
def get_ipfs_data(h):
    with tmpfile('.ipfs_data') as f:
        cmd = ['ipfs', 'get', '-o', f, h]
        _res = system_cmd_result(cwd='.', cmd=cmd, raise_on_error=True)
        data = open(f).read()
        return data
...
def start_irc():
#    nickname = socket.gethostname()[:10] + '_' + str(random.randint(0, 100))
    nickname = 'D_%s_' % (__version__) + str(random.randint(0, 100))
