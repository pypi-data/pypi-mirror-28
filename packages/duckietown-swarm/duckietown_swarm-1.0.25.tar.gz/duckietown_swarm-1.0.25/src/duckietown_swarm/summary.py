...
def publisher_summary():
    ...
    while True:
        ...
        if previous_hash != summary_hash:
            ...
            Queues.ipns_queue.put(summary_hash)
...
def publish_ipns():
    ...
    while True:
        #print('summary')
        timeout = 5
        hashes = get_at_least_one(Queues.ipns_queue, timeout)
        if hashes:
            print('ipns: found %s' % hashes)
            ...
            print('ipns: published %s -> /ipfs/%s ' % (path, summary_hash))
