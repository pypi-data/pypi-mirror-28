...
class Coopy(object):
	...
	def run(self,args,io = None):
		if (io is None):
			io = TableIO()
		if (io is None):
			haxe_Log.trace("No system interface available",_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 712, 'className': "Coopy", 'methodName': "run"}))
			return 1
		self.init()
		self.io = io
		more = True
		...
		if (cmd1 == "git"):
			...
			if (pos26 < 0):
				pos26 = 0
			res26 = args[pos26:(pos26 + len)]
			del args[pos26:(pos26 + len)]
			types = res26
		...
		if git:
			...
			if (pos28 < 0):
				pos28 = 0
			res28 = args[pos28:(pos28 + len1)]
			del args[pos28:(pos28 + len1)]
			res28
			offset = 0
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + " ")
			txt = (HxOverrides.stringOrNull(txt) + "\n")
		haxe_Log.trace(txt,_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 1122, 'className': "Coopy", 'methodName': "show"}))
...
DiffSummary = _hx_classes.registerClass("DiffSummary", fields=["row_deletes","row_inserts","row_updates","row_reorders","col_deletes","col_inserts","col_updates","col_renames","col_reorders","row_count_initial_with_header","row_count_final_with_header","row_count_initial","row_count_final","col_count_initial","col_count_final"])(DiffSummary)
