...
class DocumentCursor(object):
    ...
    def __init__(self, cls, cursor):
        """ Accepts the class type and pymongo cursor as arguments """
        self.cursor = cursor
        self.cls = cls
...
class DocumentMeta(type):
    ...
    def __new__(cls, name, base, clsdct):
        if '_collection' not in clsdct:
            clsdct['_collection'] = convert(name)
        return super().__new__(cls, name, base, clsdct)

    def __init__(self, name, bases, clsdct):
        return super().__init__(name, bases, clsdct)
...
class Document(metaclass=DocumentMeta):
    ...
    def save(self, create_new: bool = True) -> bool:
        ...
        if ("id" not in attributes or not attributes['id']) and create_new:
            del attributes['id']
            result = type(self).get_collection().insert_one(attributes)
            self.id = ObjectId(result.inserted_id)
            return True

        result = type(self).get_collection().update_one({"_id": self.id}, {"$set":self._filtered_attributes()}, upsert=create_new)
    ...
    def delete(self):
        result = type(self).get_collection().delete_one({"_id": self.id})
    ...
    def get_by_id(cls, Id):
        ...
        record = cls.get_collection().find_one({'_id': Id})
...
class MongoODM(object):
    ...
    def __init__(self, app=None, **config):
        ...
        self.config = config
        self.Document = type('Document', Document.__bases__, dict(Document.__dict__))
        ...
        if app is not None:
            ...
            self.Document.set_db(self.connection)
    ...
    def __call__(self, cls):
        ...
        new_cls.set_db(self.connection)
        return new_cls