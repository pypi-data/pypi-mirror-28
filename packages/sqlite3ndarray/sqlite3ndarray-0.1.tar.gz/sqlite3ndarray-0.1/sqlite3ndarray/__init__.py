...
__all__ = ['sql_table_info', 'sql_table_nrows', 'sql_tables', 'sql2ndarray',
           'ndarray2sql']
sqlite3_types = {
    np.int64: 'INTEGER',
    np.float64: 'REAL',
...
def _drop_table(connection, name):
    request = 'DROP TABLE {}'.format(name)
    with connection:
        connection.execute(request)


def _create_table(connection, table, valid_names, dtypes):
    columns = ['"id" INTEGER PRIMARY KEY ASC AUTOINCREMENT']
...
def ndarray2sql(database, table, data, if_exists='fail'):
    ...
    sqltypes = [types[_.type] for _ in dtypes]
    ...
    with asconnection(database) as connection:
        ...
        with connection:
            connection.executemany(request, zip(*values))
