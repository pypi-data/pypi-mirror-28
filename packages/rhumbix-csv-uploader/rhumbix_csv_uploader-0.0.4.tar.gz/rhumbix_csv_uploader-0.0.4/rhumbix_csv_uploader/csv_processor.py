...
def main():
    ...
    for path in employee_paths:
        ...
        result = employee_processor.process_csv(path, args.company_key, args.api_key)
    ...
    for path in project_paths:
        ...
        result = project_processor.process_csv(path, args.company_key, args.api_key)
        batch_key = result.json()["batch_key"]
        logging.info("Finished file:%s with batch id: %s" % (path, batch_key))
    ...
    for path in cost_code_paths:
        ...
        result = cost_code_processor.process_csv(path, args.company_key, args.api_key)
        batch_key = result.json()["batch_key"]
        logging.info("Finished file:%s with batch id: %s" % (path, batch_key))
