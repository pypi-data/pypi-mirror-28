...
class Converter:
    ...
    def instr_pos_to_line_column(self, pos):
        ...
        return line + 1, pos - (self.newline_chars[line-1] if line > 0 else -1), pos
    ...
    def to_html(self, instr, outfile = None, *, outer_pos = 0):
        ...
        def exit_with_error(message, pos):
            raise Exception(message, *self.instr_pos_to_line_column(pos))
        ...
        while i < len(instr):
            ...
            if ch == "‘":
                ...
                ins = html_escape(ins)
                if ins.find("\n") == -1: # this is a single-line code -‘block’span
...
{
    if (event.target.nodeName == 'A' || event.target.onclick)//чтобы работали ссылки в спойлерах и спойлеры2 в спойлерах
        return;
