...
def main():
    opt = OptionParser()
    opt.add_option("-i", "--input", help = "input directory -i path/directory", action = "store", type = "string", dest = "dir")
    opt.add_option("-d", "--database", help = "DB connection -d user/pwd@//host:port/service", action = "store", type = "string", dest = "db")
    opt.add_option("-r", "--range", help = "number of transactions -r 1000", action = "store", type = "int", dest = "range")
