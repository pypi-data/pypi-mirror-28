...
def pack_jpg(filepath, debug=False):
    ...
    cmd = JPEGOPTIM_PATH + JPEGIOPTIM_OPTIONS + '"' + filepath + '"' + ' 1> /dev/null' + ' 2> /dev/null'
    if debug:
...
def pack_png(filepath, debug=False):
    ...
    cmd = PNGQUANT_PATH + PNGQUANT_OPTIONS + '"' + tempfpath + '"' + ' 1> /dev/null' + ' 2> /dev/null'
    if debug:
...
class FileRepacker:
    ...
    def repack_zip_file(self, filename, outfile=None,
        ...
        results = {'stats': [0, 0, 0], 'files': []}
        if outfile is None: outfile = filename
        ...
        if filetype in SUPPORTED_EXTS:
            ...
            os.mkdir(fpath)
            fn = SZIP_PATH + ' x -o%s "%s" 1>/dev/null 2>/dev/null' % (fpath, filename)
            if options['debug']:
                            ...
                            if options['pack_archives']:
                                ...
                                if res is not None:
                                    results['files'].append([fn, res['stats'][1], res['stats'][2],
                                                             (res['stats'][1] - res['stats'][2]) * 100.0 /
                                                             res['stats'][1] if res['stats'][1] > 0 else 0])
                                    results['stats'][0] += 1
            ...
            share = (insize - outsize) * 100.0 / insize if insize > 0 else 0
            if not options['quiet']:
                logging.debug('File %s shrinked %d -> %d (%f%%)' % (outfile.encode('utf8'), insize, outsize, share))
            return results

if __name__ == "__main__":
	dr = FileRepacker()
	dr.repack_zip_file(sys.argv[1])
