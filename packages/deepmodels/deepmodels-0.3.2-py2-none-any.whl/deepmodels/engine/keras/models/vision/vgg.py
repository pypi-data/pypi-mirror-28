...
class DMVGGKeras(vgg.DMVGG):
  """Keras implementation of VGG.
  """
  # keras shared functions for models.
  dmmodel_keras = model_commons.DMModelsKeras()

  def __init__(self, model_type=commons.ModelType.VGG16):
    super(DMVGGKeras, self).__init__(model_type)
  ...
  def build_model(self, inputs=None, use_fcs=True):
    ...
    if self.model_params.model_type == commons.ModelType.VGG16:
      self.model = vgg16.VGG16(
          input_tensor=inputs,
          input_shape=input_shape,
          weights="imagenet",
          include_top=use_fcs)
    if self.model_params.model_type == commons.ModelType.VGG19:
