...
from wrappers.request import RequestBuilder


class SaverBuilder(object):
    def __init__(self):
        self.headers = {
            'User-Agent': RequestBuilder().random_user_agent_header()
        }
    ...
    def save_session_file(self, url, folder=None, save_name=None, headers=None, root_dir='download'):
        save_dir = root_dir
        if save_name is None:
            save_name = url.split('/')[-1]
        if folder:
    ...
    def save_m3u8(url):
        ...
        try:
            ...
            for line in m3u8_file.readlines():
                ...
                if '.ts' in line_text:
                    urls.append(prefix + line_text[:-1].replace('\n', '').replace('\r', ''))
                else:
                    continue
