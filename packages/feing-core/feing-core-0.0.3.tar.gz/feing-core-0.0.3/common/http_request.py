...
class Request(object):
    ...
    def get_with_cookie(self, url, retry_count=3, headers=None, proxy=None, data=None):
        """
        :param url: 请求 URL 链接
        :param retry_count: 失败重试次数
        :param headers: http header={'X':'x', 'X':'x'}
        :param proxy: 代理设置 proxies={"https": "http://12.112.122.12:3212"}
        ...
        try:
            ...
            if retry_count > 0:
                if hasattr(e, 'code') and 500 <= e.code < 600:
                    ...
                    return self.get_with_cookie(url, retry_count - 1, headers, proxy, data)
        return html
...
if __name__ == '__main__':
    content = Request().get_with_cookie('http://www.baidu.com').decode('utf-8')
    print(str(content))
    content = Request().get_with_session('http://www.baidu.com').decode('utf-8')
    print(str(content))
