...
class AdminHTTPTVFuncs(object):
    ...
    def tv_list(self, limit=50, offset=0, sort_type='alpha_asc', search=None):
        ...
        if search:
            tvs = session.query(TABLES.ZP_TV.ID,
                ...
                TABLES.ZP_TV.ZP_TV_FILEFOLDER_ID != None,
    ...
    def get_tv_filefolder_id(self, zp_tv_id):
        session = self.Session()
        try:
            ...
            zp_film_filefolder_id = zp_film_filefolder.ZP_TV_FILEFOLDER_ID
        return zp_film_filefolder_id

    def tv_path(self, zp_tv_filefolder_id):
        session = self.Session()
        try:
            zp_film_filefolder = session.query(TABLES.ZP_TV_FILEFOLDER).filter(
    ...
    def tv_monitored(self):
        session = self.Session()
        tv_running_list = []
        tvs = session.query(TABLES.ZP_TV_RUNNING.ZP_TV_ID,
                            TABLES.ZP_TV_TITLE.TITLE).join(
            TABLES.ZP_TV_TITLE, TABLES.ZP_TV_TITLE.ID == session.query(TABLES.ZP_TV_TITLE.ID).filter(
                TABLES.ZP_TV_TITLE.ZP_TV_ID == TABLES.ZP_TV_RUNNING.ZP_TV_ID).limit(1).correlate(TABLES.ZP_TV_RUNNING)
        ).join(
            TABLES.ZP_TV, TABLES.ZP_TV.ID == TABLES.ZP_TV_RUNNING.ZP_TV_ID
        ).filter(
            TABLES.ZP_TV.ZP_TV_FILEFOLDER_ID != None
        ).order_by(TABLES.ZP_TV_TITLE.TITLE.asc())
        session.close()
        for tv in tvs:
    ...
    def set_filefolder_eapieid(self, eapi_id, eapi_eid, zp_filefolder_id):
        session = self.Session()
        try:
            zp_tv_eapi_eid = session.query(TABLES.ZP_TV_EAPI_EID).filter(
                TABLES.ZP_TV_EAPI_EID.ZP_EAPI_ID == eapi_id,
                TABLES.ZP_TV_EAPI_EID.ZP_EAPI_EID == eapi_eid
            ).one()
        except orm.exc.NoResultFound:
            ...
            session.query(TABLES.ZP_TV_FILEFOLDER).filter(
                TABLES.ZP_TV_FILEFOLDER.ID == zp_filefolder_id).update({'ZP_TV_ID': zp_tv_eapi_eid.ZP_TV_ID})
            commit(session)
        session.close()
        return True
