                ...
                if image_request_url_match:
                    ...
                    if request_type == 't':
                        ...
                        if serach_params_match:
                            ...
                            text_line(draw, 50, 40, " ".join(search_string), search_string_font, '#ffa500', 'left',
                                      'top', None)
                            if zp_library_id == '1':
                                if search_type == 'a':
                                    text_line(draw, 50, 1028, 'Searching anywhere', search_type_font, '#ffa500', 'left', 'top', None)
                                    y = 190
                                    films = self.session.query(TABLES.ZP_FILM_TITLE.TITLE).filter(
                                    ...
                                    for film in films:
                                        text_line(draw, 65, y, film.TITLE, search_results_font, '#000', 'left', 'top', None)
                                        y += font_size + spacing
                                if search_type == 'b':
                                    text_line(draw, 50, 1028, 'Searching at the begninning', search_type_font, '#ffa500', 'left', 'top',
                                              None)
                                    y = 190
                                    films = self.session.query(TABLES.ZP_FILM_TITLE.TITLE).filter(
                                        TABLES.ZP_USER_FILM_ENTITY_XREF.ZP_FILM_ENTITY_ID == TABLES.ZP_FILM_TITLE.ID,
                                    ...
                                    for film in films:
                                        ...
                                        y += font_size + spacing
                                if search_type == 'e':
                                    text_line(draw, 50, 1028, 'Searching at the end', search_type_font, '#ffa500', 'left', 'top', None)
                                    y = 190
                                    films = self.session.query(TABLES.ZP_FILM_TITLE.TITLE).filter(
                                        TABLES.ZP_USER_FILM_ENTITY_XREF.ZP_FILM_ENTITY_ID == TABLES.ZP_FILM_TITLE.ID,
                                        TABLES.ZP_USER_FILM_ENTITY_XREF.ZP_USER_ID == ZP_USER_ID,
                                ...
                                if search_type == 'a':
                                    text_line(draw, 50, 1028, 'Searching anywhere', search_type_font, '#ffa500', 'left', 'top', None)
                                    y = 190
                                    tvs = self.session.query(TABLES.ZP_TV_TITLE.TITLE).filter(
                                        ...
                                        TABLES.ZP_TV_FILEFOLDER.ID == TABLES.ZP_TV.ZP_TV_FILEFOLDER_ID,
                                        TABLES.ZP_TV_TITLE.TITLE.like(r"""%{0}%""".format(search_string))).order_by(
                                        asc(TABLES.ZP_TV_TITLE.TITLE)).limit(14)
                                    for tv in tvs:
                                        text_line(draw, 65, y, tv.TITLE, search_results_font, '#000', 'left', 'top', None)
                                        y += font_size + spacing
                                if search_type == 'b':
                                    text_line(draw, 50, 1028, 'Searching at the begninning', search_type_font, '#ffa500', 'left', 'top',
                                              None)
