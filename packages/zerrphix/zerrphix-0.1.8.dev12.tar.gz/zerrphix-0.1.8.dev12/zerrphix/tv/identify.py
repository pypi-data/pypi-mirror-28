...
class Identify(TVBase):
    ...
    def add_tv_to_db(self, ZP_TV_FILEFOLDER_ID, eapi_dict):
        ...
        if not eapi_dict:
            raise Exception('eapi_dict: {0} is empty for ZP_TV_FILEFOLDER_ID: {1}'.format(eapi_dict,
                                                                                          ZP_TV_FILEFOLDER_ID))
        ...
        for eapi in eapi_dict:
            try:
                session.query(TABLES.ZP_TV_EAPI_EID).filter(
                    ...
                    TABLES.ZP_TV_EAPI_EID.ZP_EAPI_ID == self.eapi_dict[eapi]).one()
            except orm.exc.NoResultFound:
                pass
            else:
                log.error('eapi_dict[eapi]: {0} for self.eapi_dict[eapi] {1} allready in db'.format(
                    eapi_dict[eapi],
                    self.eapi_dict[eapi]))
                raise SystemExit
                return False
        ...
        session.query(TABLES.ZP_TV).filter(
            TABLES.ZP_TV.ZP_TV_FILEFOLDER_ID == ZP_TV_FILEFOLDER_ID,
            TABLES.ZP_TV.ID != ZP_TV_ID).delete()
        commit(session)
        session.close()
        ...
        for external_source_search_result in external_source_search_result_list:
            ...
            for result_title in result_title_list:
                for search_title in search_title_list:
                    ...
                    if re.match(r"""^(a |the )?{0}$""".format(search_title), result_title, re.I | re.UNICODE):
                    ...
                    elif re.match(r"""^(a |the )?{0}""".format(search_title), result_title, re.I | re.UNICODE):
                        if current_result_dict['name_match_accuracy'] < 90:
                            current_result_dict['name_match_accuracy'] = 90
                            current_result_dict['name_match_search_title'] = search_title
                            current_result_dict['name_match_result_title'] = result_title
                    elif re.search(r"""(a |the )?{0}""".format(search_title), result_title, re.I | re.UNICODE):
