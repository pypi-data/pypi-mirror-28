...
class Entity(TimeStampedModel):
    ...
    @property
    def groups(self):
        return u';'.join([group.name for group in self.group_set.all()])
...
class Contact(TimeStampedModel):
    ...
    @property
    def groups(self):
        groups = [group.name for group in self.group_set.all()]
        if not self.entity.is_single_contact:
            groups += [group.name for group in self.entity.group_set.all()]
