...
class Params(dict, metaclass=ParamsMeta):
    ...
    def __init__(self, other: typing.Dict[str, typing.Any] = None, **values: typing.Any) -> None:
        ...
        if len(missing):
            raise ValueError("Not all parameters are specified: {missing}".format(missing=missing))
        ...
        for name, value in values.items():
            ...
            if not utils.is_instance(value, value_type):
                raise TypeError("Value '{value}' is not an instance of the type: {value_type}".format(value=value, value_type=value_type))
    ...
    def setdefault(self, key, default=None):  # type: ignore
        ...
        if not utils.is_instance(default, default_type):
            raise TypeError("Default '{default}' is not an instance of the type: {default_type}".format(default=default, default_type=default_type))
