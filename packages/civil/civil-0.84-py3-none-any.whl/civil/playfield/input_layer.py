...
class InputLayer(Layer):
    """
    This class defines a layer that plugs into the PlayField. It provides code for asking string
    input from the user and provides buttons for Ok and Cancel. A question that relates to the input
    text is also rendered in the window.

    Note that this layer contains no own eventhandling, so the state of the game that has shown a
    question must track mouse/key presses and ask the layer weather a button was pressed or not. It
    must also call setText() to update the text the layer shows.
    ...
    def __init__(self, name):
        ...
        # load a button
        try:
            self.ok = pygame.image.load(properties.layer_input_ok).convert()
        except:
            # could not load the file
            print("Could not load: " + properties.layer_input_ok + ", reason: " + repr(sys.exc_info()[1]))
            sys.exit(1)

        # load a button
        try:
            self.cancel = pygame.image.load(properties.layer_input_cancel).convert()
        except:
            # could not load the file
            print("Could not load: " + properties.layer_input_cancel + ", reason: " + repr(sys.exc_info()[1]))
            sys.exit(1)
    ...
    def setQuestion(self, question):
        ...
        # set default total height of the contents. add the border twice, once for the upper border,
        # and then for a simple border between the button and the editfield
        self.totalheight = self.ok.get_height() + self.bordery + self.bordery
        self.totalwidth = self.minwidth

        # load the font for the labels
        self.labelfont = pygame.font.Font(properties.layer_input_font, properties.layer_input_font_size)

        # loop over the global location labels we have
        for line in question:
            # create the surface and the shadow
            surface = self.labelfont.render(line, 1, properties.layer_input_label_color)
        ...
        # now add some extra for the editfield, i.e. the height of the label + some space and then
        # some extra around the editfield
        self.editfieldheight = self.labelfont.get_height() + 15
        self.totalheight += self.editfieldheight

    def setText(self, text):
        """
        Sets the new text to be displayed. This is the text that the user has input so far.
        """
        # avoid empty text
        if text == '':
            text = " "

        # store new text as a surface
        self.text = self.labelfont.render(text, 1, properties.layer_input_text_color)
    ...
    def paint(self, offset_x, offset_y, dirtyrect=None):
        ...
        x = (scenario.sdl.getWidth() - width) / 2
        ...
        # paint the field behind the text
        scenario.sdl.fill(properties.layer_input_bg_color,
                          (x, y + 15, width - 2 * self.borderx, self.labelfont.get_height() + 10))

        # now blit out the text for the editfield
        scenario.sdl.blit(self.text, (x, y + 20))

        # get the button positions
        self.okx = scenario.sdl.getWidth() / 2 - (width / 2) + 15
        self.oky = y + self.editfieldheight + 15 + self.bordery
        self.cancelx = scenario.sdl.getWidth() / 2 + (width / 2) - self.ok.get_width() - 15
        self.cancely = y + self.editfieldheight + 15 + self.bordery
