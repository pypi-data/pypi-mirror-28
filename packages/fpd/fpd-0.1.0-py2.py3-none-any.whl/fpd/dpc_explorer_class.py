...
class DPC_Explorer:
    def __init__(self, d, r_min=0, r_max=None, r_min_pct=0, r_max_pct=95,
        ...
        Notes
        -----
        Increasing Y points up, X to the right.
    ...
    def _calc_data(self, calc_ransac=False):
        ...
        # flip
        if self._flip_y:
            y_centre = self.y.min() + self.y.ptp()/2.0
            self.y = 2.0*y_centre - self.y
            
        if self._flip_x:
            x_centre = self.x.min() + self.x.ptp()/2.0
    ...
    def _on_save(self, event, post_tag=None, working_dir=None):       
        ...
        data = np.column_stack((dnames+['pct', 'median', 'ransac', 'ransac_dict', 'flip_y', 'flip_x'],
    ...
    def _on_print(self, event):
        ...
        cmd_str = "r_min=%0.4f, r_max=%0.4f, descan=[%0.4f, %0.4f, %0.4f, %0.4f], cyx=[%0.4f, %0.4f], vectrot=%0.2f, gaus=%0.2f, pct=[[%0.3f, %0.3f], [%0.3f, %0.3f]], dt=%0.2f, median=%s, ransac=%s, ransac_dict=%s, flip_y=%s, flip_x=%s" %(self._r_min, self._r_max, self._descanY_factY*1000, self._descanY_factX*1000, self._descanX_factY*1000, self._descanX_factX*1000, self._yc, self._xc, self._vectrot, self._gaus, self._pct[0][0], self._pct[0][1], self._pct[1][0], self._pct[1][1], self._dt, str(self._median), str(self._ransac), str(self._ransac_dict), str(self._flip_y), str(self._flip_x))
