...
def _gen_mask(rrf, fminmax, gy):
    imr, imc = rrf.shape
    fmin, fmax = fminmax
    gx = gy*float(imc)/imr
    g = (gy, gx)
    mask = np.logical_and(rrf>=fmin, rrf<=fmax)*1.0
    mask = gaussian_filter(mask, g)
    return mask
...
def bandpass(im, fminmax, gy, mask=None, full_out=False):
    '''
...
class BandpassPlotter:
    def __init__(self, im, fminmax=(0, 0.5), gy=2, fact=0.75, cmap='gray', blit=True):
        '''
        ...
        >>> im = np.sin(y/py) + np.sin(x/px)
        
        >>> b = fpd.fft_tools.BandpassPlotter(im)
    ...
    def calc_data(self, first=False, level=0):
        out = bandpass(self.im, (self.fmin, self.fmax), gy=self.gy, full_out=first)
        if first:
            ...
            # update
            if level <= 1:
                self.mask = _gen_mask(self.rrf, (self.fmin, self.fmax), self.gy)
            if level <= 2:
    ...
    def init_fig(self):
        ...
        gs = plt.mpl.gridspec.GridSpec(2, 3, 
                                       left=None, bottom=0.3, right=None, top=None,
        ...
        self.ax_im_dif.xaxis.set_label_position('top')
        ...
        axfmin = plt.axes([0.25, 0.02, 0.65, 0.03], **d)
        axfmax = plt.axes([0.25, 0.07, 0.65, 0.03], **d)
        axsig = plt.axes([0.25, 0.12, 0.65, 0.03], **d)
        axfact = plt.axes([0.25, 0.17, 0.65, 0.03], **d)
    ...
    def init_plots(self):
        self.ax_im.matshow(self.im)
        self.ax_fft.imshow(self.im_fft, extent=self.extent,
                           interpolation='nearest') #, norm=plt.mpl.colors.LogNorm())
        ...
        self.ax_im_dif_im = self.ax_im_dif.matshow(self.im_dif)
        self.ax_im_pass_im = self.ax_im_pass.matshow(self.im_pass)
