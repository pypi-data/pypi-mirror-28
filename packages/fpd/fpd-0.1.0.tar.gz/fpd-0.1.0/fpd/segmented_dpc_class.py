...
class SegmentedDPC:    
    ...
    def _load_ims(self, ds, path, ext, rebin):
        ...
        for fid, im_name in zip(fns, ims):
            ...
            #exec('self.'+im_name+'=load(fpath)')
            #exec('self.'+im_name+'.data=self.'+im_name+'.data.astype(float)')
            #exec('self.'+im_name+'.data-=self.'+im_name+'.original_metadata.ImageList.TagGroup0.ImageData.Calibrations.Brightness.Origin')
            
        # workaround for scale info not in mag yaxis an in both noise axes
        #self.mag.axes_manager[1].scale = self.i0.axes_manager[1].scale
