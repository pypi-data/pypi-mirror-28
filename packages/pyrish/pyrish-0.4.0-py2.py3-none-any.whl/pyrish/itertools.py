...
def isplit(s: str, sep: Optional[str] = '') -> Iterator[Any]:
    ...
    NB: This is not more efficient than `str.split` or intended to replace it.
    But it's very performant when you don't need to get the whole list of element.
    It can be used with combination with ``first``.
...
AnyIter = TypeVar('AnyIter', Sequence, Iterable)


def last_n(iterable: Union[Sequence, Iterable], n: int) -> Generator[Any, None, None]:
    ...
    if is_sequence(iterable):
        ...
        subset = deque(iterable, maxlen=n)
