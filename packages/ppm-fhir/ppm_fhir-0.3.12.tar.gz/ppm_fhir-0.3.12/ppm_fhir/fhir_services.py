...
class FHIRServices:
    ...
    def get_patient_record(self, fhir_id, flatten_return=True):
        logger.debug("[P2M2][DEBUG][get_patient_record: {}]".format(fhir_id))
    ...
    def query_enrollment_accepted_date(self, email):
        ...
        logger.debug("[P2M2][DEBUG][query_enrollment_accepted_date]")
        ...
        try:
            ...
            if bundle.total > 0:
                ...
                for flag in [entry.resource for entry in bundle.entry if entry.resource.resource_type == 'Flag']:
                    ...
                    logger.debug("[P2M2][DEBUG][query_enrollment_accepted_date] Fetched approved date '{}' for user".format(approved_date))

                    return approved_date

            else:

                logger.debug("[P2M2][DEBUG][query_enrollment_accepted_date] No flag found for user")

            return None

        except KeyError as e:
            logger.exception('[P2M2][EXCEPTION][query_enrollment_accepted_date]: {}'.format(e))

            raise

    def get_enrollment_status(self, fhir_id):

        logger.debug("[P2M2][DEBUG][get_enrollment_status]")
    ...
    def remove_patient(self, participant_email, consent_questionnaire_id=None, questionnaire_id=None):
        logger.debug("[P2M2][DEBUG][remove_patient]")
    ...
    def remove_all(self, source_resource_type, source_resource_id, target_resource_types=[]):
        ...
        for resource in [entry['resource'] for entry in entries
            ...
            # Add it.
            logger.debug('[ppm-fhir][FHIRServices.remove_all] Add: {}'.format(resource_id))
        ...
        logger.debug(
            '[ppm-fhir][FHIRServices.remove_all] Delete request:{}'.format(json.dumps(transaction)))
        ...
        # Log it.
        logger.debug('[ppm-fhir][FHIRServices.remove_all] Delete response: {}'.format(response.content))
        logger.debug('[ppm-fhir][FHIRServices.remove_all] Successfully deleted all for resource: {}/{}'
                     .format(source_resource_type, source_resource_id))

        return response

    def remove_consent(self, participant_email):
        logger.debug("[P2M2][DEBUG][remove_consent]")
    ...
    def remove_point_of_care_list(self, participant_email):
        logger.debug("[P2M2][DEBUG][remove_point_of_care_list]")
    ...
    def remove_consent_response(self, participant_email, questionnaire_id):
        logger.debug("[P2M2][DEBUG][remove_consent_response]")
    ...
    def remove_questionnaire(self, participant_email, questionnaire_id):
        logger.debug("[P2M2][DEBUG][remove_questionnaire]")
    ...
    def remove_enrollment_flag(self, participant_email):
        logger.debug("[P2M2][DEBUG][remove_enrollment_flag]")
