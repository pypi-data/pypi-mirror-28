...
def register_firebase_event(request):
    ar = request.adhara_request
    ar.get_criteria_object().set_criteria(get_user_criteria(request))
    rv = RestView(ar, FirebaseEvents)
    if request.method == "POST":
        return rv.put_or_create().serialize()
    return rv.execute().serialize()
