...
class AdharaResponse:
    ...
    def set_permission(self, name, is_permitted):
        self._permissions[name] = is_permitted
    ...
    def _model_to_dict(self, instance, fields=None, exclude=None):
        ...
        self._call_transformer(instance, data)
        return data
    ...
    def transform_row(self, data, fields=None, exclude=None):
        if isinstance(data, Model):
            ...
            if self._permissions:
                dict_response["_permissions"] = self._permissions
    ...
    def _call_transformer(self, row, transformed_row):
        if self._request.transformer:
            self._request.transformer(row, transformed_row)
    ...
    def serialize(self, dict_response=None, response_mode=ResponseModes.JSON):
        if response_mode == ResponseModes.JSON:
            if self._status == ResponseStatus.FAILURE:
                return ApiResponse.AsiddhauPratikriyaa(dict_response or self.get_response())
            elif self._status == ResponseStatus.ERROR:
                return ApiResponse.SkhalitaPratikriyaa(dict_response or self.get_response())
            else:
                return ApiResponse.Pratikriyaa(dict_response or self.get_response())
