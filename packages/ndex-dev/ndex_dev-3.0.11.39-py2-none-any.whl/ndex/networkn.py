...
def data_to_type(data, data_type):
    ...
    if data_type == "boolean":
        ...
        if(type(data) is str):
            ...
            return_data = str(data).lower() == 'true'
        else:
        ...
        elif type(data[0]) is unicode:
            return_data = [str(s).lower() == 'true' for s in data]
        else:
    ...
    def __init__(self, cx=None, server=None, username=None, password=None, uuid=None, networkx_G=None, data=None, **attr):
     ...
     #   self.node_prepresent_map ={} #stores the mapping from cx nodeId to its represents
        ...
        if not cx and server and uuid:
            if username:
                user_agent =  'networkn ndex-user-' + username
            else:
                user_agent =  'networkn '

            ndex = nc.Ndex(server, username, password, user_agent=user_agent)
