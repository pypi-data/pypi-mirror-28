...
__all__ = ['HitCommute', 'calcHitTime', 'calcCommuteTime']

ZERO = 1e-6

class HitCommuteBase(NMA):

    """ Class for hitting and commute time analysis of proteins """
    ...
    def __init__(self, name='Unknown'):
        ...
        self._affinity = None
        self._gamma = None
        self._hitMatrix = None
        self._commuteMatrix = None

    def __repr__(self):

        return ('<{0}: {1} ({3} nodes)>'
                .format(self.__class__.__name__, self._title,
                        self._n_atoms))
...
class HitCommute(HitCommuteBase):
    ...
    def setAffinity(self, affinity):
        ...
        elif (not affinity.ndim == 2 or
              affinity.shape[0] != affinity.shape[1]):
            raise ValueError('affinity must be a square matrix')
        elif affinity.dtype != float:
            try:
                affinity = affinity.astype(float)
            except:
                raise ValueError('affinity.dtype must be float')

        self._reset()
        self._affinity = affinity
        self._n_atoms = affinity.shape[0]
        self._dof = affinity.shape[0]
