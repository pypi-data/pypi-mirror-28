...
class CMFModel:
    ...
    def print_solver_time(self, solver_time, start_time, last_time, step):
        ...
        if self.solver_settings['verbosity']:
            ...
            elapsed_time_print = f'Elapsed Time: {elapsed_time.seconds//3600}:{(elapsed_time.seconds//60)%60}:' \
                                 f'{elapsed_time.seconds%60}'
            current_time_step_print = f'Current Time Step: {current_time.seconds//3600}:' \
                                      f'{(current_time.seconds//60)%60}:{current_time.seconds%60}'
            estimated_time_left_print = f'Estimated Time Left: {time_left.seconds//3600}:' \
                                        f'{(time_left.seconds//60)%60}:{time_left.seconds%60}'
