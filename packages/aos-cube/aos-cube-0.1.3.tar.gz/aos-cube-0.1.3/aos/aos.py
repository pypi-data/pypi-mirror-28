...
def __run_setup():
    ...
    if host_os == 'Win32':
        if which('pip') == None:
            print "error: unable to install pyserial automatically, try install by yourself"
        else:
            popen('pip install --user -qU pyserial', shell=True)
    elif host_os == 'Linux64':
        if which('apt-get') != None:
            popen('sudo apt-get -q update', shell=True)
            install_cmd = 'sudo apt-get -yq install '
            packages  = 'git wget make flex bison gperf unzip python-pip'
            packages += ' gcc-multilib libssl-dev libssl-dev:i386'
            packages += ' libncurses-dev libncurses-dev:i386'
            packages += ' libreadline-dev libreadline-dev:i386'
        elif which('yum') != None:
            install_cmd = 'sudo yum -yq install '
            packages  = 'git wget make flex bison gperf unzip python-pip'
            packages += ' gcc gcc-c++ glibc-devel glibc-devel.i686 libgcc libgcc.i686'
            packages += ' libstdc++-devel libstdc++-devel.i686'
            packages += ' openssl-devel openssl-devel.i686'
            packages += ' ncurses-devel ncurses-devel.i686'
            packages += ' readline-devel readline-devel.i686'
        else:
            print "error: unsupported linux distribution"
        popen(install_cmd + packages, shell=True)
        popen('pip install --user -qU pyserial', shell=True)
    elif host_os == 'Linux32':
        if which('apt-get') != None:
            popen('sudo apt-get update', shell=True)
            install_cmd = 'sudo apt-get -yq install '
            packages  = 'git wget axel make flex bison gperf unzip python-pip'
            packages += ' libssl-dev libncurses-dev libreadline-dev'
        elif which('yum') != None:
            install_cmd = 'sudo yum -yq install '
            packages  = 'git wget make flex bison gperf unzip python-pip'
            ...
            print "error: unkonw linux version"
        popen(install_cmd + packages, shell=True)
        popen('pip install --user -qU pyserial', shell=True)
    elif host_os == 'OSX':
        if which('pip') == None:
            popen('sudo easy_install pip', shell=True)
        popen('pip install --user -qU pyserial', shell=True)

    downloads = {}
    if os.path.isdir('./build/compiler') == False:
        toolchains = {
            'Win32': 'https://code.aliyun.com/vivid8710/aos-toolchain-windows/repository/archive.zip?ref=master',
            'Linux64': 'https://code.aliyun.com/vivid8710/aos-toolchain-linux64/repository/archive.zip?ref=master',
            'Linux32': 'https://code.aliyun.com/vivid8710/aos-toolchain-linux64/repository/archive.zip?ref=master',
            'OSX': 'https://code.aliyun.com/vivid8710/aos-toolchain-osx/repository/archive.zip?ref=master'
            }
...
def install_esptool():
    ...
    try:
        ...
        if os.path.isdir('./tmp') == False:
            os.mkdir('./tmp')
        os.chdir('./tmp')
        urllib.urlretrieve('https://code.aliyun.com/vivid8710/aos-esptool/repository/archive.zip?ref=master', 'esptool.zip')
...
def generate_aos_program(program_path, aos_sdk_path):
    ...
    threading.Thread(target=restore_aos_sdk, args=(program_aos_path, aos_sdk_path,)).start()
...
def new(name, scm='git', component=False, protocol=None):
    ...
    if d_type == 'program':
        ...
        if sys.platform == 'win32':
            window_root_path = get_path_root(d_path)
            print("window root path %s" % window_root_path)
        if aos_sdk_path:
...
def print_component_status(format):
    ...
    if cwd_type == 'program':
        aos_path = Program(os.getcwd()).get_cfg(OS_PATH)
        os_name = 'aos'
    else:
        aos_path = Global().get_cfg(AOS_SDK_PATH)

    aos_components = get_aos_components(aos_path)
    if format == 'json':
        ...
        if cwd_type == 'program':
            ...
            with cd(pd.get_cfg(PROGRAM_PATH)):
                ...
                for path, name in aos_components.items():
                    cube_add = False
                    cube_remove = False

                    rel_aos_path = relpath(aos_path, path)
                    rel_path = os.path.join(os_name, rel_aos_path)

                    if cube_add_components and os.path.dirname(rel_aos_path) in cube_add_components:
                        cube_add = True

                    if cube_remove_components and os.path.dirname(rel_aos_path) in cube_remove_components:
                        cube_remove = True

                    json_components_dict[os.path.dirname(rel_path)] = {'name': name, 'cube_add': cube_add, 'cube_remove': cube_remove}
        else:
            cube_add = False
            cube_remove = False
            for path, name in aos_components.items():
