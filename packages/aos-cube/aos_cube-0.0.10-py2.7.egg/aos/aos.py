...
default_dir_list = ['build', 'include','LICENSE', 'NOTICE', 'README.md', 'app', os.path.join('framework', 'fsyscall'), os.path.join('kernel', 'ksyscall')]
...
def get_target_program(project_path, aos_sdk_path, target):
    ...
    for d in default_dir_list:
        ...
        dst = os.path.join(project_path, 'os', d)
    ...
    for sm in sdk_makefiles:
        sm = sm.strip()
        if sm:
            copy_dir_dict[os.path.abspath(os.path.dirname(os.path.join(aos_sdk_path, sm)))] = os.path.abspath(os.path.dirname(os.path.join(project_path, 'os', sm)))
    ...
    for sp in sdk_prebuild_libs:
        sp = sp.strip()
        if sp:
            copy_dir_dict[os.path.abspath(os.path.dirname(os.path.join(aos_sdk_path, sp)))] = os.path.abspath(os.path.dirname(os.path.join(project_path, 'os', sp)))
    ...
    for si in sdk_includes:
        si = si.strip()
        if si:
            copy_dir_dict[os.path.abspath(os.path.join(aos_sdk_path, si))] = os.path.abspath(os.path.join(project_path, 'os', si))
    ...
    for co in components:
        ...
        dst_dir = os.path.abspath(os.path.join(project_path, 'os', config_dict[co + '_LOCATION']))
        ...
        for sf in src_files:
            sf = sf.strip()
            if sf:
                copy_dir_dict[os.path.abspath(os.path.dirname(os.path.join(src_dir, sf)))] =  os.path.abspath(os.path.dirname(os.path.join(dst_dir, sf)))
    ...
    app = config_dict['APP']
    copy_dir_dict[os.path.abspath(os.path.join(aos_sdk_path, config_dict[app + '_LOCATION']))] = os.path.abspath(os.path.join(project_path, app))
    ...
    with open(os.path.join(aos_sdk_path, 'out', target, 'dependency.json'), 'r') as fin:
        ...
        for co in components:
            location = os.path.join('os', relpath(aos_sdk_path, os.path.abspath(os.path.join(aos_sdk_path, config_dict[co + '_LOCATION'])))) if co != app else app
            if os.path.join('out', target) not in location:
                ...
                component_dep[co] = {'location': location, 'status': 'used', 'dependencies': dep_dict[co], 'ref': ref}
    
    with open(os.path.join(project_path, COMPONENTS), 'w') as fout:
        fout.write(json.dumps(component_dep, sort_keys=True, indent=4))

    pd.set_cfg(OS_PATH, os.path.join(project_path, 'os'))
...
# New command
@subcommand('new',
    dict(name='target', help='Application and board, example helloworld@mk3060'),
...
def new(target, path='.', scm='git', program=False, component=False, aoslib=False, create_only=False, depth=None, protocol=None):
    global cwd_root
    if not target or len(target.split('@')) != 2:
        error("Please set application and board, example: helloworld@mk3060!!!")
     
    d_path = path if os.path.isabs(path) else os.path.abspath(path or os.getcwd())
    ...
    action("Create new %s \"%s\" (%s)" % (d_type, target.split('@')[0], scm))
    p = Program(d_path)
    if d_type == 'program':
        ...
        if not create_only and not p.get_os_dir() and not p.get_aoslib_dir():
            ...
            if  (d == 'aos' and Global().get_cfg(AOS_SDK_PATH)):
               action('AOS_SDK_PATH is already set, use it\'s components.')
            else :     
                try:
        ...
        if d == 'aos':
            ...
            get_target_program(d_path, Global().get_cfg(AOS_SDK_PATH), target)
            p.set_cfg(PROGRAM_PATH, d_path)
            p.set_cfg('target', target)
        
        action("Successfully create (%s) program in %s. " % (target, d_path))
    else:
        p.unset_root(d_path)
        with cd(p_path):
            sync()
...
def get_all_official_component():
    ...
    if aos_sdk_path:
        ...
        action('Please set AOS_SDK_PATH when try to get official component or when new an AliOS-Things program default AOS_SDK_PATH set')
...
def print_component_status():
    ...
    if cwd_type != 'program':
        ...
        for component, path in official_component_dict.items():
            print '| %-30s | %-70s | %-7s |' % (component, os.path.dirname(os.path.join(OS_NAME, relpath(aos_sdk_path, path))), 'used' if component in program_component else "unused")
...
def import_(url, path=None, ignore=False, depth=None, protocol=None, top=True):
    ...
    if os.path.isdir(repo.path) and len(os.listdir(repo.path)) > 1:
        ...
        info("Directory \"%s\" is not empty. Please ensure that the destination folder is empty." % repo.path, 1)
        return

    text = "Importing program" if top else "Add component"
    action("%s \"%s\" from \"%s\"%s" % (text,  os.path.basename(repo.path), formaturl(repo.url, protocol), ' at '+(repo.revtype(repo.rev, True))))
    ...
    if makefile and makefile.endswith('.mk') and os.path.isfile(makefile):
        with open(makefile) as fin:
            for line in fin:
                ...
                if '$(NAME)_COMPONENTS' in line and not line.startswith('#'):
...
def add_components(name, recursive=False):
    official_component_dict = get_all_official_component()
    ...
    if component_name not in official_component_dict:
        warning("Component %s must be in AliOS-Things." % (name))
    ...
    copy_directory(sdk_component_dir, os.path.abspath(os.path.join(os.getcwd(), 'os', relpath(Global().get_cfg(AOS_SDK_PATH), sdk_component_dir))), True)

    location = os.path.join('os', relpath(Global().get_cfg(AOS_SDK_PATH), sdk_component_dir))
    ...
    program_component[component_name] = {'location': location, 'dependencies': dependencies, 'status': 'used', 'origin': origin, 'ref': ref}
    with open(COMPONENTS, 'w') as fout:
        fout.write(json.dumps(program_component, sort_keys=True, indent=4))
    
    if recursive:
        for dep in dependencies:
            add_components(dep, True)
        ...
        if lib.name != OS_NAME:
            ...
            with cd(pd.get_cfg(PROGRAM_PATH)):
                ...
                if lib.name in program_component:
                    ...
                    program_component[lib.name] = {'location': location, 'dependencies': dependencies, 'status': 'used', 'origin': origin, 'ref': ref}
                    with open(COMPONENTS, 'w') as fout:
                        fout.write(json.dumps(program_component, sort_keys=True, indent=4))
...
def remove_components(name, recursive=False):
    ...
    if component_name in program_component:
        ...
        status = program_component[component_name]['status']
        ...
        if os.path.isdir(lib.path) and Repo.isrepo(lib.path):
            ...
            if (not lib.is_local and not lib_repo.is_local and
                ...
                with cd(lib.path):
                    gc, msg = lib_repo.can_update(clean, clean_deps)
                if gc:
                    action("Remove component \"%s\" (changed URL). Will add from new URL." % (relpath(cwd_root, lib.path)))
                    rmtree_readonly(lib.path)
                    repo.unignore(relpath(repo.path, lib.path))
                else:
                    if ignore:
...
def make():
    ...
    if os.path.isfile(os.path.join(os.getcwd(), Cfg.file)):
        ...
        with open(os.path.join(program_path, COMPONENTS), 'r') as fin:
            ...
            root_dir = ['example', 'board', 'kernel', 'platform', 'utility', 'framework', 'tools', 'test', 'device', 'security']
            for component, info in json_components.items():
                location = relpath('os', info['location']) if info['location'].startswith('os') else info['location']
