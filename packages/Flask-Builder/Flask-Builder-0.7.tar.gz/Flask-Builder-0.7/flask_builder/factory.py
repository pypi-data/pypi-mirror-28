...
# !NB: use it till flask-sqlalchemy will be able to use pools
class SQLAlchemyNullPool(SQLAlchemy):
    def apply_driver_hacks(self, app, info, options):
        options.update({
            'poolclass': NullPool,
        })
        super(SQLAlchemyNullPool, self).apply_driver_hacks(app, info, options)


db = SQLAlchemyNullPool()
...
def register_blueprints(app, blueprints=None):
    try:
        blueprints = blueprints or os.listdir('app')
    except FileNotFoundError:
        logger.error('No app folder found, blueprints were not imported')
        return
    for name in blueprints:
        is_dir = os.path.isdir(os.path.join('app', name))
        is_underscored = name.startswith('_')
        if is_dir and not is_underscored:
            try:
