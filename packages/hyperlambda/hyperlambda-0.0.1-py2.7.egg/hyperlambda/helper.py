...
class warp_lambda(object):
    ...
    def __init__(self, description="hyperlambda Function",version=1.0,verbose=1,function_name=None,region=settings.region,zip_file=None,code=None,role=None,handler=None,runtime='python2.7',
                        timeout=180,data={},environment=None,tags=None,security_group_ids=settings.security_group_ids,key_name=settings.key_name,callback_url=None,
    ...
    def create_bash(self):
        ...
        if self.environment !=None:
            for key,itm in self.environment['Variable'].iteritems():
    ...
    def create_instance(self):
        ...
        instance_arg = dict(
            ...
            LaunchSpecification=dict(
                ...
                UserData=base64.b64encode(settings.user_data.format(self.runtime,self.runtime)+self.create_bash())
            ),
            Type='one-time',
        )
        if self.spot_price:
            instance_arg['SpotPrice']=self.spot_price
        if self.key_name:
        ...
        if spot_instance_id:
            ...
            if reservations:
                ...
                self.thread_terminate_timeout_instance = threading.Timer(self.timeout+35,self.terminate_instance,[True])
        ...
        try:
            ...
            if response['InstanceStatuses'][0]['InstanceState']['Name'] != 'running':
                ...
                if self.state == 'success':
                    ...
                    self.finishedtime = datetime.datetime.now()
                    duration = self.finishedtime - self.started_time
                    logging.info("HyperLambda script Execution completed successfully with time duration of {} seconds and response: {}".format(duration.total_seconds(),self.response))
                    ...
                    self.finishedtime = datetime.datetime.now()
                    duration = self.finishedtime - self.started_time
                    logging.error("HyperLambda script Execution Failed with time duration of {} seconds with error: {} seconds".format(duration.total_seconds()),self.message)
                    self.finishedtime = datetime.datetime.now()
                    return 1
