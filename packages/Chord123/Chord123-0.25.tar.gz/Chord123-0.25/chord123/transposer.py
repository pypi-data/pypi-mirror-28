import re
from .music_theory import KEYS_SHARP, KEYS_FLAT, MODE, INCREMENT, isSection
    ...
    def parse(self):
        ...
        for p in self.document.paragraphs:
            ...
            for j in range(len(inline)):
                if isSection(inline[j].text):
                    continue
                inline[j].text = re.sub(r"[A-G][#b]?((?![#A-Za-z])|(?=m[^A-Za-z]?)|(?=sus[^A-Za-z])|(?=dim[^A-Za-z]))", lambda chord: self.transpose(chord.group(0), increment), inline[j].text)
