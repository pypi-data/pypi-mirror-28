...
class Simulator_WS(object):
    ...
    def _send_state(self, to_server):
        ...
        for step in self._sim_steps:
            if step.state:
                ...
                logging.info("missing step in send_state")
        self._sim_steps = []
    ...
    def _on_set_properties(self, from_server):
        ...
        self._prediction_schema = data.prediction_schema
        self._objective_name = data.reward_name
    ...
    def _on_send(self, to_server):
        ...
        method = getattr(self, method_name, lambda: logging.info("finished"))
        method(to_server)
    ...
    def _on_recv(self, from_server):
        ...
        def _raise(*args):
            raise Exception(
                'recv_message: unknown message from server')
    ...
    def _connect(self):
        ...
        try:
            ...
            logging.info("trying to connect: %s", url)
            req = HTTPRequest(
                url,
    ...
    def run(self):
        ...
        if self._ws is None:
            ...
            if message is not None:
                raise RuntimeError("Bonsai Server: {}".format(message))
        ...
        if self._prev_message_type == ServerToSimulator.PREDICTION:
            for step in self._sim_steps:
                ...
                logging.debug("%s", state_message)
                step.state = state_message
                step.reward = reward
                step.terminal = terminal

                if terminal:
                    # synthetic episode start, ignore result
        ...
        if (to_server.message_type):
            ...
            try:
                ...
                raise RuntimeError("Bonsai Server: {}".format(repr(e)))

        # read response from server
        in_bytes = yield self._ws.read_message()
        if in_bytes is None:
            raise RuntimeError("Bonsai Server: Connection closed.")
