...
logging = logging.getLogger('brain')
...
class Brain(object):
    def __init__(self, config, name=None):
        ''' Construct Brain object by passing in
            a Config object with an optional name
            argument
        '''
        self.config = config
    ...
    def update(self):
        """ Requests server for brain information and status """
        logging.debug('Getting %s info...', self.config.brain)
        self._info = _request_info(
            self._brain_url(), self._request_header(), self._proxy_header())
