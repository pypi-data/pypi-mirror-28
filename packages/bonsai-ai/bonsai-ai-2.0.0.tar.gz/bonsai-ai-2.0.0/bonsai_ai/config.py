...
class Config(object):
    ...
    def _parse_config(self, profile):
        ...
        parser.read([config_path, _DOT_BONSAI])

        # read the values
        def assign_key(key):
            if parser.has_option(section, key):
                self.__dict__[key] = parser.get(section, key)

        # get the profile
        section = _DEFAULT
        if profile is None:
            section = _DEFAULT
            if parser.has_option(_DEFAULT, _PROFILE):
                section = parser.get(_DEFAULT, _PROFILE)
        else:
            section = profile
    ...
    def _parse_args(self, argv):
        ...
        parser.add_argument(
            ...
            const='latest',
            default=None)
        parser.add_argument('--verbose', help=_VERBOSE_HELP)
        parser.add_argument('--performance', help=_PERFORMANCE_HELP)
        ...
        if args.verbose is not None:
            ...
            # logging::log().set_enable_all(true);
            pass
