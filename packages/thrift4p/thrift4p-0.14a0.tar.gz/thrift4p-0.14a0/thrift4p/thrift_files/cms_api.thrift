...
service CmsService {
   ...
   //插入打款详情
   BatchResult insertDetail(1:string batchId,2:string userId,3:string balance,4:string allOrderNum,5:string actualOrderNum);
   //更新批次状态
   BatchResult closeBatch(1:string batchId);

   // 出租车订单挂起加扣款类型
   Result taxiOrderHoldAndDebit(1:i64 taxi_order_id, 2:string holdContent, 3:bool isSend, 4:i64 sys_op_id,5:i32 debitType,6:string debitMoney);
