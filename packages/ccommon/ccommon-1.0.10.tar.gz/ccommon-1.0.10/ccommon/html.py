...
class Html:
    ...
    def draw(self, x_names=range(10), l_range=[0, 10], r_range=None, x_name=None, l_name=None, r_name='', title=None,
             l_compare=[], r_compare=[], file_name=None, dpi=100, fname=None):
        import matplotlib
        matplotlib.use('Agg')
        ...
        if x_name is not None:
            ax.set_xlabel(x_name, fontproperties=font)

        if l_range is not None:
            ax.set_ylim(l_range[0], l_range[1])

        if l_name is not None:
            ax.set_ylabel(l_name, fontproperties=font)
        ...
        if r_range is not None:
            ...
            if r_name is not None:
                ax2.set_ylabel(r_name, fontproperties=font)
    ...
    def setImg(self, x_names=range(10), l_range=[0, 10], r_range=None, x_name=None, l_name=None, r_name='', title=None,
               l_compare=[], r_compare=[], dpi=100, fname=None):
        file_name = 'default.png'
        self.draw(x_names, l_range, r_range, x_name, l_name, r_name, title, l_compare, r_compare, file_name, dpi,fname)
