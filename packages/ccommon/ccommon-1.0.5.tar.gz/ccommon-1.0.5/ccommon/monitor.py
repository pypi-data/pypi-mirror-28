...
class ItemCost:
    def __init__(self, min=-1, max=0, count=0, cost=0, ave=0):
        self.min = min
        self.max = max
        self.count = count
        self.cost = cost
    ...
    def loadSum(self):
        if self.count == 0:
            return {'min': 0, 'max': 0, 'count': 0, 'cost': 0, 'ave': 0}
        else:
            return {'min': self.min, 'max': self.max, 'count': self.count, 'cost': self.cost,
                    'ave': float(self.cost) / float(self.count)}
