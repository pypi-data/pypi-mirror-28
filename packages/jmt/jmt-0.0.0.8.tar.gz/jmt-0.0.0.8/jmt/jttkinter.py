...
def inspectMainScriptForModules():
    ...
    for submods,jtflist in jmtfuncs.items():
        for val in jtflist:#do something with functions list
            if val in txt:
                
                try: funcsUsed[submods]+=[val]
                except: funcsUsed[submods]=[val]
    ...
    for FUmods,FUfuncs in funcsUsed.items():
        for FUfunc in FUfuncs:
            ...
            txtfunc=getFuncText(FUmods,FUfunc)
