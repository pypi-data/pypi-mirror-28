...
from .exceptions import OrmException, OrmParameterError, OrmModelDeclarationError
...
class OrmSessionAbstract(object):
    ...
    def set_time_zone(self, time_zone):
        ...
        @time_zone:             (str) The time zone. Use MySQL correct known time zone.
                                (None) If 'None' - reset DBMS time zone to the global default.
        """
        self._check_opened()
        self.time_zone = time_zone
        # TODO! Check PostgreSQL timezone syntax
        if self.time_zone is None:
            self.execute("SET time_zone = @@global.time_zone")
        else:
            self.execute("SET time_zone=%(time_zone)s", {'time_zone': self.time_zone})
    ...
    def inqure_session_user(self):
        ...
        if isinstance(self.session_user, OrmModel):
            pk = self.session_user.__pk__.popitem()
            return pk[1]
        return self.session_user

    def begin(self):
        """ Begins transaction. `START TRANSACTION` using MySQLdb library interface. """
        self._check_opened()
        self.cn.begin()

    def rollback(self):
        """ Rolls back the last transaction. `ROLLBACK` using MySQLdb library interface. """
        self._check_opened()
        self.cn.rollback()

    def commit(self):
        """ Commits the transaction. `COMMIT` using MySQLdb library interface. """
        self._check_opened()
        self.cn.commit()
    ...
    def execute(self, request, parameters=None):
        """ Executes the raw SQL query which not returning anything.

        @request:       Textual raw SQL query to execute to;
        @parameters:    (optional) (dict) with parameters for the query;

        Parameters in the query may be used using s(parameter_name)% format.
        """
        if not request:
            return
        ...
        do_debug = self.debug is True \
            or (isinstance(self.debug, str)
                and (
                    ...
                    or ('alter' in self.debug and rql[:6] == 'create' or rql[:5] == 'alter' or rql[:4] == 'drop')
                ))
        if do_debug:
            print("")
            print(">> %s" % request)
    ...
    def query(self, request, parameters=None):
        """ Executes the raw SQL query and return `OrmRawResult` instance
        with results of the query.

        @request:       Textual raw SQL query to execute to;
        @parameters:    (optional) (dict) with parameters for the query;

        Parameters in the query may be used using s(parameter_name)% format.
        The results must be taken using '.all()' or '.one()' methods of the
        `OrmRawResult` instance:
            session.execute("SELECT id FROM mytable").all()
        """
    ...
    def new(self, cls, *args, **kwargs):
        """ Creates a new object instance of given `cls` type and automatically assigns it with
        this session. """
...
class MysqlSession(OrmSessionAbstract):
    ...
    def get_table_struct(self, table):
        """ Reads the table structure from DBMS of the OrmModel.
        Reads attributes definitions and keys, including indexes,
        primary keys, foreign keys, unique keys, etc. """
        keys = {}
        columns = {}
        indexes = {}
        foreign_keys = {}
    ...
    def create_model_table(self, model, apply=True, restrict_new_fk=False):
        ...
        @returns:       (str) SQL query for ALTER the table. """
