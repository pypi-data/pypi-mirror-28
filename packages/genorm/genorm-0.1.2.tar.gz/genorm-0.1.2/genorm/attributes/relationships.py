...
class RelatedAbstract(AttributeAbstract):
    def __init__(self, foreign, on_delete=None):
...
class RelatedToOneAbstract(RelatedAbstract):
    def __init__(self, foreign, on_delete=None, always_deferred=False):
        super(RelatedToOneAbstract, self).__init__(foreign, on_delete=on_delete)
...
class UserIdentAbstract(ManyToOne):
    ...
    def on_model_declared(self):
        ...
        fpk = fpk[0]
        foreign = "%s.%s" % (fmodel.__class__.__name__, fpk.model_k)
        c = Relationship(foreign, on_delete=FK_ONDELETE_SET_NULL)
        c.declaration_order = 5
        k = "%s_id" % self.model_k
