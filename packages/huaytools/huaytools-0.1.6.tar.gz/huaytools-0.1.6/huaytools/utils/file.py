...
def get_dir_filenames(dirpath, abspath=True, recursive=True, yield_=False):
    """获取目录下所有文件名 （默认递归）
    ...
    Examples:
        ...
        >>> fs_list = get_dir_filenames('D:/Tmp')
        >>> fs_gen = get_dir_filenames('D:/Tmp', yield_=True)

        %timeit hy.get_dir_filenames('D:/Tmp')
        182 ms ± 1.13 ms per loop (mean ± std. dev. of 7 runs, 10 loops each)

        %timeit hy.get_dir_filenames('D:/Tmp', yield_=True)
...
def rename_batch(dirpath, rename_fn=rename_replace_sep, recursive=False):
    """文件批量重命名
