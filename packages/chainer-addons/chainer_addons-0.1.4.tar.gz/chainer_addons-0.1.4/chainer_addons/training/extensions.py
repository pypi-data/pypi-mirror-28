...
class ContiniousLearningRate(extension.Extension):
	...
	def __init__(self, attr, lr, target, epochs, offset, optimizer=None):
		...
		# lr * factor^epoch => current_lr
		# lr * factor^epochs = target -> estimate factor
		# self._factor = pow(target / lr, 1 / epochs)

		# alternative with log10
		self._factor = pow(10, np.log10(target / lr) / epochs)

	def __call__(self, trainer):
		if trainer.updater.epoch - self._offset > self._epochs: return

		new_lr = self._lr * self._factor ** (trainer.updater.epoch - self._offset)
		self.set_lr(trainer, new_lr)
