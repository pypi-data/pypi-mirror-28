...
class TestRecordArtifactsAsDict(unittest.TestCase):
  ...
  def setUpClass(self):
    ...
    open("foo", "w").write(u"foo")
    open("bar", "w").write(u"bar")

    os.mkdir("subdir")
    os.mkdir("subdir/subsubdir")
    open("subdir/foosub1", "w").write(u"foosub")
    open("subdir/foosub2", "w").write(u"foosub")
    open("subdir/subsubdir/foosubsub", "w").write(u"foosubsub")
