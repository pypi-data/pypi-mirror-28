...
from .tools import config_read, cfg_save_search
...
## on the command line. some remarks for the fields:
# duedate:   YYYY-MM-DD
# estimate:  "2h 35m"
# labels:    either a CSV string or an actual yaml list
# epic_link: holds a jira ticket ID (only for non-epics)
# epic_name: a description string   (only for epics)
"""
...
def perform_search(jql, limit=250):
    jira_obj = get_jira()
    ...
    # continue either forever with breaking inside or until limit is reached
    while limit == 0 or len(results) < limit:
        # TODO - let this go to stderr and remove the thing
        print(".", end="", flush=True)
        results += jira_obj.search_issues(jql, startAt=index, maxResults=limit)
        # actually, if no new results came in - also break
        delta = len(results) - last
        if delta == 0: break
        index += delta
        last = len(results)
...
def search_wrapper(searchstring, save_as=None, just_save=False, limit=50):
    if not just_save:
        perform_search(searchstring, limit=limit)
    if save_as:
...
def create_dict_jira(input_dict):
    """
    ...
    :param input_dict: A cleaned (!) creation dict
    """
...
def create_dict_prep(create_dict):
    """
    For further processing we need (and want ;) to clean the creation dict.
    First we want to remove any None and '' elements, cause they don't have
    semantic value in JIRA. Second, we want to trim all string fields.

    :param create_dict: The un-cleaned creation dict
    :return: The nice-and-clean creation dict
    """
    create_dict = {k: v.strip() if isinstance(v, str) else v
