...
def new_ticket(field_values, noi):
    ...
    user_input = editor.edit(contents=template.encode("utf-8"))

    # convert to yaml and strip values to look nice
    create_dict = yaml.load(user_input)
    create_dict = {k: (v.strip() if v is str else v)
                   for k, v in create_dict.items()}
    print(create_dict)

    # ask user one last time
    ...
    # go for it
    # https://is.gd/cyFsu4
    create_dict = construct_create_dict(create_dict)
    get_jira().create_issue(fields=create_dict)
