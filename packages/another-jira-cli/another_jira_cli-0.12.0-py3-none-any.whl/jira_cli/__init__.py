...
def log_work(date, comment, dry_run, ticket, logged_work):
    """
    Create a work log entry in a ticket.

    The simplest way of calling this command is:

        TICKET_ID WORKLOG_STRING(S)
    ...
    * "20150110" the 10th of January in the year 2015.
    ...
    worklog_date = parse_date_str(date)
    worklog_date = worklog_date.replace(hour=9, minute=0, second=0)
    # TODO - remove default timezone!!
    print("DATE           : {}".format(worklog_date.to('Europe/Berlin').format()))
    print("WORKLOG STRING : {}".format(worklog_string))
    print("WORKLOG COMMENT: {}".format(comment if comment else '<empty>'))
    if dry_run:
...
def fill_day(comment, day_hours, day_start, ticket, save, date):
    ...
    just_worklogs = [tup[1] for tup in tickets_and_logs]
    use_start_time = start_time
    if len(just_worklogs) > 0:
        use_start_time = max([arrow.get(wl.started)
