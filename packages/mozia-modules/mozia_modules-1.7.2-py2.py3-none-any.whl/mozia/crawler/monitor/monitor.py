    ...
    def check_product_sizes(self, task, source):
        ...
        for sku in platform_product['skus']:
            ...
            if not sku_matched:
                ...
                if sku_matched.get('supply_price'):
                    f_supply_price = round(float(sku_matched.get('supply_price')), 2)
                if sku_matched.get('original_price'):
                    f_original_price = round(float(sku_matched.get('original_price')), 2)

                if sku.get('supply_price'):
                    f_old_supply_price = float(sku.get('supply_price'))
                if sku.get('original_price'):
                    f_old_original_price = float(sku.get('original_price'))
                item = {
