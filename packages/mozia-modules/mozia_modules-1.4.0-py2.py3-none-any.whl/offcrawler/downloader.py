...
def download_images(task):
    ...
    cover = oss_urls[0]
    save_product_images(task["product_id"], cover, oss_urls)
    save_cover_to_dir(task, cover)


def save_product_images(product_id, cover, oss_urls):
    cover = "%s/cover.jpg" % os.path.dirname(cover)
    dao.platform.save_product_images(product_id, cover, oss_urls)
...
def get_today_covers():
    ...
    cover_dir = "/home/develop/crawler/data/covers/%s" % today
    if os.path.isdir(cover_dir):
        return [name.replace("-", "/") for name in os.listdir(cover_dir)]
    else:
        os.makedirs(cover_dir)
        return []
