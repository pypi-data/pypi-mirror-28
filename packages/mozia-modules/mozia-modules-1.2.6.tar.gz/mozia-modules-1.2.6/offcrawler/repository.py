...
class TaskRepository(Repository):
    KEY_PRODUCT_TASKS = "net a porter:product:tasks"
    KEY_PRODUCT_IMAGE_TASKS = "platform:product_image:tasks"
    KEY_PRODUCT_IMAGE_PS_TASKS = "platform:product_ps_task:tasks"
    ...
    def create_product_tasks(self):
        ...
        for source in product_sources:
            task_scheduler.push(self.KEY_PRODUCT_TASKS, json.dumps(source))
    ...
    def get_product_image_task_length(self):
        return task_scheduler.length(self.KEY_PRODUCT_IMAGE_TASKS)

    def create_product_image_task(self):
        task_length = self.get_product_image_task_length()
        if task_length > 0:
            print ("no create tasks, has task in queue:", task_length)
            return
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(sql)
            product_image_tasks = cursor.fetchall()

        for image_task in product_image_tasks:
            print("create product image task:", image_task)
            task_scheduler.push(self.KEY_PRODUCT_IMAGE_TASKS, json.dumps(image_task))

    def get_product_image_task(self):
        product_image_task = task_scheduler.pop(self.KEY_PRODUCT_IMAGE_TASKS)
        return json.loads(product_image_task) if product_image_task else None

    def set_product_image_task_status(self, product_id, task_status):
        sql = """
        UPDATE spider.t_product_image_task SET `status` = %s WHERE product_id=%s
        """
        with self.connection.cursor() as cursor:
            cursor.execute(sql, (task_status, product_id))
        self.connection.commit()

    def create_product_image_ps_task(self, product_id, cover):
        self.set_product_image_task_status(product_id, 1)
        task_scheduler.push(self.KEY_PRODUCT_IMAGE_PS_TASKS, json.dumps({
