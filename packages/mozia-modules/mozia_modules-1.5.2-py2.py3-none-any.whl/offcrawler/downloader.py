...
from offcrawler.tools.oss_utils import download_file_from_images_media
from offcrawler.tools import save_file_to_ftp, TRANSFER_COVER_DIR, PROCESS_COVER_DIR, DOWNLOAD_COVER_DIR
import os
...
def save_product_images(product_id, oss_urls):
    ...
    # dao.task.set_product_image_task_status(product_id, 1)
...
def save_oss_cover_to_ftp(cover_url):
    print(cover_url)
...
def start_download_cover_to_ftp(sleep_seconds=30):
    while True:
        task = dao.task.get_ftp_cover_task()
        while task:
            save_oss_cover_to_ftp(task["cover_url"])
            task = dao.task.get_ftp_cover_task()
        print("no ftp cover task, sleep...", time.time())
        time.sleep(sleep_seconds)


def get_today_covers():
    today = time.strftime('%Y%m%d', time.localtime(time.time()))
    cover_dir = "%s/%s" % (PROCESS_COVER_DIR, today)
