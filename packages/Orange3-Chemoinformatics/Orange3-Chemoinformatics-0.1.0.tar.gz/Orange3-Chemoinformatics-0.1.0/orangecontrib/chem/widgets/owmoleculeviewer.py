...
_ImageItem = namedtuple(
    ...
    ["index",   # Row index in the input data table
     ...
     "future"]  # Future instance yielding a pixmap
)
...
class OWMoleculeViewer(widget.OWWidget):
    ...
    def _updateStatus(self):
        ...
        self.info.setText(
            "Retrieving:\n" +
            "{} of {} molecules".format(self._successcount, count))

        if self._errcount + self._successcount == count:
            if self._errcount:
                self.info.setText(
                    "Done:\n" +
                    "{} molecules, {} errors".format(count, self._errcount)
                )
            else:
                self.info.setText(
                    "Done:\n" +
                    "{} molecules".format(count)
                )
            attr = self.allAttrs[self.smilesAttr]
            if self._errcount == count:
                self.error("No SMILES found! Make sure the '%s' attribute "
                           "contains SMILES notations" % attr.name)
