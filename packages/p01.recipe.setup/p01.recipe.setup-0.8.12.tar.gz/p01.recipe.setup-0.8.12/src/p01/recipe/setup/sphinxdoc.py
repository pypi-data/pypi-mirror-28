...
class SphinxDocRecipe(CHOWNMixin, CHMODMixin, LoggerMixin):
    ...
    def getSourceDirectory(self, pkg=None):
        ...
        if pkg is not None:
            sourcedir = os.path.join(pkg.location,
                pkg.project_name.replace('.','/'))
        else:
            sourcedir = self.options.get('source', )
        ...
        if not os.path.isdir(sourcedir):
            raise zc.buildout.UserError("Source directory %s does not exist" %
                sourcedir)
        return os.path.normpath(sourcedir)
    ...
    def install(self):
        ...
        if self.pkgName is not None:
            ...
            msg = '\n'.join(infos)
            data = dict(message_from_string(msg).items())
            metadata = {
        ...
        # get source directory
        sourcedir = self.getSourceDirectory(pkg)
