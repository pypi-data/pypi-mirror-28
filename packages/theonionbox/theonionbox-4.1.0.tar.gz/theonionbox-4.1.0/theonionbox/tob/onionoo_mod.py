...
# Patching FuturesSession
from .sourced.session import FuturesSession


class tobFuturesSession(FuturesSession):
...
class OnionOOFactory(object):
    ...
    def __init__(self, proxy):
        ...
        # self.executor = ThreadPoolExecutor(max_workers=10)     # enough to query > 100 Tors at once...
        self.query_lock = RLock()
        self.onionoo_lock = RLock()
        self.hidden_index = 1
        self.is_refreshing = False
        self.new_nodes = {}
        # self.logger = logging.getLogger('theonionbox')
        self.active_futures = list()

    def add(self, fingerprint):

        lgr = logging.getLogger('theonionbox')

        # Ensure that this is a key ... at least a kind of!
        ...
        if len(self.onionoo) > old_len:
            lgr.debug('Fingerprint {} added to onionoo query queue.'.format((fingerprint)))
            return True
    ...
    def remove(self, fingerprint):
        ...
        if len(self.onionoo) < old_len:
            lgr.debug('Fingerprint {} deleted from onionoo query queue.'.format((fingerprint)))
            return True
    ...
    def refresh(self, only_keys_with_none_data=False, async_mode=True):
        ...
        if len(self.onionoo) < 1:
            return False

        # self.is_refreshing = True
        lgr.info('Querying Onionoo for Tor network data: Only New = {} | Async = {}'.format(only_keys_with_none_data, async_mode))

        # Prepare the session
        with (tobFuturesSession(max_workers=len(self.onionoo)) if async_mode is True else Session()) as session:
            ...
            # Python version specific way to iterate through the 'onionoo' dict.
            # Both ways accept if we change the dict between iteration steps!
            if py30:
                iter_onionoo = self.onionoo.items
            else:
                iter_onionoo = self.onionoo.viewitems

            # run through the dict of keys and query onionoo for updated documents
            for key, document in iter_onionoo():
                # item = self.onionoo[key]

                if only_keys_with_none_data is True:
                    if document.has_document() is True:
                        continue

                try:
                    data_type, fingerprint = key.split(':')
                except ValueError:
                    # This definitely is weird!
                    continue

                # Now we put together the query details...
                ...
                headers = {'accept-encoding': 'gzip'}
                if len(document.ifModSince) > 0:
                    headers['if-modified-since'] = document.ifModSince

                query_address = query_base + '/' + data_type

                # response callback hook
                process_response_hook = {'response': partial(self.process_query_response, document)}

                # Here we go...
                try:
                    if async_mode is False:
                        lgr.info("Onionoo: Querying '{}' for ${}.".format(query_address,
                                                                           fingerprint))

                    r = session.get(query_address, params=payload, headers=headers, timeout=10,
                                    hooks=process_response_hook)

                    if async_mode is True:
                        lgr.info("Onionoo: Querying '{}' async ({}) for ${}.".format(query_address,
                                                                           r,
                                                                           fingerprint))

                    # if we're calling async, r(esponse) is a f(uture)!
                    if async_mode is True:
                        try:
                            if not r.done():
                                self.active_futures.append(r)
                        except:
                            pass


                except Exception as exc:
                    lgr.info("Onionoo: Querying '{}'{}for ${}. failed: ".format(query_address,
                                                                                ' async ' if async_mode is True else ' ',
                                                                                fingerprint,
                                                                                exc))

        lgr.debug('active_futures: starting @ {}'.format(len(self.active_futures)))
        ...
        for future in self.active_futures:
            ...
            if future.done() is True:
                msg = 'done'
            elif future.cancelled() is True:
                msg = 'cancelled'
            elif future.running() is True:
                msg = 'running'
                f_list.append(future)

            lgr.debug('{} is {}.'.format(future, msg))

            try:
                x = future.result()
            except Exception as exc:
                lgr.debug('{} reports Exception: {}'.format(future, exc))

        self.active_futures = f_list

        lgr.debug('active_futures: after cleaning = {}'.format(len(self.active_futures)))

    def process_query_response(self, for_document, response, *args, **kwargs):
        ...
        for_document.ifModSince = response.headers['last-modified']
        ...
        try:
            ...
            # lgr.info("Onionoo: Failed to un-json network data; error code says '{}'.".format(exc))
            return
