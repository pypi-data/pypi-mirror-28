...
from .formel import voigt,Elements,felectron,waterdensity,watercompressibility,loglist
...
def readpdh(pdhFileName):
    ...
    A header looks like this::
     ...
     SAXS 
     470         1         0         0         0         0         0         0 
     0.000000E+00   0.264500E+03   0.000000E+00   0.100000E+01   0.154200E+00 
     0.818682E+02   0.306286E-01   0.181261E-02   0.154053E+00   0.100000E+01 
     .....
    ...
    data.beamProfType=_beamProfType.setdefault(data.line_2[1],0)
    #forth line 
    ...
    #beam-length profile parameter a
    if data.line_4[0]!=0: data.a =   data.line_4[0]
    #beam-length profile parameter b
    if data.line_4[1]!=0: data.b =   data.line_4[1]
    #half-width of the beam-width profile (at half-height)
    if data.line_4[2]!=0: data.bxw = data.line_4[2]
    #detector slit length (equivalent to width of integration area
    if data.line_4[3]!=0: data.dIW =  data.line_4[3]
    #qscale if given , default=1/nm for SAXSPACE
    if data.line_4[4]==1:
        data.qscale='A^-1'
    else:
        data.qscale='nm^-1'
...
def _wBLength(beam):
    ...
    The relevant integration width is detected automatically.
    ...
    if beam.beamProfType[0] =='m'  :
        ...
        if dIW==0:
            ...
            #weight=np.trapz(profile.Y[interval],profile.X[interval])
...
def _wBWidth(beam):
    ...
    if beam.bxw==0:
        return None
    sigma = beam.bxw/np.sqrt(np.log(2.0))
    # points with maximum width 2.5*sigma
    x=np.r_[-2.5*sigma:2.5*sigma:21j]
    return np.c_[x,np.exp(-x**2/sigma**2)/(np.sqrt(np.pi)*sigma)].T
...
def _smear(q,data):
    ...
    if wx is None:
        if wy is None:
            ...
            val=interpolate.splev(qy.flatten(),data.tckUnsm,der=0).reshape(qy.shape)
            valwy=val*wy[1][None,:]
        ...
        if wy is None:
            # only x
            qx=q[:,None]+wx[0]
            val=interpolate.splev(qx.flatten(),data.tckUnsm,der=0).reshape(qx.shape)
            valwx=val*wx[1][:,None]
            valq=integrate(valwx,wx[0],axis=-1)
        else:
            # smear over both x and y
            qxy=np.sqrt(((q[:,None]+wx[0])**2)[:,:,None]+wy[0]**2)
            val=interpolate.splev(qxy.flatten(),data.tckUnsm,der=0).reshape(qxy.shape)
...
def desmear(Ios,beamProfile,maxIterations=15, windowsize=3,**kwargs):
    """
    Desmearing according to Lake algorithm with a convergence criterion to stop recursion at best desmearing.
    
    The convergence criterion reaches a minimum and the Lake algorithm [1]_ is stopped as described in [2]_.
    In each step a smoothing based on the ratio desmeared/observed as described in [2]_ is used (point average with windowsize 3).
    ...
    while True:#decreasing:
        ...
        Idesmeared[-1].Y=_smooth(Idesmeared[-2],Idesmeared[0],windowsize)*gamman # calc iteration with smoothing
        meangamma=np.abs(gamman.mean()-1)                                        # calc convergence criterion
        ...
        try:
            if (Idesmeared[-1].decreasing == False) and (Idesmeared[-2].decreasing == False):
                break
        except :
            pass
        if len(Idesmeared)>maxIterations:
            print('len(Idesmeared)>maxIterations', len(Idesmeared))
            break
    return  Idesmeared[Idesmeared.convergence.array.argmin()]

def _smooth(desmeared,observed,windowsize=3):
    """
    Iterative smoothing based on ratio desmeared/observed
    
    Parameters
    ----------
    desmeared,observed : dataArray
        desmeared and observed data
    windowsize : int
        windowsize for averaging
        
    References
    ----------
    ..[1] Comparison of iterative desmearing procedures for one-dimensional small-angle scattering data
          Vad and Sager J. Appl. Cryst. (2011) 44,32-42
    
    """
    windowsize=max(windowsize,1)
    ratio=desmeared.copy()
    ratio.Y=desmeared.Y/observed.Y
    ratio=ratio.prune(number=int(len(desmeared.Y)/windowsize))
    return observed.Y*ratio.interp(observed.X)
...
def prepareBeamProfile(data=None,**kwargs):
    ...
    bxw : float
        Beam half-width (at half-height) that defines the Gaussian to describe the width profile in detector q units.
        See getBeamWidth to fit to a measurement.
    dIW : float
...
def getBeamWidth(empty,minmax=[-0.03,0.03],showfit=False):
    """
    ...
    ----------
    empty : dataArray
        Measurement with the transmitted beam included.
        This is used within min max to fit a Gaussian.
    minmax : [float,float]
    ...
    Returns
    -------
        float as the beam width

    """
    gauss=lambda x,A,mean,sigma,bgr:A*np.exp(-0.5*(x-mean)**2/sigma**2)/sigma/np.sqrt(2*np.pi) + bgr
    ymin=empty.Y.min()
    ymax=empty.Y.max()
    if minmax[0]=='a': # auto
        imax=empty.Y.argmax()
        while empty.Y[imax+1]<empty.Y[imax]:
            imax+=1
        xmax=empty.X[imax]
        imin=empty.Y.argmax()
        while empty.Y[imin-1]<empty.Y[imin]:
            imin-=1
        xmin=empty.X[imin]
    else:
        xmin=minmax[0]
    ...
    if showfit:
        empty.makeErrPlot()
    empty.fit(gauss,{'mean':0,'sigma':0.015,'bgr':ymin,'A':ymax-ymin},{},{'x':'X'},condition=lambda a:(a.X<xmax) & (a.X>xmin) )
    return empty.sigma
...
def addXMLParameter(dataa):
    ...
    """
    if hasattr(dataa,'_isdataArray'):
        datalist=dL(dataa)
    elif hasattr(dataa,'_isdataList'):
        datalist=dataa
    else:
        raise Exception(dataa,' is not dataArrau or dataList')
    for data in datalist:
        f=open(data.name[0])
        lines=filter(lambda a:len(a.strip())>0 and a.strip()[0]=='<',f.readlines())
        f.close()
        root=xml.etree.ElementTree.fromstringlist(lines)
        for par in root.iter('parameter'):
            setattr(data,par[2].text,[_w2f(par[ii].text) for ii in (3,1) ])
            # return data
...
def copyfiles(pattern,root=os.curdir,destination='copy',link=False):
    ...
    for file in files:
        newname=os.path.join(destination,os.path.basename(file))
        print( newname)
        if not link:
...
def moveSAXSPACE(pattern,root='./',destination='./despiked',skip='BeamProfile',medwindow=5,SGwindow=5,sigma=0.2,
    ...
    Notes
    -----
    Default values are adjusted to typical SAXSPACE measurement SAXS
    not to reduce peaks and transmission peak and overshot region excluded
    sigma =0.2 because the measurement error seems to be to big
    ...
    if not os.path.exists(destination):
        os.mkdir(destination)
    for file in files:
        if 'BeamProfile' in file: continue
        newname=os.path.join(destination,os.path.basename(file))
        print( file,'->',newname)
        f=open(file,'rU')
        ...
        data=dA(numbers)
        datanew=removespikes(data,xmin=0.12,medwindow=medwindow,SGwindow=SGwindow,sigma=sigma,order=order)
        f=open(newname,'w')
        f.writelines(header)
