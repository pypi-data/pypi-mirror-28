        ...
        for dataset in datasets:
                ...
                if isinstance(kwargs[keyword],Symbol):
                    ...
                    for key,i,l in zip(keys,kwargs[keyword],range(len(keys))):
                        ...
                        if isinstance(i,(int,float)):
                            if i>=0:
                                self.lastsymbol[l]=i%self.symbolmax[l]
                                setattr(symbol,key,self.lastsymbol[l])
                                if key=='fillcolor': # a default for given color
                                    setattr(symbol,'fillpattern',1)
                            else:
                                self.lastsymbol[l]=(self.lastsymbol[l]+abs(i)-1)%self.symbolmax[l]+1
                                setattr(symbol,key,self.lastsymbol[l])
                        else:
                            setattr(symbol,key,self.lastsymbol[l])
            else:
                self.lastsymbol[0]=(self.lastsymbol[0])%self.symbolmax[0]+1
                self.lastsymbol[2]=(self.lastsymbol[2])%self.symbolmax[2]+1
            ...
            if 'line' in kwargs or 'li' in kwargs:
                ...
                if isinstance(kwargs[keyword],Line):
                    ...
                    #type=None,linestyle=None,linewidth=None,color=None
                    line=Line(type=1,color=self.lastline[2])
                    if isinstance(kwargs[keyword],(int,float)):
                    ...
                    for key,i,l in zip(keys,[1]+kwargs[keyword],range(len(keys))):
                        if isinstance(i,(int,float)):
                            if i>=0:
                                self.lastline[l]=i%self.linemax[l]
                            else:  #increment for negative
                                self.lastline[l]=(self.lastline[l]+abs(i)-1)%self.linemax[l]+1
                        # sync for non integer with symbol color
                        if key is 'color'and not isinstance(kwargs[keyword][2], int):  self.lastline[l]=self.lastsymbol[2]
                        # avoid zero as white color
                        if key is 'color' and self.lastline[l]==0:  self.lastline[l]=1
