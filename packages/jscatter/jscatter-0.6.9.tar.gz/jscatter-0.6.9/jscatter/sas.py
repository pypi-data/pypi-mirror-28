...
def getBeamWidth(empty,minmax=[-0.03,0.03],showfit=False):
    """
    Fit Gaussian to primary beam of measurement.
    ...
    if showfit:
        empty.makeErrPlot()
    empty.fit(_gauss,{'mean':0,'sigma':0.015,'bgr':ymin,'A':ymax-ymin},{},{'x':'X'},condition=lambda a:(a.X<xmax) & (a.X>xmin) )
    return empty.sigma
...
def transmissionCorrection(data, dark, edge=0.03, emptybeam=None, exposure=None):
    r"""
    ...
    The primary peak maximum is searched below edge.
    ...
    emptybeam : dataArray
        Empty beam measurement. If not provided the empty beam transmission is set to 1.
    edge : float, default 0.03
        Wavevector value below beam stop edge.
    ...
    **Correction**
    ...
    In the simplest case (equ. 19) one finds for a sample inside of a container with equal thicknesses (:math:`z`)
    for sample, emty cell and buffer measurement (ommitting the overall q dependence and after resorting).

    .. math:: I_s = \frac{1}{z_S}(\frac{I_S-I_{dark}}{T_S} -\frac{I_{EC}-I_{dark}}{T_{EC}}) - \frac{1}{z_B}(\frac{I_B-I_{dark}}{T_B} -\frac{I_EC-I_{dark}}{T_{EC}})
    ...
    where
     ...
     - :math:`I_dark` is the dark current measurement
     - :math:`I_EC`   is the empty cell measurement
     - :math:`z_x`    corresponding sample thickness
     - :math:`T_x`    corresponding transmission

    This may be completed if we consider that a volume fraction :math:`1-\Phi` of solvent contributes to :math:`I_S`.

    .. math:: I_s=(\frac{I_S-I_{dark}}{T_S}-\frac{I_{EC}-I_{dark}}{T_{EC}})-(1-\Phi)(\frac{I_B-I_{dark}}{T_B} -\frac{I_EC-I_{dark}}{T_{EC}})

    in arbitrary units.
    The ommited sample thickness can be acounted for, if the same procedure is used
    for absolute calibration (e.g. water measurement in the same cell with same thickness).

    The transmission is measured as the ratio :math:`T=\frac{I(q=0)_{sample}}{I(q=0)_{emptybeam}}` with :math:`I(q=0)`
    as the primary beam intensity. Again in the above equation :math:`I(q=0)_{emptybeam}`
    only gives a common scaling factor and can be neglected.

    As an alternative also the emptycell measurement can be used as :math:`T_{sample in cell}=T_{empty cell}T_{sample without cell}`.

    In this case the transmission describes the transmission of the sample without the cell.
    ...
    if emptybeam is None:
        ...
        transmissionCorrection(data=emptybeam,dark=dark,edge=edge,emptybeam=None,exposure=exposure)
        ebtransmission = emptybeam.transmission
        if data is emptybeam:
            # data and emptybeam are the same object and we have to prevent here to do the correction twice
            # this happens if a measurement is used as the emptybeam and is found in data and emptybeam
            return
    ...
    temp=data[:,imin:imax]
    data.centerTransmissionPeak=(temp.X*temp.Y/temp.Y.sum()).mean()
    data.Y=data.Y/data.transmission/data.Exposure[0]
    data.eY = data.eY / data.transmission/data.Exposure[0]
...
def moveSAXSPACE(pattern,root='./',destination='./despiked',skip='BeamProfile',medwindow=5,SGwindow=5,sigma=0.2,
    ...
    for file in files:
        ...
        data=dA(numbers)
        datanew=removespikes(data,xmin=None,medwindow=medwindow,SGwindow=SGwindow,sigma=sigma,order=order)
        f=open(newname,'w')
        f.writelines(header)
