...
class dataListBase(list):
    ...
    def _read_objekt(self,objekt=None,
        ...
        if isinstance(objekt,dataList):
            ...
            try:
                ...
                for ff in filelist:
                    # read file
                    inputz=_read(ff,block=self._block,usecols=usecols,skiplines=skiplines,replace=replace,ignore=ignore,delimiter=delimiter,takeline=takeline,lines2parameter=lines2parameter)
                    ...
                    if isinstance(inputz,str):
                        ...
                        if isinstance(index,int):
                            # convert single integer to slice
                            indexi=slice(index,index+1)
                        elif isinstance(index,slice):
                            # is already slice
                            indexi=index
                        elif any([isinstance(a,int) for a in index]):
                        ...
                        for i in range(len(inputz))[indexi]:
                            datalist.append(dataArray(inputz,index=i, XYeYeX=XYeYeX))
        if len(datalist)==0:
    ...
    @inheritDocstringFrom(list)
    def append(self,objekt=None,index=slice(None),usecols=None,skiplines=None,
               replace=None,ignore='#',delimiter=None,takeline=None,lines2parameter=None):
        """
        ...
        """
        obj=self._read_objekt(objekt,index=index,usecols=usecols,skiplines=skiplines,replace=replace,ignore=ignore,delimiter=delimiter,takeline=takeline,lines2parameter=lines2parameter)
    ...
    @inheritDocstringFrom(list)
    def insert(self,i,objekt=None,index=0,usecols=None,skiplines=None,replace=None,ignore='#',delimiter=None,takeline=None,lines2parameter=None):
        """
        ...
        objekt,index,usecols,skiplines,replace,ignore,delimiter,takeline,lines2parameter : options
         ...
         """
        obj=self._read_objekt(objekt,usecols=usecols,skiplines=skiplines,replace=replace,ignore=ignore,delimiter=delimiter,takeline=takeline,lines2parameter=lines2parameter)
...
class dataArrayBase(np.ndarray):
    ...
    def __new__(subtype, input=None,
        ...
        XYeYeX : list integers, default=[0,1,2,None,None,None]
            Columns for X, Y, eY, eX, Z, eZ.
            Change with eg. setColumnIndex(3,5,32).
    ...
    def prune(self,lower=None,upper=None,number=None,kind='lin',col='X',weight='eY',keep=None,type='mean'):
        ...
        if type[:4]=='mean':
            ...
            if type!='mean':
                ...
                temp.setColumnIndex(iex=(getattr(self,'_ix')+nn if hasattr(self,'X') else ''),
                                    iey=(getattr(self,'_iy')+nn if hasattr(self,'Y') else ''),
                                    iez=(getattr(self,'_iz')+nn if hasattr(self,'Z') else ''))
            return temp
