...
FOV = join(SAVE_DIR, "DEV_out_doascalib_fov.fts")
...
if __name__ == "__main__":
    ...
    aa_list = prepare_aa_image_list()
    cdata = pyplis.DoasCalibData()
    cdata.load_from_fits(FOV)
    
    fov = cdata.fov
    
    img = aa_list.this
    
    fov_x, fov_y = fov.pos_abs
    
    val0 = img.avg_in_roi(pos_x=fov_x, pos_y=fov_y, radius=1)
    
    mask=fov.fov_mask_abs(img.shape)
    val1 = img.avg_in_roi(mask=mask)
    ...
    calib = pyplis.DoasCalibData()
    calib.load_from_fits(join(SAVE_DIR, "ex06_doascalib_aa.fts"))
    
    mask = calib.fov.fov_mask #is at pyrlevel 2
    
    tau_series = stack.get_time_series(mask=mask)[0]
    
    from numpy import min, max, logical_and
    def crop_two_timeseries_index(s1, s2):
        if s1.index[0] > s2.index[-1] or s1.index[-1] < s2.index[0]:
            raise IndexError("Time series do not overlap")
        start = max([s1.index[0], s2.index[0]])
        stop = min([s1.index[-1], s2.index[-1]])
        return (s1[logical_and(s1.index >= start, s1.index <= stop)],
                s2[logical_and(s2.index >= start, s2.index <= stop)])
        
    s1, s2 = crop_two_timeseries_index(doas_time_series, tau_series)
