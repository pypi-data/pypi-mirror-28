...
class WorksheetItemSchema(Schema):
    ...
    # worksheet contains one of the following, detrmined by type
    type = fields.String(validate=validate.OneOf(set(WORKSHEET_ITEM_TYPES)), required=True)
    subworksheet = fields.Relationship(include_data=True, attribute='subworksheet_uuid', type_='worksheets', allow_none=True)
    bundle = fields.Relationship(include_data=True, attribute='bundle_uuid', type_='bundles', allow_none=True)
    value = fields.String()  # For type = directive or markup
    # For ordering in the worksheet
