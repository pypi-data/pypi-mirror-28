...
class MarathonUCRSpawnerException(Exception):
    pass


class MarathonUCRSpawner(Spawner):
    ...
    def _ip_connect_changed(self, change):
        if jupyterhub.version_info >= (0, 8):
            warnings.warn(
                "MarathonUCRSpawner.hub_ip_connect is no longer needed with JupyterHub 0.8."
    ...
    def _port_connect_changed(self, change):
        if jupyterhub.version_info >= (0, 8):
            warnings.warn(
                "MarathonUCRSpawner.hub_port_connect is no longer needed with JupyterHub 0.8."
    ...
    def __init__(self, *args, **kwargs):
        super(MarathonUCRSpawner, self).__init__(*args, **kwargs)
    ...
    def get_state(self):
        state = super(MarathonUCRSpawner, self).get_state()
        state['user_options'] = self.stored_user_options = self.user_options
        return state

    def load_state(self, state):
        super(MarathonUCRSpawner, self).load_state(state)
    ...
    def start(self):
        ...
        while True:
            ...
            if app_info is None:
                raise MarathonUCRSpawnerException("Application %s is lost", self.app_id)
            elif app_info.instances == 0:
                raise MarathonUCRSpawnerException("No instance for application %s", self.app_id)
