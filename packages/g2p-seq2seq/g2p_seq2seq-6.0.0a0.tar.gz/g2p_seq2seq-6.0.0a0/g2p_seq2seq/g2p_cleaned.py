...
class G2PModel(object):
  ...
  def __init__(self, params, file_path="", is_training=False):
    ...
    self.input_ready, self.processing_input = False, {}
  ...
  def __prepare_interactive_model(self,
    ...
    def input_fn():
      ...
      gen_fn = make_input_fn(inp_)#self.get_input_fn())
      example = gen_fn()
    ...
    with estimator_lib.ops.Graph().as_default() as g:
      ...
      estimator_spec = self.estimator._call_model_fn(
  ...
  def transmit_input_fn(self, inp):
    if not self.input_ready:
      self.processing_input = inp
  ...
  def interactive(self):
    ...
    while not self.mon_sess.should_stop():
      ...
      pronunciation = self.decode_word(word).next()
      print(pronunciation)
