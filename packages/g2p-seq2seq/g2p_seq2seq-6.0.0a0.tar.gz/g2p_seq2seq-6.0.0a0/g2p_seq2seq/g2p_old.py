...
class G2PModel(object):
  ...
  def __init__(self, params, file_path="", is_training=False):
    ...
    self.word_processed, self.processing_word = False, {}
  ...
  def decode_word(self, mon_sess, predictions, inp):#input_string):
    """Decode word.
    ...
    #def input_fn():
    #  #gen_fn = decoding.make_input_fn_from_generator(_interactive_input_fn(hparams, word))#decoding._interactive_input_fn(hparams))
    #  gen_fn = make_input_fn(inp)
    #  example = gen_fn()
    #  #example = decoding._interactive_input_tensor_to_features_dict(example,
    #  #    self.estimator.params)#hparams)
    ...
    ##result_iter = self.estimator.predict(input_fn)#{"inputs":word})
    ##result_iter = predict(input_fn, esti=self.estimator,
    ##    model_dir=self.params.model_dir)
    #mon_sess, predictions = self.__prepare_interactive_model(input_fn,
    #     model_dir=self.params.model_dir)

    #for result in result_iter:
    #while not mon_sess.should_stop():
    self.transmit_word_fn(inp)
    preds_evaluated = mon_sess.run(predictions)
    Tracer()()
    result = None
    if not isinstance(predictions, dict):
  ...
  def transmit_word_fn(self, word):
  ...
  def get_word_fn(self):
    if not self.word_processed:
      ...
      return self.processing_word

  def interactive(self):
    """Interactive decoding."""
    #estimator, decode_hp = self.__prepare_decode_model()
    #decode_interactively(self.estimator, self.decode_hp)
    ...
    if True:
      ...
      def input_fn():
        #gen_fn = decoding.make_input_fn_from_generator(_interactive_input_fn(hparams, word))#decoding._interactive_input_fn(hparams))
        gen_fn = make_input_fn(inp)#self.get_word_fn)
        example = gen_fn()
        #example = decoding._interactive_input_tensor_to_features_dict(example,
        #    self.estimator.params)#hparams)
        example = _interactive_input_tensor_to_features_dict(example,
      ...
      with estimator_lib.ops.Graph().as_default() as g:
        features = self.estimator._get_features_from_input_fn(
            input_fn, estimator_lib.model_fn_lib.ModeKeys.PREDICT)
        mon_sess = self.__prepare_interactive_model(g, features,#input_fn,
            model_dir=self.params.model_dir)
    ...
    while not mon_sess.should_stop():
      ...
      #predictions = self.estimator._extract_keys(self.estimator_spec.predictions, None)#self.predict_keys)
      #def input_fn():
      #  gen_fn = make_input_fn(inp)
      #  example = gen_fn()
      #  example = _interactive_input_tensor_to_features_dict(example,
      ...
      #features = self.estimator._get_features_from_input_fn(
      #    input_fn, estimator_lib.model_fn_lib.ModeKeys.PREDICT)
      #mon_sess, predictions = self.__prepare_interactive_model(features,#input_fn,
      #    model_dir=self.params.model_dir)
