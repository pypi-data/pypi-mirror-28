...
def zipdir(path, ziph):
    ...
    files_to_ignore = get_ignored(gitignore)
    for root, dirs, files in os.walk(path):
        files = [f for f in files if not f[0] == '.']
        dirs[:] = [d for d in dirs if not d[0] == '.']
        ignore = list(set(dirs).intersection(files_to_ignore))
        for i in ignore:
            dirs.remove(i)
        for file in files:
...
def upload(token, data, options):
    ...
    if options['zip_option'] == 'z':
        zipf = zipfile.ZipFile('.zipignore', 'w', zipfile.ZIP_DEFLATED)
        zipdir('.', zipf)
        zipf.close()
        files = {'files': open('.zipignore', 'rb')}
