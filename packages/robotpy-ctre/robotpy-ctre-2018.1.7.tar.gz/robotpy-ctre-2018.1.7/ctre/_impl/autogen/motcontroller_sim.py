...
class MotController:
    ...
    def _calculate_1ms(self):
        ...
        if calculating:
            out = min(out, 1023 * self.hal_data['peak_fwd_output'])
            out = max(out, 1023 * self.hal_data['peak_rev_output'])
            if out >= 0:
    ...
    def _calculate_pid(self, pos, revMotDuringCloseLoopEn, oneDirOnly):
        ...
        if CloseLoopRampRate:
            if self._outBeforRmp >= self._out:
                deltaUp = min(self._outBeforRmp - self._out, CloseLoopRampRate)
                out += deltaUp
            else:
                deltaDn = min(self._out - self._outBeforRmp, CloseLoopRampRate)
                out -= deltaDn
        else:
