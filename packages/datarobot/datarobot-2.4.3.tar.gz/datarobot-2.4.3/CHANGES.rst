...
Bugfixes
********
- When calling `get_result` for a Job, ModelJob, or PredictJob that has errored,
  `AsyncProccessUnsuccessfulError` will now be raised instead of `JobNotFinished`, consistently with
  the behaviour of `get_result_when_complete`.
