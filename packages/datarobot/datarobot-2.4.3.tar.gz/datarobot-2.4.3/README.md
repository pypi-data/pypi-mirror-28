# DataRobot Python SDK

DataRobot SDK is a Python library for working with the
[DataRobot](!http://datarobot.com) platform API.

This README is primarily intended for those developing the SDK. There is
also documentation intended for users of the SDK contained in the `docs`
...
## Topics
 * Setup topics:
    * [Common DataRobot SDK setup](#datarobot-sdk-setup)
...
### DataRobot SDK Setup
There are three different ways to set up the SDK to authenticate with the
...
#### Setup with cfg file
The SDK will look for a config file `~/.config/datarobot/drconfig.yaml` by default. You can also
