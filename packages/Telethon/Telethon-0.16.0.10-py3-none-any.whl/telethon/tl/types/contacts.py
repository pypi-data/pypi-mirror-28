...
class Found(TLObject):
    ...
    def __init__(self, my_results, results, chats, users):
        """
        :param list[TLObject] my_results:
    ...
    def to_dict(self, recursive=True):
        return {
            'my_results': ([] if self.my_results is None else [None if x is None else x.to_dict() for x in self.my_results]) if recursive else self.my_results,
