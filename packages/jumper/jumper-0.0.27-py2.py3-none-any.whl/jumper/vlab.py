...
class EmulationError(VlabException):
    def __init__(self, exit_code, message):
        super(EmulationError, self).__init__(message)
...
class TranspilerError(VlabException):
    def __init__(self, message):
        super(TranspilerError, self).__init__(message)
        self.exit_code = 3
        self.message = message
...
class Vlab(object):
    ...
    def __init__(self, working_directory=None, config_file=None, gdb_mode=False, sudo_mode=False, print_trace=False, trace_output_file="", print_uart=False, uart_output_file=""):
        ...
        if not os.path.isfile(config_file):
            print ('Config file not found at: {}'.format(os.path.abspath(config_file)))
    ...
    def _valid_file_existence(self, file_path, err_name):
        if not os.path.isfile(file_path):
            print("Failed to open file \"" + err_name + "\" (at: '" +file_path + "')")
    ...
    def load(self, file_path):
        ...
        #     print("Invalid file extension - please use a binary file with a .bin extension.")
        #     if ()
        #     sys.exit(-1)
        ...
        if self._local_jemu:
            try:
                ...
                print("Transpiler failed with an error: " + e.message)
    ...
    def start(self, ns=None):
        ...
        if self._jemu_debug:
            ...
            cwd: {}\n\
            command: {}\n\
            Press any key to continue...".format(self._working_directory, ' '.join(jemu_cmd))
                  )
        else:
            try:
                ...
                print (e.message)
                raise EmulationError(5, e.message)

        @timeout_decorator.timeout(6)
        def wait_for_uart():
            print (self._uart_device_path)
        ...
        try:
            ...
            self.stop()
            print("Error: Uart does't exist")
            raise EmulationError(5, "Error: Uart does't exist")
        ...
        try:
            ...
            self.stop()
            print("Error: Couldn't connect to Jemu")
            raise EmulationError(5, "Error: Couldn't connect to Jemu")
