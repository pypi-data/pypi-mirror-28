...
def print_lol(the_list, tabs):
    """Print_lol takes a positional argument the_list, checks if each item from that is a
        list itself recursively and prints the item if it is not. It also uses the secondary argument
        Tabs to reference the indent printed for each sub-list (use 0 for no indent)"""
