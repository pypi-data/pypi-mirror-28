...
class KoguTest(unittest.TestCase):
    ...
    def test_register_plot_defaults(self):
        sys.stdout = stdout = StringIO()
    ...
    def test_register_plot_metrics(self):
        sys.stdout = stdout = StringIO()

        # Expected fail cases
        # We allow empty metrics to be passed to the method in this case.
        for name in self._error_names(missing=False):
            Kogu.plot("line", name)
            self._assert_warn(
        ...
        Kogu.plot("line", "x")
        self._assert_line_format("#PLOT", stdout.getvalue())
        # use better regex to validate the metrics part
    ...
    def test_register_plot_settings(self):
        sys.stdout = stdout = StringIO()
    ...
    def test_register_plot_settings_double_quotes(self):
        sys.stdout = stdout = StringIO()
    ...
    def test_send_file(self):
        ...
        Kogu.upload(path)
        # self._assert_warn(stdout.getvalue(), "non-existing-file")
    ...
    def test_send_metric_validation(self):
        sys.stdout = stdout = StringIO()
        ...
        # Strip whitespace in identifiers
        Kogu.metrics({" A ": 3})
        self._assert_metric_line(stdout.getvalue())
        self.assertRegexpMatches(stdout.getvalue(), r".*A=3.*")
        self._clean(stdout)

        # Ignore leading/trailing whitespace in identifiers
    ...
    def test_load_parameters_to_module_scope(self):
    ...
    # parameters json. Parameters available in parameters.json are
    # outputed via param lines
    def test_load_parameters_priority(self):
        with open(self._parameters_file_name(), "w") as json:
            json.writelines(u'{"epochs":50,"spaces":25}')
