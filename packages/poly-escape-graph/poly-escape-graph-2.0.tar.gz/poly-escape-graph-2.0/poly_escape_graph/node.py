...
class Node(Flask):
    ...
    def listen(self):
        ...
        :return: {"status": "OK"}
        """
        self.logger.debug(f'{self.name} listen {request}={request.json}')
    ...
    def subscribe(self):
        ...
        for tag in data[TAGS]:
            ...
            self.logger.debug(f'{plugin.url} subscribe to {self.name} with tag {tag}')
    ...
    def unsubscribe(self):
        ...
        for tag in data[TAGS]:
            ...
            if plugin is not None:
                self.logger.debug(f'{plugin.url} unsubscribe to {self.name} with tag {tag}')
