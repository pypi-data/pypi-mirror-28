...
class Node(Flask):
    ...
    def __init__(self, name, **kwargs):
        ...
        self._regular_name = '-'.join(self.name.lower().split())
        self._db_name = f'{self._regular_name}.db'
        self._config()
        self._create_db()
    ...
    def _config(self):
        self.config.update(
            {
                SQLALCHEMY_DATABASE_URI: f'sqlite:///{self._db_name}',
    ...
    def _create_db(self):
        """
            Remove pre-existing database and create a new one with the model for subscribers plugins
        """
    ...
    def _create_model(self, name, *bases, **attributes):
        """
            Create a dynamic class to simulate a Model
            This class must inherit from sql SQLAlchemy.Model
        """
    ...
    def info(self):
        """
            ...
            You can overwrite this to add info about your plugin
    ...
    def subscribers(self):
        """
            A property for the current subscribers of this plugin formatted as follows
            [
                {"url": "subscriber-1", "tags": ["tag-1", "tag-2"]},
                {"url": "subscriber-2", "tags": ["tag-2", "tag-3"]},
                {"url": "subscriber-3", "tags": ["tag-3", "tag-1"]}
    ...
    def publish(self, tags: List[str], **data):
        ...
        :return: None
        """
        subscribers = set(p.url for tag in tags for p in self.db.session.query(self.plugin).filter_by(tag=tag))
        for subscriber in subscribers:
            try:
                self.request().post(f'{subscriber}/listen', json={"tags": tags, "data": data}, timeout=0.1)
            except RequestException as e:
                self.logger.warning(e)  # Timeout or NoConnection

    def publish_file(self, tags: List[str], files, **data):
        subscribers = set(p.url for tag in tags for p in self.db.session.query(self.plugin).filter_by(tag=tag))
        for subscriber in subscribers:
            try:
                d = {"tags": json.dumps(tags), "data": json.dumps(data)}
                fs = {name: open(path, 'rb') for name, path in files.items()}
                self.request().post(f'{subscriber}/listen', data=d, files=fs, timeout=1)
            except RequestException as e:
                self.logger.warning(e)  # Timeout or NoConnection
            finally:
                for file in fs.values():
                    file.close()
    ...
    def request(self, retries=3, back_off_factor=0.3, status_force_list=(500, 502, 504), session=None):
        """
