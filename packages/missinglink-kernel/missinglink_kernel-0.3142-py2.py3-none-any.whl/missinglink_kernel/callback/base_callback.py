...
class BaseCallback(object):
    ...
    @classmethod
    def _get_rm_env(cls):
        return dict([x for x in os.environ.items() if x[0].startswith('MALI_') or x[0] == 'MALI'])

    __rm_root_mapping = {
        'MALI': 'in_context',
        'MALI_JOB_ID': 'job_id',
        'MALI_INVOCATION_ID': 'invocation_id',
        'MALI_PROJECT_ID': 'project_id',
        'MALI_CLUSTER_ID': 'resource_manager',
        'MALI_CLUSTER_MANAGER': 'socket_server',
        'MALI_OUTPUT_DATA_VOLUME': 'output_volume_id'
    }
    __rm_data_mapping = {
        'MALI_DATA_USE_ITERATOR': 'use_iterator',
        'MALI_DATA_VOLUME': 'volume',
        'MALI_DATA_QUERY': 'query'
    }
    ...
    def __remove_version_part(cls, query):
        ...
        transformer = RemoveSplitTransformer()
    ...
    def bind_data_generator(
            ...
            cache_directory='./ml_cache', batch_size=32, use_threads=False, processes=-1):
        ...
        return MLDataGenerator(volume_id, query, data_callback, cache_directory, batch_size, use_threads, processes)
