...
class DataSync(object):
    ...
    def __append_index_file(self, file_info, combined_index_files):
        ...
        ctime = (file_info.get('ctime', mtime) - epoch).total_seconds()
    ...
    def __download_results(self, process_control, storage, results, callback=None):
        ...
        for item in results:
            ...
            task_async = process_control.execute(
                ...
                callback=wrap_callback_func(item))
    ...
    def download_iter(self, iter_results, cache, multi_process_control):
        ...
        futures = []
        for async_result, metadata in self.__download_results(multi_process_control, cache, results):
            futures.append((async_result, metadata))

        for async_result, metadata in futures:
            async_result.wait()
            yield metadata
