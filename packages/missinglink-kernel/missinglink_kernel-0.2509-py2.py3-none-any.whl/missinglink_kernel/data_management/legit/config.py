...
CONFIG_FILENAME = 'missinglink.cfg'
...
def default_config_file(config_prefix, filename=CONFIG_FILENAME):
...
class Config(object):
    def __init__(self, config_prefix=None, config_file=None):
        ...
        if self.config_file:
            ...
            self.config_file_abs_path = find_first_file(self.config_prefix, CONFIG_FILENAME)
        ...
        if should_check_file_exists and not os.path.exists(self.config_file_abs_path):
            raise MissingLinkException('config file %s not found' % self.config_file_abs_path)
    ...
    def save(self):
        ...
        if self.config_file_abs_path is None:
            self.config_file_abs_path = find_first_file(self.config_prefix, CONFIG_FILENAME) if not self.config_prefix else None
