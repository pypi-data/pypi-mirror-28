...
def generate_find(domain=None, class_name=None, classes=None, service_file=None, api_file=None, filename=None):
	...
	print 'You have {}, \nnow please input one of choice in the filter '.format(
		str(domain['fields']).replace('[', '(').replace(']', ')'))
	print 'Filter by : ',
	filter_by = raw_input()
	filter_by_list = []
	service_name = ''
	if ',' in filter_by:
		...
		for filter_key in split:
			if filter_key.strip() not in domain['fields']:
		...
		if filter_by not in domain['fields']:
			filter_by = ''
		else:
...
def generate_paging(domain=None, class_name=None, classes=None, service_file=None, api_file=None, filename=None):
	...
	print 'Order by : ',
	order_by = str(raw_input()).strip()
	print 'Is this ascending (yes/no) ? ',
	...
	if order_by not in domain['fields']:
...
def generate(filename=None):
	...
	print (classes_names)
	print '***************************************************************************** '
	print 'So please select one that you need ? ',
	class_name = raw_input()
	
	print 'Okay now, what is query you want to add(c, r, u, or d) ? ',
	q_type = str(raw_input()).lower()
	...
	if q_type == 'c':
		...
		print 'Do you want pagination (yes/no) ? ',
		is_paging = str(raw_input()).lower()
		...
		if paging:
			generate_paging(domain=domain, class_name=class_name, classes=classes, service_file=service_file,
			                ...
			                filename=filename)
			return
		print 'is this just one item (yes/no) ?',
		is_one_item = str(raw_input()).lower()
		...
		if one_item:
			generate_find(domain=domain, class_name=class_name, classes=classes, service_file=service_file,
			              ...
			              filename=filename)
			return
		print 'is have all item (yes/no) ?',
		is_all_item = str(raw_input()).lower()
