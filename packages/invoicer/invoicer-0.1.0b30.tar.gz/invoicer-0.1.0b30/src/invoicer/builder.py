...
class DocumentBuilder(object):
    ...
    def build(self, records, date=None):
        ...
        for temp_doc in temp_docs:
            # FIXME : doc number should check also existing docs in sent & paid state !!!
            doc_name = self._format_doc_name(date, len(docs) + 1, self.base.numbering)
    ...
    def _format_doc_name(self, date, num, numbering_format):
        if numbering_format == 'YYYY0MMcc':
            return "{:%Y}0{:%m}{:02d}".format(date, date, num)
        elif numbering_format == 'YYYYMMccc':
            return "{:%Y%m}{:03d}".format(date, num)
        elif numbering_format == 'YYYYccc':
