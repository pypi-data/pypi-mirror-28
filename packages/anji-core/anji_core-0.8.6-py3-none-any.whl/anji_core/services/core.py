...
class ServiceControlTask(RegularTask):
    ...
    def task_execute(self):
        ...
        if not target_service.is_exists():
            ...
            is_target_service_running = target_service.is_running()
            if self.control_signal == 'restart':
                target_service.restart()
                message_body = f'Service {target_service.name} was restarted'
                message_type = MessageType.info
            elif self.control_signal == 'start' and not is_target_service_running:
                target_service.start()
                message_body = f'Service {target_service.name} was started'
                message_type = MessageType.info
            elif self.control_signal == 'stop' and is_target_service_running:
                target_service.stop()
                message_body = f'Service {target_service.name} was stopped'
                message_type = MessageType.info
            else:
