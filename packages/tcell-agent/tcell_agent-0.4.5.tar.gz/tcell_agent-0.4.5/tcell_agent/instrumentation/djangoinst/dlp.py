...
def appsensor_add_db_rows_to_meta(request, total_records, appsensor_policy):
    if appsensor_policy:
...
def dlp_instrumentation():
    ...
    def _tcell_execute_sql(_tcell_original, self, *args, **kwargs):
        ...
        try:
            ...
            if (dataloss_policy and dataloss_policy.enabled) or appsensor_policy.instrument_database_queries:
                try:
                ...
                if col_supported:
                    if self.select:
                        try:
                            ...
                            if appsensor_policy.instrument_database_queries:
