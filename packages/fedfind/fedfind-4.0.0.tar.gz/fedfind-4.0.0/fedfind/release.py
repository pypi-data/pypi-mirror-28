...
def _parse_cid(cid):
    ...
    if ciddic['version_type'] in ('updates', 'updates-testing') and dist != "FedoraRespin":
        raise fedfind.exceptions.UnsupportedComposeError(
            "get_release(): fedfind does not support updates or updates-testing "
...
def _guess_nightly_respin(dist, release, date):
...
def _guess_nightly_branched_atomic(dist, release, date, respin):
    ...
    possible candidates (Production, Docker, Cloud), but we don't
    include those. Split out to make pylint happier.
    """
...
def _get_nightly(dist, release, milestone, date, respin=None):
    ...
    if milestone.lower() == 'branched' or dist.lower() in ('fedora-atomic', 'fedora-docker',
                                                           'fedora-cloud'):
        if respin is None:
            return _guess_nightly_respin(dist, release, date)
        if dist.lower() == 'fedora-atomic':
            klass = AtomicNightly
        elif dist.lower() == 'fedora-docker':
            klass = DockerNightly
...
def get_release(release='', milestone='', compose='', respin=None,
    ...
    if release == 'Rawhide':
        ...
        if respin is not None:
            ...
            return _guess_nightly_respin(dist, release, compose)
...
class CloudNightly(Pungi4Release):
    ...
    def __init__(self, release, compose, respin=0, cid=None):
        ...
        compose = str(compose)
        respin = str(respin)
        path = "Fedora-Cloud-{0}-{1}.{2}/compose".format(release, compose, respin)
        url = '/'.join((fedfind.const.NIGHTLY_BASE, path))
...
class DockerNightly(Pungi4Release):
    ...
    def __init__(self, release, compose, respin=0, cid=None):
        ...
        compose = str(compose)
        respin = str(respin)
        path = "Fedora-Docker-{0}-{1}.{2}/compose".format(release, compose, respin)
        url = '/'.join((fedfind.const.NIGHTLY_BASE, path))
