...
class Headers(dict):
    ...
    def __init__(self, items, request):
        ...
        self.request = request  # type: Union[Part, Request]
        super().__init__()
        self.update(items)
    ...
    def __getitem__(self, key):
        ...
        if key == 'Content-length':
            if self.request.data is None:
                ...
                content_length = len(self.request.data) + 1
            value = str(content_length)
        else:
            value = super().__getitem__(key)
            if (
                key == 'Content-type' and
...
class Request(Part, urllib.request.Request):
    """
...
class MultipartRequest(Request):
    ...
    @data.setter
    def data(self, data):
        return Request.data.__set__(self, data)
