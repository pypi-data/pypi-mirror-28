...
from repository_v3.mlrepositoryartifact.spss_pipeline_model_artifact import SpssPipelineModelArtifact
...
def MLRepositoryArtifact(ml_artifact,
                         training_data=None,
    ...
    if isinstance(ml_artifact, str):
        ...
        if meta_props.get()[MetaNames.FRAMEWORK_NAME] == SPSS_MODELER:
            return _get_spss_model(ml_artifact, name, meta_props)
...
def _get_spss_model(spss_artifact, name, meta_props):
    return SpssPipelineModelArtifact(spss_artifact,
                                     name=name,
                                     meta_props=meta_props)