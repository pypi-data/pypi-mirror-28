...
from repository_v3.swagger_client.models import MlAssetsCreateModelInput, FrameworkOutputRepository, AuthorRepository, EvaluationDefinitionRepository
...
class ModelCollection:
    ...
    def all(self):
        ...
        all_models = self.repository_api.repository_list_models()
        list_model_artifact = []
    ...
    def save(self, artifact):
        ...
        elif artifact.meta.prop(MetaNames.FRAMEWORK_NAME).startswith("spss-modeler"):
            if isinstance(artifact.ml_pipeline_model, str):
                return self._save_spss_pipeline_model(artifact)
            else:
    ...
    def _save_tensorflow_pipeline_model_tar(self, artifact):
        ...
        """
        def _is_valid_tf_archive(gz_file):
        ...
        if not _is_valid_tf_archive(artifact.ml_pipeline_model):
            raise UnsupportedTFSerializationFormat('The specified compressed archive is invalid. Please ensure the Tensorflow model is'
                             ' serialized using tensorflow.saved_model.builder.SavedModelBuilder API')
    ...
    def _save_spss_pipeline_model(self, artifact):
        """
        Saves model in repository service.

        :param SpssPipelineModelTarArtifact artifact: artifact to be saved in the repository service
        :return: saved artifact with changed MetaProps
        :rtype: SpssPipelineModel
        """

        logger.debug('Creating a new Spss model artifact: {}'.format(artifact.name))
        ...
        if latest_version is not None:
            ...
            latest_version_output = ModelVersionOutput(
                ...
                ModelVersionOutputEntity(
                    ...
                    latest_version.content_href, None, None, content_status, content_location
                )
            )
        else:
    ...
    def _prepare_model_input(artifact):
        ...
        if artifact.meta.prop(MetaNames.FRAMEWORK_NAME).startswith("scikit") \
            ...
            return MlAssetsCreateModelInput(
                ...
                or artifact.meta.prop(MetaNames.FRAMEWORK_NAME).startswith("spss-modeler"):
            return MlAssetsCreateModelInput(
                framework=mframework,
                name=artifact.name,
                description=meta.prop(MetaNames.DESCRIPTION),
            ...
            model_input = MlAssetsCreateModelInput(
                ...
                name=artifact.name,
                description=meta.prop(MetaNames.DESCRIPTION),
                experiment_url=pipeline_artifact.meta.prop(MetaNames.TRAINING_DEFINITION_VERSION_URL),
                author=mauthor,
