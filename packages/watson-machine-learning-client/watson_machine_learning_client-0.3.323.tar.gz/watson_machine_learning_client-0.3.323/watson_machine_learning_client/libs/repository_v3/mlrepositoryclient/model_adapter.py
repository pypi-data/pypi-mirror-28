...
from repository_v3.mlrepository.spss_model_artifact import SpssModelArtifact
...
from repository_v3.mlrepositoryartifact.spss_pipeline_model_loader import SpssPipelineModelLoader
...
class ModelAdapter(object):
    ...
    def artifact(self):
        ...
        elif re.match('spss-modeler', self.model_type['name']) is not None:
            model_artifact_builder = type(
                "SpssModelArtifact",
                (SpssPipelineModelLoader, SpssModelArtifact, object),
                {}
            )
        ...
        if self.model_entity.get('framework').get('name').startswith('mllib'):
            if self.model_entity.get('experiment_url') is not None:
                training_def_ver_url = self.model_entity.get('experiment_url')
                experiment_version_url = training_def_ver_url.replace('training_definitions', 'experiments')
                prop_map[MetaNames.EXPERIMENT_VERSION_URL] = experiment_version_url
...
# TODO Fix Evaluation
        ...
        if model_artifact.meta.prop(MetaNames.FRAMEWORK_NAME).startswith('mllib'):
            model_artifact._experiment_version_url = model_artifact.meta.prop(MetaNames.EXPERIMENT_VERSION_URL)
