...
#       if re.match('sparkml-pipeline-\d+\.\d+', self.pipeline_type['name']) is not None:
      ...
      #  name = ''.join(map(lambda x: chr(ord(x)),self.pipeline_output.entity['name']))
        ...
        if self.version_output is not None:
            ...
            experiment_version_url = version_url.replace('training_definitions', 'experiments')
            training_definition_url = version_url.split("/versions")[0]
            experiment_url= experiment_version_url.replace('training_definitions', 'experiments')



            version_prop_map = {
                MetaNames.VERSION: self.version_output['guid'],
                MetaNames.EXPERIMENT_VERSION_URL: experiment_version_url,
                MetaNames.EXPERIMENT_URL:experiment_url,
            ...
            version_prop_map = {
                MetaNames.EXPERIMENT_VERSION_URL: self.pipeline_output.entity['training_definition_version']['url'],
