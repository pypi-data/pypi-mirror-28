    ...
    def populate_field(self, field):
        ...
        if type(val) == LAMBDA_TYPE:
            try:
                ...
                debug("ERROR POPULATING FIELD", field, "TEMPLATE IS", self.__template.name, "RECORD IS", self)
                exit_error()
...
class Template(object):
    ...
    def load_template(self, template):
        ...
        if "mixin" in doc:
            self.setup_mixins(template, doc["mixin"])
    ...
    def load_csv_file(self, filename):
        ...
        with readfile(filename, "rb") as csvfile:
    ...
    def setup_random_field(self, field, **kwargs):
        ...
        def run_random():
            return eval(randfunc, GLOBALS, ll)
    ...
    def setup_case_field(self, field, **field_data):
        cases = []
        default_func = None
        case_id = 0
        for obj in field_data["case"]:
    ...
    def setup_fields(self, template, fields):
        ...
        for field in fields:
            ...
            try:
                ...
                self.debug("ERROR SETTING UP FIELD %s IN %s" % (field, template))
                exit_error()
    ...
    def setup_field(self, field, field_data):
        ...
        if type(field_data) == dict:
            ...
            elif "case" in field_data:
                val_func = self.setup_case_field(field, **field_data)
            ...
            self.debug("UNKNOWN FIELD DATA", field_data, type(field_data))
            exit_error()
    ...
    def gen_record(self, args={}, scrub_record=True):
        ...
        for k in args:
            if not k in self.args:
                self.debug("Error: %s is not an args: field in %s" % (k, self.name))
                exit_error()
    ...
    def print_records(self, num_records):
        ...
        for _ in xrange(num_records / chunk_size):
    ...
    def gen_records(self, num_records):
        self.debug("*** GENERATING %s RECORDS" % num_records)
