...
DataConstructor = Union[Dict[str, Union[ndarray, List]], ndarray]
ColConstructor = Dict[str, ndarray]
DtypeCol = Dict[str, List[str]]

# can change to array of strings?
ColumnType = Optional[Union[List[str], ndarray]]
...
class DataFrame(object):
    ...
    def _initialize_data_from_dict(self, data: DataConstructor) -> None:
        ...
        for i, (col, values) in enumerate(data.items()):
            ...
            self._data[col] = arr
            self._column_dtype[col] = arr.dtype.kind
    ...
    def _initialize_data_from_array(self, data: ndarray) -> None:
        ...
        for i, col in enumerate(self._columns):
            self._data[col] = np.ascontiguousarray(data[:, i])
            self._column_dtype[col] = data.dtype.kind
    ...
    def _initialize_from_object_array(self, data: ndarray) -> None:
        ...
        for i, col in enumerate(self._columns):
            ...
            self._data[col] = np.ascontiguousarray(arr)
            self._column_dtype[col] = 'O'

    def _map_types_to_columns(self) -> None:
        for col, kind in self._column_dtype.items():
            self._dtype_column[kind].append(col)

    def _add_accessors(self):
        if self.shape[1] != 1:
            return None
        only_dtype: str
        for dtype, arr in self._dtype_column.items():
            if arr is not None:
                only_dtype = dtype
    ...
    def _build_repr(self) -> Tuple:
        ...
        for column in columns:
            if column != '...':
                data = [column] + self._data[column][idx].tolist()
            else:
                data = ['...'] * (len(idx) + 1)
                long_len.append(3)
            ...
            elif self._column_dtype[column] == 'i':
    ...
    def _construct_df_from_selection(self, rs, cs) -> 'DataFrame':
        ...
        for col in cs:
            new_data[col] = self._data[col][rs].copy()

        return self._construct_from_new(new_data, cs)
    ...
    def to_dict(self, orient: str='array') -> Dict[str,
                                                   Union[ndarray, List]]:
        '''
        ...
        for col in self._data:
            if orient == 'array':
                ...
                data[col] = self._data[col].tolist()
        return data

    def _is_numeric_or_bool(self):
        return set(self._column_dtype.values()) <= _NUMERIC_KINDS

    def _is_numeric_strict(self):
        return set(self._column_dtype.values()) <= {'i', 'f'}

    def _is_string(self):
        return set(self._column_dtype.values()) == {'O'}

    def _is_only_numeric_or_string(self):
        dtypes = set(self._column_dtype.values())
    ...
    def select_dtypes(self, include=None, exclude=None):
        ...
        current_dtypes = set(self._column_dtype.values())
        if arg_name == 'include':
        ...
        new_data = {col: arr
                    ...
                    if self._column_dtype[col] in include_final}
        new_columns = [col for col in self._columns if col in new_data]

        return self._construct_from_new(new_data, new_columns)

    @classmethod
    def _construct_from_new(cls, data, columns):
        df_new = cls.__new__(cls)
        df_new._column_dtype = {col: data[col].dtype.kind for col in columns}
        df_new._data = data
        df_new._columns = np.asarray(columns)
        df_new._dtype_column: List[str] = {'b': [], 'i': [],
                                           'f': [], 'O': []}
        df_new._map_types_to_columns()
        df_new._hasnans = {}
        return df_new

    def _op(self, other, op_string):
        def do_eval(d, op_string, o):
            eval_string = f"{'d'} .{op_string}({'o'})"
        ...
        if isinstance(other, (int, float, bool)):
            if self._is_numeric_or_bool():
                new_data: Dict[str, ndarray] = {}
                for col, arr in self._data.items():
                    new_data[col] = do_eval(arr, op_string, other)
            ...
            if self._is_string():
                if op_string in ['__add__', '__radd__', '__gt__', '__ge__',
                    ...
                    new_data: Dict[str, ndarray] = {}
                    for col, arr in self._data.items():
                        new_data[col] = do_eval(col, op_string, other)
            else:
        ...
        return self._construct_from_new(new_data, self._columns.copy())
    ...
    @property
    def dtypes(self):
        arr = [utils.convert_kind_to_dtype(self._column_dtype[col])
               for col in self._columns]
        arr = np.array(arr, dtype='O')
        columns = ['Column Name', 'Data Type']
        cn = self._columns.astype('O')
        new_data: Dict[str, ndarray] = {'Column Name': cn, 'Data Type': arr}
        return self._construct_from_new(new_data, columns)
    ...
    def _op_logical(self, other, op_logical):
        ...
        for i in range(self.shape[1]):
            ...
            arr1 = self._data[col]
            if is_other_df:
                arr2 = other._data[other._columns[i]]
            else:
                arr2 = other[:, i]
            new_data[col] = getattr(arr1, op_logical)(arr2)

        return self._construct_from_new(new_data, self._columns.copy())

    def __invert__(self):
        if set(self._column_dtype.values()) == {'b'}:
            new_data = {col: ~arr for col, arr in self._data.items()}
            return self._construct_from_new(new_data, self._columns.copy())
        else:
            raise TypeError('Invert operator only works on DataFrames '
                            'with all boolean columns')

    def _astype_internal(self, column, new_dtype):
        """
        Changes one column dtype in-place
        """
        col_data = self._data[column].astype(new_dtype)
        kind = col_data.dtype.kind
        self._remove_column(column)
        self._write_new_column_data(column, kind, col_data)

    def _delete_dtype_column(self):
        self._dtype_column: DtypeCol = {'O': [], 'b': [], 'f': [], 'i': []}

    def _remove_column(self, column):
        """
        Removes column from _colum_dtype, _dtype_column, and _data
        Keeps column name in _columns
        """
        del self._data[column]
        dtype = self._column_dtype.pop(column)
        self._dtype_column[dtype].remove(column)

    def _write_new_column_data(self, column, kind, data):
        """
        Adds data to _data, the data type info but does not
        append name to columns
        """
        self._column_dtype[column] = kind
        self._dtype_column[kind].append(column)
        self._data[column] = data

    def _add_new_column(self, column, kind, data):
        self._write_new_column_data(column, kind, data)
        self._columns = np.concatenate((self._columns,
                                        [column]))
    ...
    def _full_columm_add(self, column, kind, data):
        ...
        # column is in df
            ...
            if self._column_dtype[column] != kind:
                self._remove_column(column)
                self._write_new_column_data(column,
                                            kind, data)
    ...
    def _setitem_all_other(self, rs, cs, value):
        ...
        if value_kind:
            ...
            for col in cs:
                if self._column_dtype[col] == 'i' and value_kind == 'f':
                    self._astype_internal(col, np.float64)
                self._data[col][rs] = value
        # not scalar
        else:
            if isinstance(value, list):
                ...
                arrs = [value._data[col] for col in value._columns]
            else:
                raise TypeError('Must use a scalar, a list, an array, or a '
                                'DataFrame when setting new values')
    ...
    def _validate_setitem_col_types(self, columns, kind):
        ...
        """
        for col in columns:
            cur_kind = self._column_dtype[col]
    ...
    def _get_kinds(self, cols, other):
        kinds = [self._column_dtype[col] for col in cols]
        if isinstance(other, list):
            other_kinds = [arr.dtype.kind for arr in other]
        else:
            other_kinds = [other._column_dtype[col] for col in other._columns]
    ...
    def astype(self, dtype):
        if isinstance(dtype, str):
            dtype = utils.check_valid_dtype_convet(dtype)
            ...
            for column, new_dtype in dtype.items():
                ...
                new_kind = utils.convert_dtype_to_kind(new_dtype)

                old_kind = df_new._column_dtype[column]

                if old_kind == new_kind:
                    continue
                col_data = df_new._data[column].astype(new_dtype_numpy)
                df_new._remove_column(column)
                df_new._write_new_column_data(column, new_kind, col_data)
            return df_new
        else:
    ...
    def hasnans(self):
        if self._hasnans == {}:
            for col, arr in self._data.items():
                if arr.dtype.kind == 'O':
                    self._hasnans[col] = va.isnan_object(arr)
                elif arr.dtype.kind == 'f':
                    self._hasnans[col] = np.isnan(arr).any()
                else:
                    self._hasnans[col] = False

        arr_hn = np.array([self._hasnans[col] for col in self._columns])
        new_data = {'Column Name': self._columns.astype('O'),
                    'Has NaN': arr_hn}
    ...
    def _get_values(self, name):
        if name in ['count']:
            return self.values
        if name in ['argmax', 'argmin', 'cummax', 'cummin', 'cumsum']:
            if not self._is_only_numeric_or_string():
                raise TypeError(f'Method {name} only works with '
                                'axis="columns" when all columns are '
                                'numeric or string')
            return self.values

    def _stat_funcs(self, name, axis, skipna,
                    use_numpy, **kwargs):
        ...
        if axis == 0:
            for col, arr in self._data.items():
                kind = arr.dtype.kind
                func = stat.funcs[kind][skipna][name]
                dtype = utils.convert_kind_to_numpy(kind)
                hasnans = self._hasnans.get(col)
                if hasnans is not None and kind in 'fO':
            ...
            if use_numpy:
                if not self._is_string():
                    values = self.select_dtypes(['number', 'bool']).values
                else:
                    values = self.values
                ...
                result_arr = np.empty(n, dtype=np.int64)
                result_list = []
                for i in range(n):
                    res = func(values[i])
                    if isinstance(res, ndarray):
                        ...
                        result_arr[i] = res
                    if result_list:
                        result_arr = np.row_stack(result_list)
                        for i, col in enumerate(self._columns):
                            new_data[col] = result_arr[:, i]
                        new_columns = self._columns.copy()
                    else:
                        new_data = {name: result_arr}
                        new_columns = np.array([name], dtype='O')
                return self._construct_from_new(new_data, new_columns)

    def sum(self, axis='rows', skipna=True):
        return self._stat_funcs('sum', axis, skipna, use_numpy=True)

    def max(self, axis='rows', skipna=True):
        return self._stat_funcs('max', axis, skipna, use_numpy=True)

    def min(self, axis='rows', skipna=True):
        return self._stat_funcs('min', axis, skipna, use_numpy=True)

    def mean(self, axis='rows', skipna=True):
        df = self._get_numeric()
        return df._stat_funcs('mean', axis, skipna, use_numpy=True)

    def median(self, axis='rows', skipna=True):
        df = self._get_numeric()
        return df._stat_funcs('median', axis, skipna, use_numpy=True)

    def std(self, axis='rows', skipna=True, ddof=1):
        df = self._get_numeric()
        return df._stat_funcs('std', axis, skipna, use_numpy=True, ddof=ddof)

    def var(self, axis='rows', skipna=True, ddof=1):
        df = self._get_numeric()
        return df._stat_funcs('var', axis, skipna, use_numpy=True, ddof=ddof)

    def abs(self):
        df = self._get_numeric()
        new_data = {col: np.abs(arr) for col, arr in df._data.items()}
        return self._construct_from_new(new_data, df._columns.copy())

    __abs__ = abs

    def _get_numeric(self):
        if not self._has_numeric_or_bool():
            raise('All columns must be either integer, float, or boolean')
    ...
    def any(self, axis='rows', bool_only=False):
        if bool_only:
            df = self.select_dtypes('bool')
        else:
            df = self
        return df._stat_funcs('any', axis, skipna=False, use_numpy=True)

    def all(self, axis='rows', bool_only=False):
        if bool_only:
            df = self.select_dtypes('bool')
        else:
            df = self
        return df._stat_funcs('all', axis, skipna=False, use_numpy=True)

    def argmax(self, axis='rows', skipna=True):
        return self._stat_funcs('argmax', axis, skipna=skipna, use_numpy=False)

    def argmin(self, axis='rows', skipna=True):
        return self._stat_funcs('argmin', axis, skipna=skipna, use_numpy=False)

    def count(self, axis='rows', numeric_only=False):
        if numeric_only:
            df = self.select_dtypes(['number', 'bool'])
        else:
            df = self
        return df._stat_funcs('count', axis, skipna=False, use_numpy=False)
    ...
    def clip(self, lower=None, upper=None):
        ...
        if self._has_numeric_or_bool():
            ...
            raise NotImplementedError('dtypes not compatible')
        new_data = {}
        for col, arr in df._data.items():
            if arr.dtype.kind == 'O':
                hasnans = self._hasnans.get(col)
                if lower is None:
                    lower = chr(0)
                if upper is None:
                    upper = chr(1000000)
                new_data[col] = _math.clip_str(arr, lower, upper, hasnans)
            else:
                new_data[col] = arr.clip(lower, upper)
        return self._construct_from_new(new_data, df._columns.copy())

    def cummax(self, axis='rows', skipna=True):
        return self._stat_funcs('cummax', axis, skipna, use_numpy=False)

    def cummin(self, axis='rows', skipna=True):
        return self._stat_funcs('cummin', axis, skipna, use_numpy=False)

    def cumsum(self, axis='rows', skipna=True):
        return self._stat_funcs('cumsum', axis, skipna, use_numpy=False)
    ...
    def value_counts(self, col):
        ...
        new_dtype_column: DtypeCol = {'O': ['Column Values'],
                                      'b': [], 'f': [], 'i': ['Counts']}
        new_columns = ['Column Values', 'Counts']
        return self._construct_from_new(new_dtype_column,
...
class StringClass(object):
    ...
    def _create_df(self, arr, dtype):
        ...
        new_dtype_column: DtypeCol = {'O': [], 'b': [], 'f': [], 'i': []}
        new_data: Block = {'O': None, 'b': None, 'f': None, 'i': None}

        new_dtype_column[dtype] = new_columns
        new_data[dtype] = arr
        return self.df._construct_from_new(new_dtype_column, new_data,
                                           new_columns)
