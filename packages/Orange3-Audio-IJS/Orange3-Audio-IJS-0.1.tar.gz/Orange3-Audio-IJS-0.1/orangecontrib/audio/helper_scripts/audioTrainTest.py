...
def featureAndTrainRegression(
    ...
    for c in CSVs:                                                  # for each CSV
        ...
        regressionNames.append(
            ntpath.basename(c).replace(
        ...
        for c in range(len(PrecisionClassesAll[i])):
            print(
                "\t{0:.1f}\t{1:.1f}\t{2:.1f}".format(
                    100.0 *
                    PrecisionClassesAll[i][c],
                    100.0 *
                    RecallClassesAll[i][c],
                    100.0 *
                    F1ClassesAll[i][c]),
            )
...
def fileRegression(inputFile, modelName, modelType):
    ...
    if modelType == 'svm':
        ...
        [_, _, _, mtWin, mtStep, stWin, stStep, computeBEAT] = loadKNNModel(
            regressionModels[0], True)
    ...
    for ir, r in enumerate(regressionModels):
        ...
        if modelType == 'svm':
            ...
            [Model, MEAN, STD, mtWin, mtStep, stWin,
                stStep, computeBEAT] = loadKNNModel(r, True)
        curFV = (MidTermFeatures - MEAN) / STD                  # normalization
        R.append(
            regressionWrapper(
                Model,
