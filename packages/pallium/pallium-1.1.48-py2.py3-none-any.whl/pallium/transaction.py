...
from pallium.main import *
from pallium.script import *
...
def deserialize(tx):
    ...
    if obj["flag"]:
        for i in range(ins):
            witnessCount = read_var_int()
            for j in range(witnessCount):
                obj["ins"][i]["witness"].append({"script": read_var_string()})
...
def serialize(txobj):
    ...
    if not txobj["flag"] == 0:
        ...
        o.append(num_to_var_int(txobj["flag"]))
    ...
    if not txobj["flag"] == 0:
        for inp in txobj["ins"]:
            o.append(num_to_var_int(len(inp["witness"])))
            for witness in inp["witness"]:
                o.append(num_to_var_int(len(witness["script"])) + witness["script"])
...
def mk_multisig_script(pubs, k, message=None):  # [pubs], k, message
    redeem_script = [k] + pubs + [len(pubs)] + [0xae]
    if message:
        # append message to redeem script
        redeem_script += [string_to_hex(message)] + [0x75]
    return serialize_script(redeem_script)
...
def verify_tx_input(tx, i, script, sig, pub):
    ...
    tx = serialize(txobj)
    modtx = signature_form(tx, int(i), hashcode)
...
def mktx(*args):
    ...
    for arg in args:
        if isinstance(arg, list):
            for a in arg:
                if is_inp(a):
                    if isinstance(a, dict) and "outpoint" in a:
                        ins.append({"tx_id": a["outpoint"]["hash"], "index": int(a["outpoint"]["index"]), "script": ""})
                    else:
                        if isinstance(a, dict) and "output" in a:
                            a = a["output"]
                        ins.append({"tx_id": a[:64], "index": int(a[65:]), "script": ""})
                else:
                    if isinstance(a, string_or_bytes_types):
                        addr = a[:a.find(':')]
                        val = int(a[a.find(':')+1:])
                        a = {}
            ...
            if is_inp(arg):
                if isinstance(arg, dict) and "outpoint" in arg:
                    ins.append({"tx_id": arg["outpoint"]["hash"], "index": int(arg["outpoint"]["index"]), "script": ""})
                elif isinstance(arg, dict) and "output" in arg:
                    ins.append({"tx_id": arg["output"][:64], "index": int(arg["output"][65:]), "script": ""})
                else:
                    ins.append({"tx_id": arg[:64], "index": int(arg[65:]), "script": ""})
            else:
                if isinstance(arg, string_or_bytes_types):
                    addr = arg[:arg.find(':')]
                    val = int(arg[arg.find(':')+1:])
                    arg = {}
                    if re.match('^[0-9a-fA-F]*$', addr):
                        arg['script'] = addr
                    else:
...
def make_raw_tx(ins, outs):
    ...
    """
    tx_obj = {'locktime': 0, 'version': 1, 'ins': [], 'outs': []}
    for i in ins:
        tx_obj['ins'].append({
            'outpoint': {
                'hash': i['tx_id'],
