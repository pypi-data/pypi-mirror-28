...
class GopherClient(cmd.Cmd):
    ...
    def _guess_text_itemtype(self, selector):
    ...
    def _guess_binary_itemtype(self, selector):
        mimetype, encoding = mimetypes.guess_type(selector)
    ...
    def _autodetect_itemtype(self, gi, f):
        ...
        try:
            ...
            raw_bytes.seek(0)
            itemtype = self._guess_binary_itemtype(gi.path)
            new_gi = GopherItem(gi.host, gi.port, gi.path, itemtype, gi.name)
        ...
        if hits:
            ...
            itemtype = self._guess_text_itemtype(gi.path)
            new_gi = GopherItem(gi.host, gi.port, gi.path, itemtype, gi.name)
        text.seek(0)
        return new_gi, text
