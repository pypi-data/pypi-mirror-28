...
class TkPane(tk.Frame):
    ...
    * set
    ...
    def can_use(self, other_pane):
        """Set handler functions for the other pane to enable this pane or clear its data.
        
        :param other_pane: The pane which must have valid data for this pane to be enabled.
       
        The other pane's lists of Handler callbacks are modified to enable or
        clear the other pane's data keys from this pane's data dictionary when the
        other pane's data are valid or invalid, respectively.  This pane's 'clear_data()'
        method is also added to the other pane's 'on_clear' callback list.
        """
        if not has_handler_function(other_pane.on_change_data_valid, self.enable):
            other_pane.on_change_data_valid.append(PaneDataHandler(self.enable))
        if not has_handler_function(other_pane.on_exit_data_valid, self.enable):
    ...
    def clear_data(self, keylist=None):
        ...
        :param data_keys: A list of keys of the dictionary entry to remove.
        
        If no data keys are specified, all data values managed by this pane will
        be removed.  If any of the data keys removed are for the pane's own
        data, the 'on_clear' callbacks will be called.
        """
        cascade_clear = False
        if keylist is None:
    ...
    def clear(self, keylist=[]):
        ...
        dictionary, *and also* remove the pane's own data.  If any of the pane's
        own data are removed (i.e., valid values exist), the 'on_clear' callbacks
        will be called.
        """
        self.clear_pane()
        self.clear_data(list(set(keylist) | set(self.datakeylist)))
