...
def _jupyter_kube_start(cluster_namespace=None,
                        image_registry_namespace=None):
    pass


def _dashboard_kube_start(cluster_namespace=None,
                         image_registry_namespace=None):
    pass
...
def _service_connect(service_name,
    ...
    if not local_port:
        ...
        print("")
        cmd = 'kubectl port-forward %s :%s --namespace=%s' % (pod.metadata.name, service_port, cluster_namespace)
        print(cmd)
        print("")
    else:
        print("")
        print("Proxying local port '%s' to app '%s' port '%s' using pod '%s' in namespace '%s'." % (local_port, service_port, service_name, pod.metadata.name, cluster_namespace))
        print("")
        print("If you break out of this terminal, your proxy session will end.")
        print("")
        print("Use 'http://127.0.0.1:%s' to access app '%s' on port '%s' in namespace '%s'." % (local_port, service_name, service_port, cluster_namespace))
        print("")
        cmd = 'kubectl port-forward %s %s:%s --namespace=%s' % (pod.metadata.name, local_port, service_port, cluster_namespace)
        print(cmd)
        print("")
    ...
    with _warnings.catch_warnings():
        ...
        for pod in pods: 
            if (service_name in pod.metadata.name):
                _subprocess.call('kubectl top pod %s --namespace=%s' % (pod.metadata.name, cluster_namespace), shell=True)
    print("")
...
@app.route("/predict-server-build/<string:model_name>/<string:model_tag>/<string:model_type>/<string:model_path>", methods=['GET'])
...
def _create_predict_kube_Kubernetes_yaml(model_name,
    ...
    if image_registry_base_chip == 'gpu':
...
def predict_kube_test(model_name,
    ...
    model_endpoint_url = _get_model_kube_endpoint(model_name,
                                                  cluster_namespace=cluster_namespace,
                                                  image_registry_namespace=image_registry_namespace)
...
def predict_sage_describe(model_name,
    ...
    if response and response['ResponseMetadata']['HTTPStatusCode'] == 200:
        ...
        print("")
        model_endpoint_url = _get_sage_endpoint_url(model_name=model_name,
                                                    model_region=model_region,
                                                    image_registry_namespace=image_registry_namespace)
        print("")
        print("Endpoint url: '%s'" % model_endpoint_url)
        print("")
        print("Request time: %s milliseconds" % (total_time.microseconds / 1000))
        print("")
...
def _service_logs(service_name,
    ...
    with _warnings.catch_warnings():
        ...
        if found:
            ...
            print("")
            if container_name:
                cmd = "kubectl logs -f %s -c %s --namespace=%s" % (pod.metadata.name, container_name, cluster_namespace)
            else:
                cmd = "kubectl logs -f %s --namespace=%s" % (pod.metadata.name, cluster_namespace)
            print(cmd)
            print("")      
            _subprocess.call(cmd, shell=True)
            print("")
...
def _service_describe(service_name,
    ...
    with _warnings.catch_warnings():
        ...
        print("Connecting to '%s'" % pod.metadata.name)
        print("")
        _subprocess.call("kubectl describe pod %s --namespace=%s" % (pod.metadata.name, cluster_namespace), shell=True)
    print("")
...
def predict_kube_autoscale(model_name,
    ...
    cmd = "kubectl autoscale deployment %s-%s-%s --cpu-percent=%s --min=%s --max=%s --namespace=%s" % (image_registry_namespace, model_name, model_tag, cpu_percent, min_replicas, max_replicas, cluster_namespace)
    print("")
    print("Running '%s'." % cmd)
    print("")
    _subprocess.call(cmd, shell=True)
...
def _get_model_kube_endpoint(model_name,
    ...
    with _warnings.catch_warnings():
        ...
        if not endpoint:
            endpoint = 'localhost'
            print("Endpoint for model_name '%s' cannot be found.  Using '%s' instead." % (model_name, endpoint))
