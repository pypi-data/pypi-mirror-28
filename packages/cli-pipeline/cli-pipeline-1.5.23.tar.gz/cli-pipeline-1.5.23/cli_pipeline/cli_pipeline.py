...
_kube_faas_deploy_template_registry = {'predict-faas-deploy': (['predict-faas-deploy.yaml.template'], []),
                                       'predict-faas-gpu-deploy': (['predict-faas-gpu-deploy.yaml.template'], [])}
...
def _create_predict_kube_Kubernetes_yaml(model_name,
    ...
    if image_registry_base_chip == 'gpu':
        ...
        rendered = _jinja2.Environment(loader=_jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = _os.path.normpath('.pipeline-generated-%s-%s-%s-router-gpu-split-deploy.yaml' % (image_registry_namespace, model_name, model_tag))
    ...
    if image_registry_base_chip == 'gpu':
        model_faas_deploy_yaml_templates_path = _os.path.normpath(_os.path.join(pipeline_templates_path, _kube_faas_deploy_template_registry['predict-faas-gpu-deploy'][0][0]))
        path, filename = _os.path.split(model_faas_deploy_yaml_templates_path)
        rendered = _jinja2.Environment(loader=_jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = _os.path.normpath('.pipeline-generated-%s-%s-%s-faas-gpu-deploy.yaml' % (image_registry_namespace, model_name, model_tag))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
    else:
        model_faas_deploy_yaml_templates_path = _os.path.normpath(_os.path.join(pipeline_templates_path, _kube_faas_deploy_template_registry['predict-faas-deploy'][0][0]))
        path, filename = _os.path.split(model_faas_deploy_yaml_templates_path)
        rendered = _jinja2.Environment(loader=_jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = _os.path.normpath('.pipeline-generated-%s-%s-%s-faas-deploy.yaml' % (image_registry_namespace, model_name, model_tag))
        with open(rendered_filename, 'wt') as fh:
            fh.write(rendered)
            print("'%s' => '%s'" % (filename, rendered_filename))
            rendered_filenames += [rendered_filename]
...
@app.route("/predict-kube-start/<string:model_name>/<string:model_tag>/", methods=['GET'])
def predict_faas_start(   model_name,
                          model_tag,
                          namespace=None,
                          log_stream_url=None,
                          log_stream_topic=None,
    ...
    for rendered_yaml in rendered_yamls:
        # For now, only handle '-faas-deploy'
        if '-faas-deploy' in rendered_yaml:
...
def _get_all_model_endpoints(namespace=None,
                             image_registry_namespace=None):
