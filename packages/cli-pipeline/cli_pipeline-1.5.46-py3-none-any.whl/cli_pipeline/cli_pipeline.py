...
def _create_predict_kube_Kubernetes_yaml(model_name,
    ...
    if model_chip == 'gpu':
        ...
        rendered = _jinja2.Environment(loader=_jinja2.FileSystemLoader(path)).get_template(filename).render(context)
        rendered_filename = _os.path.normpath('.pipeline-generated-%s-%s-%s-router-%s-%s-split-deploy.yaml' % (image_registry_namespace, model_name, model_tag, model_runtime, model_chip))
