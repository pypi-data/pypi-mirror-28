...
def score_and_write_batch(args, model, file_out, fifo, batch_size, python_batch_size):
	...
	for i in range(batch_size):
		...
		fidx = 7
		if args.tensor_map in tensor_maps.TENSOR_MAPS_2D and len(fifo_data) > fidx:
			read_tuples = []
...
def predictions_to_snp_scores(predictions):
	...
	return np.log(defines.EPS + snp / (not_snp + defines.EPS))
...
def predictions_to_indel_scores(predictions):
	...
	return np.log(defines.EPS + indel / (not_indel + defines.EPS))
