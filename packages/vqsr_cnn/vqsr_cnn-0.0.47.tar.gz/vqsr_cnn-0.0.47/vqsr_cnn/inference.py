...
def score_and_write_batch(args, model, file_out, fifo, batch_size, python_batch_size):
	...
	for i in range(batch_size):
		...
		if args.tensor_map in tensor_maps_2d and len(fifo_data) > fidx:
			read_tuples = []
			var = Variant(fifo_data[0], int(fifo_data[1]), fifo_data[2], fifo_data[3])
			while fidx+7 < len(fifo_data):
				read_tuples.append( Read(fifo_data[fidx],
										 [int(q) for q in fifo_data[fidx+1]],
										 fifo_data[fidx+2],
										 fifo_data[fidx+3],
										 fifo_data[fidx+4],
										 fifo_data[fidx+5],
			...
			for r in read_tuples:
				read_batch.append(read_tuple_to_read_tensor(args, r, ref_start, insert_dict))
				print('read_batch len:', len(read_batch), 'read batch -1 shape', read_batch[-1].shape)

	if args.tensor_map in tensor_maps_2d and len(read_batch) > 0:
...
def read_tuple_to_read_tensor(args, read_tuples, ref_start, insert_dict):
	...
	print('Try to make a read tensor with shape:', tensor.shape)
	...
	print('Made a read tensor with shape:', tensor.shape)
	return tensor
