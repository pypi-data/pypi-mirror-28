...
from coralogix import __version__
from coralogix.debug_logger import DebugLogger
from threading import Thread, Lock, current_thread
from coralogix.httpsender import CoralogixHTTPSender

try:
    import ujson as json
    print("manager.py: Using ujson library for JSON parsing (compiled in C)")
except Exception:
    import json
    print("manager.py: Using standard library for JSON parsing (interpreted in Python)")
...
class LoggerManager(object):
    ...
    def send_init_message(cls):
        cls.add_logline("The Application Name {} and Subsystem Name {} from the Python SDK, version {} has started to send data".format(
            cls._bulk_template['applicationName'], cls._bulk_template['subsystemName'], __version__), Coralogix.Severity.INFO, Coralogix.CORALOGIX_CATEGORY, threadId=current_thread().ident)
    ...
    def add_logline(cls, message, severity, category, **kwargs):
        ...
        try:
            ...
            if cls._buffer_size < Coralogix.MAX_LOG_BUFFER_SIZE:
                # Validate message
                message = cls._msg2str(message) if message and (not str(message).isspace()) else "EMPTY_STRING"

                # Validate severity
                severity = int(severity) if (cls._is_number(severity) and str(severity)) else int(Coralogix.Severity.DEBUG)
                severity = severity if (int(Coralogix.Severity.DEBUG) <= severity <= int(Coralogix.Severity.CRITICAL)) else int(Coralogix.Severity.DEBUG)

                # Validate category
                category = str(category) if category and (not category.isspace()) else Coralogix.CORALOGIX_CATEGORY

                # Combine a log-entry from the must parameters together with the optional one.
                new_entry = {'text': message, 'timestamp': time.time() * 1000 + cls._time_delta, 'severity': severity, 'category': category}
                ...
                if Coralogix.MAX_LOG_CHUNK_SIZE <= new_entry_size:
                    ...
                    DebugLogger.warning("add_logline(): received log message too big of size= {} MB, bigger than max_log_chunk_size= {}; throwing...".format(new_entry_size/1024**2, Coralogix.MAX_LOG_CHUNK_SIZE))
                    return
    ...
    def _send_bulk(cls, time_sync=True):
        ...
        try:
            ...
            while ((len(json.dumps(cls._buffer[:size]).encode("utf8")) > Coralogix.MAX_LOG_CHUNK_SIZE) and (size > 1)):
                size = int(size/2)
            ...
            cls._buffer_size = max(cls._buffer_size, 0)  # cls._buffer_size if cls._buffer_size >= 0 else 0
        except Exception as e:
            DebugLogger.exception("Failed to send bulk", e)
        finally:
            cls._mutex.release()

        DebugLogger.info("Buffer size after removal is: {}".format(len(json.dumps(cls._buffer).encode("utf8"))-2))
    ...
    def update_time_delta_interval(cls):
        ...
        try:
            ...
            if (time.time() - cls._time_delta_last_update) >= (60 * Coralogix.SYNC_TIME_UPDATE_INTERVAL):  # 5 minutes
    ...
    @classmethod
    def _run(cls):
        """ Start a new timer thread to watch the buffer. """
        try:
            ...
            if not cls._stop:
                DebugLogger.exception("Failed to start buffer worker thread", e)
    ...
    def _internal_run(cls):
        ...
        try:
            while True:
                ...
                next_check_interval = Coralogix.FAST_SEND_SPEED_INTERVAL if cls._buffer_size > (Coralogix.MAX_LOG_CHUNK_SIZE / 2) else Coralogix.NORMAL_SEND_SPEED_INTERVAL
                DebugLogger.debug("Next buffer check is scheduled in {} seconds".format(next_check_interval))
