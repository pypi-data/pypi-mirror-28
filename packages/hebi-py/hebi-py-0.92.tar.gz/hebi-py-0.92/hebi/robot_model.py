...
class RobotModel(__UnmanagedObject):
  ...
  def add_actuator(self, actuator_type):
    ...
    actuator = parse_actuator(actuator_type)
    ...
    if (actuator.name.startswith('X5')):
      ...
      raise RuntimeError('Unknown or unsupported actuator')
  ...
  def get_end_effector(self, frame_type, positions):
    """
  ...
  def solve_inverse_kinematics(self, initial_positions, *objectives):
    ...
    # TODO: Create objectives interface and change function called on them here
    for entry in objectives:
      entry._add_objective(ik)
