...
def __get_bool(group_message, field, getter=None):
  ...
  tmp = c_int32(0)
  for i, message in enumerate(group_message.modules):
    getter(message, field, byref(tmp))
...
def __get_highresangle(group_message, field, getter=None):
  ...
  for i, message in enumerate(group_message.modules):
    getter(message, field, byref(revolutions), byref(radian_offset))
...
def __get_string(group_message, field, getter=None):
  ...
  for i, message in enumerate(group_message.modules):
    ...
    alloc_size = max(alloc_size, alloc_size_c.value)
...
def __set_string(group_message, field, value, setter=None):
  ...
  alloc_size_c = c_size_t(0)
  for i, message in enumerate(group_message.modules):
    string_buffer = value[i].encode('utf8')
    alloc_size_c.value = len(value[i]) + 1
...
# get_group_feedback_vector3f = funpart(__get_flag, getter=hebiCommandGetFlag)
