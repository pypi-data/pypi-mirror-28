...
class MacAddress(object):
  ...
  def __str__(self):
    return _bytes_to_mac_str(self._bytes)
...
def _mac_address_from_bytes(a, b, c, d, e, f):
  return MacAddress(int(a), int(b), int(c), int(d), int(e), int(f))


def _mac_address_from_string(address):
  if (type(address) != str):
    try:
      address = str(address)
    except:
...
class EntryList(UnmanagedObject):
  """
  A list of module entries. This is used internally by the Lookup class.
  Do not create directly.
  """
  ...
  def __get_entry(self, index):
    ...
    name = c_buffer.value.decode('utf-8')
    ...
    family = c_buffer.value.decode('utf-8')
...
class LookupDelegate(UnmanagedObject):
  ...
  def get_group_from_family(self, family, timeout_ms=None):
    timeout_ms = self.__parse_to(timeout_ms)
    family_buffer = create_str(family, len(family))
  ...
  def get_connected_group_from_name(self, family, name, timeout_ms=None):
    timeout_ms = self.__parse_to(timeout_ms)
    family_buffer = type_utils.create_string_buffer_compat(family, len(family))
    name_buffer = type_utils.create_string_buffer_compat(name, len(name))
