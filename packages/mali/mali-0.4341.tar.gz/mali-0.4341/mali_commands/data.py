...
import click
import requests
from mali_commands.commands import DataSync
from mali_commands.json_utils import multi_line_json_from_data, newline_json_file_from_json_file, \
    json_data_from_files, validate_json, normalize_item
...
from mali_commands.data_volume import create_data_volume, with_repo, default_data_volume_path, with_repo_dynamic, \
    map_volume
from mali_commands.options import data_volume_id_argument, no_progressbar_option, processes_option
from mali_commands.path_utils import expend_and_validate_path, get_total_files_in_path, safe_make_dirs, safe_rm_tree, \
    DestPathEnum
...
@click.pass_context
def sync_to_data_volume(ctx, volumeid, datapath, commit, processes, no_progressbar):
    with with_repo_dynamic(ctx, volumeid, datapath) as repo:
        datapath = __expend_and_validate_path(datapath, expand_vars=False, validate_path=True)

        data_sync = DataSync(ctx, repo, no_progressbar)
        combined_meta_files, combined_index_files = data_sync.create_combined_index_and_metadata(datapath)

        if combined_meta_files is not None:
            data_sync.upload_and_update_metadata(combined_meta_files)

        files_to_upload, total_files_to_upload = data_sync.upload_and_update_index(combined_index_files)
        ...
        if total_files_to_upload > 0:
            with tqdm(total=total_files_to_upload, desc='Syncing files', unit=' files', ncols=80, disable=no_progressbar) as bar:
                ...
                data_sync.upload_in_batches(files_to_upload, total_files_to_upload, callback=lambda _: bar.update())
        else:
            click.echo('No change detected, nothing to upload.')
...
def add_to_data_volume(ctx, volumeid, files, commit, processes, no_progressbar):
    ...
    with tqdm(total=total_files, desc="Adding files", unit=' files', ncols=80, disable=no_progressbar) as bar:
        with with_repo(ctx.obj.config, volumeid) as repo:
            ...
            data_sync.upload_in_batches(files, total_files, callback=lambda x: bar.update())
