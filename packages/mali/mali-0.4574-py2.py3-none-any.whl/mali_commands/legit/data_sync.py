...
class DataSync(object):
    ...
    def __upload_file_for_processing(self, file_obj, file_description):
        ...
        retry = Retrying(wait_exponential_multiplier=50, wait_exponential_max=5000)

        result = self.__ctx.obj.handle_api(self.__ctx.obj, requests.post, url, msg, retry=retry)
    ...
    def __meta_companion_file_name(self, metadata_file_name):
        return metadata_file_name[:-len(self.metadata_ext)]
    ...
    def __append_index_file(self, file_info, combined_index_files):
        ...
        if file_info['sys'] == 'local':
            ...
            rel_path = file_name
            mtime = (file_info['mtime'] - epoch).total_seconds()
    ...
    def create_combined_index_and_metadata(self, data_path):
        ...
        files_metadata = {}
        for rel_file_name in metadata_files_list:
            if rel_file_name.endswith(self.metadata_ext):
                if self.__meta_companion_file_name(rel_file_name) not in data_files_info:
                    continue
    ...
    def upload_in_batches(self, files_info, total_files=None, callback=None):
        ...
        for current_batch in get_batch_of_files_from_paths(files_info, batch_size):
            porcelain.add(self.__repo, current_batch, callback=callback)
