...
from .luqum.tree import OrOperation, Word, Item
...
class FunctionSplit(QueryFunction):
    def __init__(self, name, node):
        ...
        try:
            split_vars = map(float, split_vars)
...
class MLQueryTransformer(LuceneTreeTransformer):
    @classmethod
    def __handle_internal_function(cls, node):
    ...
    def visit_search_field(self, node, parents):
        return self.__handle_internal_function(node)
...
# noinspection PyClassicStyleClass
class SQLQueryBuilder(MLQueryVisitor):
...
def get_split_vars(split):
    ...
    start = 0.
    for phase, percentage in split.items():
...
def resolve_tree(tree):
    ...
    resolver2 = MLQueryTransformer()(resolver(tree))
