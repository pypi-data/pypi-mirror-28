...
import click
import requests
from mali_commands.legit.data_sync import DataSync
...
from mali_commands.legit.path_utils import expend_and_validate_path, get_total_files_in_path, safe_make_dirs, safe_rm_tree, \
    DestPathEnum
...
@click.option('--bucket')
@click.pass_context
def _cmd_create_data_volume(ctx, displayname, description, org, datapath, bucket):
    data = {}
...
def sync_to_data_volume(ctx, volumeid, datapath, commit, processes, no_progressbar):
    ...
    with with_repo_dynamic(ctx, volumeid, datapath) as repo:
        ...
        if combined_index_files is None:
            click.echo('No files found for upload.')
            return
