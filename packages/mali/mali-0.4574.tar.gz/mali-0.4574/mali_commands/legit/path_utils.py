...
def __enumerate_path_s3(bucket, fields=None):
    bucket = remove_moniker(bucket)

    bucket, prefix = __split_bucket_prefix(bucket)
...
def file_name_to_info(path, fields=None):
    params = {
        'path': path,
        'sys': 'local',
    }
    ...
    params['mtime'] = datetime.datetime.fromtimestamp(st.st_mtime)
    params['ctime'] = datetime.datetime.fromtimestamp(st.st_ctime)
    params['size'] = st.st_size
    params['mode'] = st.st_mode
...
def __enumerate_path_local(path, fields=None):
    if os.path.isfile(path):
        yield file_name_to_info(path, fields)
        return
