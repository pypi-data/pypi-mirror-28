...
#!defFunction(anvil.users,_)!2: "Get the row from the users table that corresponds to the currently logged-in user. Returns None if no user is logged in." ["get_user"]
def get_user():
...
#!defFunction(anvil.users,_,email,password,[remember])!2: "Log in with the specified email address and password. By default, login status is not remembered between sessions. Returns None if authentication failed." ["login_with_email"]
...
#!defFunction(anvil.users,_,email,password,[remember])!2: "Sign up for a new account with the specified email address and password. By default, login status is not remembered between sessions. Raises anvil.users.UserExists if an account is already registered with this email address." ["signup_with_email"]
...
if is_server_side():
    ...
    #!defFunction(anvil.users,_,user_row,[remember])!2: "Set the specified user object (a row from a Data Table) as the current logged-in user. It must be a row from the users table. By default, login status is not remembered between sessions." ["force_login"]
    def force_login(user, remember=False):
        anvil.server.call("anvil.private.users.force_login", user, remember=remember)
    ...
    #!defFunction(anvil.users,!,[additional_scopes],[remember])!2: "Log in with a Google account. Prompts the user to authenticate with Google, then logs in with their Google email address (if that user exists). Returns None if the login was cancelled or we have no record of this user.\nIf supplied, additional_scopes are passed on to google.auth.login(). By default, login status is not remembered between sessions." ["login_with_google"]
    ...
    #!defFunction(anvil.users,!,[remember])!2: "Sign up for a new account with the email address associated with the user's Google account. Prompts the user to authenticate with Google, then registers a new user with that email address. Raises anvil.users.UserExists if this email address is already registered; returns new user or None if cancelled. By default, login status is not remembered between sessions." ["signup_with_google"]
    ...
    #!defFunction(anvil.users,!,[remember])!2: "Log in with a Facebook account. Prompts the user to authenticate with Facebook, then logs in with their Facebook email address (if that user exists). Returns None if the login was cancelled or we have no record of this user. By default, login status is not remembered between sessions." ["login_with_facebook"]
    ...
    #!defFunction(anvil.users,!,[remember])!2: "Sign up for a new account with the email address associated with the user's Facebook account. Prompts the user to authenticate with Facebook, then registers a new user with that email address. Raises anvil.users.UserExists if this email address is already registered; returns new user or None if cancelled. By default, login status is not remembered between sessions." ["signup_with_facebook"]
    ...
    #defFunction(anvil.users,!,[remember])!2: "Log in with a Raven account. Prompts the user to authenticate with Raven, then logs in with their Raven account (if that user exists). Returns None if the login was cancelled or we have no record of this user. By default, login status is not remembered between sessions." ["login_with_raven"]
    ...
    #defFunction(anvil.users,!,[remember])!2: "Sign up for a new account with the email address associated with the user's Raven account. Prompts the user to authenticate with Raven, then registers a new user with that email address. Raises anvil.users.UserExists if this email address is already registered; returns new user or None if cancelled. By default, login status is not remembered between sessions." ["signup_with_raven"]
    ...
    #!defFunction(anvil.users,!,[remember])!2: "Display a sign-up form allowing a user to create a new account. The "Remember me" checkbox is checked by default, if enabled. Returns the new user object, or None if cancelled." ["signup_with_form"]
    def signup_with_form(_link_back_to_login_on_already_exists=False,remember=True):
        if not get_client_config().get("allow_signup"):
            raise Exception("New user signup is not enabled")
        ...
        if get_client_config().get("allow_remember_me", False) and not get_client_config().get("confirm_email", False):
            remember_me_checkbox = CheckBox(text="Remember me", checked=remember)
    ...
    #!defFunction(anvil.users,!,[show_signup_option=],[remember])!2: "Display a login form and allow user to log in. The "Remember me" checkbox is checked by default, if enabled. Returns user object if logged in, or None if cancelled.\n\nIf signups are enabled, the form will also show the option to sign up for a new account. This can be disabled by passing show_signup_option=False." ["login_with_form"]
    def login_with_form(show_signup_option=True,remember=True):
        lp = LinearPanel()
        email_box = None
        passwd_box = None
        remember_me_checkbox = None
