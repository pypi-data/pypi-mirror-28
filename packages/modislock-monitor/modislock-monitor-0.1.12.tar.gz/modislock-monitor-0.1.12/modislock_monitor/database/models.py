...
class Locations(Base):
    ...
    id_locations = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(Unicode(45), unique=True)
    reader_settings_id_reader = Column(Integer, ForeignKey('reader_settings.id_reader'), nullable=True)
...
class Readings(Base):
    ...
    id_readings = Column(Integer, primary_key=True, autoincrement=True)
...
class StaticReadings(Base):
    __tablename__ = 'static_readings'

    id_static_readings = Column(Integer, primary_key=True, autoincrement=True)
    timestamp = Column(TIMESTAMP, nullable=False, default=func.now())
    value = Column(Unicode(10), nullable=False)
    sensors_id_sensors = Column(Integer, ForeignKey('sensors.id_sensors'), nullable=False)

    def __repr__(self):
        return "StaticReadings(id_static_readings={self.id_static_readings}, timestamp={self.timestamp}, " \
               "value={self.value}, sensors_id_sensors={self.sensors_id_sensors})".format(self=self)
