...
add_reading:BEGIN
    ...
    CREATE TEMPORARY TABLE readings_window (id_readings INT NOT NULL PRIMARY KEY) ENGINE=MEMORY;

	SET @sqlstmt= CONCAT('INSERT INTO readings_window SELECT id_readings FROM readings ORDER BY id_readings DESC LIMIT ', KeepLimit);
	PREPARE st FROM @sqlstmt; 
	EXECUTE st; 
	DEALLOCATE PREPARE st;
    
    SELECT SLEEP(5);
	DELETE FROM readings WHERE NOT EXISTS(SELECT NULL FROM readings_window w WHERE w.id_readings = readings.id_readings);
    DROP TABLE IF EXISTS readings_window;
...
GRANT SELECT, INSERT, TRIGGER, UPDATE, CREATE TEMPORARY TABLES, DELETE ON TABLE `modislock`.* TO 'modismon';
...
DROP EVENT IF EXISTS `purge_overflow` $$
    
CREATE EVENT IF NOT EXISTS `purge_overflow` 
	ON SCHEDULE EVERY 1 HOUR STARTS '2017-01-01 00:00:00' 
	DO BEGIN
        SET @record_count := (SELECT COUNT(*) FROM readings);
        IF @record_count > 2000 THEN TRUNCATE readings;
        END IF;
	END $$
