...
class CommandV(Extensible):
    ...
    def create(self):
        ...
        self.instance_manager.add_repo_web()
    ...
    def backup(self, backup_dir, database_name, cformat=False, reason=False, tmp_dir=False,
               prefix=False):
        res = {'command': 'backup'}
        tmp_dir = tmp_dir or self.__config.get('container_config').get('temp_folder')
        try:
    ...
    def deactivate_backup(self, db_config, backup, prefix, store_path, cformat, admin_password,
        ...
        temp_folder = self.__config.get('container_config').get('temp_folder')
        working_dir = mkdtemp(dir=temp_folder, prefix='deployv_')
    ...
    def _production_backup_and_destroy(self, container_name, status_checks,
        ...
        manager = self.__instance_manager
        config = self.__config
        tmp_dir = tmp_dir or config.get('container_config').get('temp_folder')
        res = {'result': {}}
        # Stop instance
