...
class BuildImage(object):
    ...
    def _clone_dependencies(self, instance_path, extra_path):
        ...
        temp_folder = self._config.get('container_config').get('temp_folder', '/tmp')
    ...
    def build(self):
        ...
        if self.full_stack_image:
            values = {
                ...
                'db_user': self._config.get('instance').get('config').get('db_user'),
                'db_password': self._config.get('instance').get('config').get('db_password'),
                'db_name': self._config.get('instance').get('config').get('db_name')
            }
...
def build_image(config, version=False, working_folder=False, force=False, tag=False):
    ...
    working_folder = working_folder or config.get('container_config', {}).get('temp_folder')
    ...
    build_class = BuildImage(config, version, working_folder, force,
