...
class InstanceV(DockerV):
    ...
    def add_authorized_keys(self):
        tmp = self.config.get('temp_folder') or '/tmp'
    ...
    def deploy_key(self):
        ...
        """
        if not utils.is_base64_encode(self.__full_config.get('instance').get('ssh_key')):
    ...
    def add_repo_web(self):
        for repo in self.__full_config.get("instance").get("repositories"):
            if repo.get('name') == 'isolated_addons':
                ...
                return
        version = utils.version_cid(self.__full_config.get('instance').get('customer_id'))
    ...
    def restore_database(self, db_name=False, backup_dir=False):
        ...
        temp_folder = self.config.get('temp_folder') or '/tmp'
...
def deactivate_database(config, db_name):
    ...
    sqls = {
        ...
        'orchest_parameters': ("UPDATE ir_config_parameter SET value=''"
                               " WHERE key in ('orchestv.github_access_token',"
                               " 'orchestv.docker_hub_access_token',"
                               " 'orchestv.quay_io_access_token', 'orchestv.odoo_listener_id',"
                               " 'orchestv.worker_security_token',"
                               " 'orchestv.gitlab_access_token')")
    }
