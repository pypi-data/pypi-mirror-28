...
#
class SQLFactory_MySQL(AbstractSQLFactory):

	def __init__(self):
		super().__init__(
			"MySQL",
			{
				EnumDBColType.PK : "BIGINT AUTO_INCREMENT PRIMARY KEY NOT NULL",
				EnumDBColType.BOOL : "TINYINT",
				EnumDBColType.INT32 : "INT",
				EnumDBColType.INT64 : "BIGINT",
				EnumDBColType.DOUBLE : "DOUBLE PRECISION",
				EnumDBColType.STR256 : "VARCHAR(256)",
				EnumDBColType.CLOB : "LONGTEXT",
				EnumDBColType.BLOB : "LONGBLOB",
			},
			{
	...
	def sqlStmt_addTableColumn(self, tableDef, columnDef):
		return ("ALTER TABLE `{}` ADD COLUMN {} DEFAULT %s".format(tableDef.name, self._sql_colDefStr(columnDef)), self._defaultValue(columnDef.type))
	#
	...
	def sqlStmt_dropTableColumn(self, tableDef, columnDef):
		return "ALTER TABLE `{}` DROP COLUMN `{}`".format(tableDef.name, columnDef.name)
	#
	...
	def sqlStmt_renameTableColumn(self, tableDef, columnDef, newName):
		return ("ALTER TABLE `{}` CHANGE COLUMN `{}` `{}` {} DEFAULT %s".format(tableDef.name, columnDef.name, newName, self._sql_colDefStr(columnDef)), self._defaultValue(columnDef.type))
	#
	...
	def sqlStmt_modifyTableColumnType(self, tableDef, columnDef, fieldType, bIsNullable):
		return ("ALTER TABLE `{}` CHANGE COLUMN `{}` {} {} DEFAULT %s".format(tableDef.name, columnDef.name, self._toSQLType(fieldType), "NULL" if bIsNullable else "NOT NULL"), self._defaultValue(columnDef.type))
	#
