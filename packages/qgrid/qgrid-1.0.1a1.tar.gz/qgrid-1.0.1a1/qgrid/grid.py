...
class QgridWidget(widgets.DOMWidget):
    ...
    _view_module_version = Unicode('1.0.1-alpha.1').tag(sync=True)
    _model_module_version = Unicode('1.0.1-alpha.1').tag(sync=True)
    ...
    def _update_table(self, update_columns=False, triggered_by=None,
        ...
        from_index = max(self._viewport_range[0] - self.PAGE_SIZE, 0)
        to_index = max(self._viewport_range[0] + self.PAGE_SIZE, 0)
    ...
    def _handle_qgrid_msg_helper(self, content):
        ...
        if content['type'] == 'cell_change':
            ...
            from_index = max(content['top'] - self.PAGE_SIZE, 0)
            to_index = max(content['top'] + self.PAGE_SIZE, 0)
