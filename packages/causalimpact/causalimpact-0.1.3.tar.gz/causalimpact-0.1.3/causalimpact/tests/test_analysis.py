...
class TestRunWithData(object):
    ...
    def test_missing_values(self):
        ...
        point predictions have missing values at the time points the result
        time series has missing values."""
        predicted_cols = [impact.inferences.columns.get_loc(col) for col in impact.inferences.columns if (
            "response" not in col and "point_effect" not in col)]
        effect_cols = [impact.inferences.columns.get_loc(
            col) for col in impact.inferences.columns if "point_effect" in col]
        response_cols = [impact.inferences.columns.get_loc(
            col) for col in impact.inferences.columns if "response" in col]
        ...
        assert (np.any(np.isnan(impact.inferences.iloc[
                95:100, predicted_cols])) == False)
    ...
    CausalImpact models: one on the raw data, one after setting the last
    entries of the gap to NA, and one after setting earlier entries of the gap
    to NA. Test that the estimated effects of the 3 fits are nearly identical.
    """

    def test_identical_fit(self):
        pre_period = [0, 70]
        post_period = [101, 200]
        impact = CausalImpact(data, pre_period, post_period, model_args)
        impact.run()
        data_na_end = data.copy()
        data_na_end.iloc[90:100, 0] = np.nan
        impact_na_end = CausalImpact(
            data_na_end, pre_period, post_period, model_args)
        impact_na_end.run()
        data_na_middle = data.copy()
        data_na_middle.iloc[71:80, 0] = np.nan
        impact_na_middle = CausalImpact(
            data_na_middle, pre_period, post_period, model_args)
        impact_na_middle.run()

        """Do not compare columns with cumulative response (which is impacted by NAs in
        response) and cumulative predictions (which are set to cumulative
        response before the post-period.)"""
        print(impact.inferences.columns)
        assert_frame_equal(impact.inferences.iloc[
                           :70, :], impact_na_end.inferences.iloc[:70, :])
        assert_frame_equal(impact.inferences.iloc[
                           :70, :], impact_na_middle.inferences.iloc[:70, :])

    def test_bayesian(self):
        impact = CausalImpact(data, pre_period, post_period,
                              model_args, estimation='notMLE')
    ...
    def test_standardize(self):
        ...
        impact1 = CausalImpact(data, pre_period, post_period, model_args)
        impact1.run()
        model_args = {"niter": 123, 'standardize_data': True}
        impact2 = CausalImpact(data, pre_period, post_period, model_args)
        impact2.run()
        pd.util.testing.assert_numpy_array_equal(impact1.inferences.response.values, data.y.values)
        pd.util.testing.assert_numpy_array_equal(impact2.inferences.response.values, data.y.values)
        assert_frame_equal(impact1.inferences, impact2.inferences)

    def test_ground_truth(self):
        """ Recover ground truth (generative model with 1 covariate)
        without/without <standardize.data>"""
        n = 500
        x1 = np.sin(np.arange(n) / 100) + np.random.randn(n)/10
        w = np.random.randn(n)/1
        beta = [5, 3]
        y = beta[0] + beta[1] * x1 + w
        pre_period = [0, 250]
        post_period = [251, 499]
        data = pd.DataFrame({"y": y, "x": x1})
        model_args = {"niter": 500, "standardize_data": False}
        impact1 = CausalImpact(data, pre_period, post_period, model_args)
        impact1.run()
        fit = impact1.model.fit()
        estimates1 = fit.params
        assert estimates1[2] == beta[1]
