...
regNamesTable = {} # Keys are str, values are integers
revertRegNamesTable = {} # Keys are integers, values are str. This is the invert of regNamesTable


def printRegTranslation():
	"""
	Prints the translation of registers ( eax, esp, ... ) into intern representation ( R0, R1, R2, ... )
...
def setArch( arch ):
	...
	if( arch in supportedArchs ):
		...
		raise AnalysisException("Architecture %s is not supported. Sorry ! " % arch)
...
def getIR(opCodeStr, address ):
	...
	index = 0
	ins = []
	# disasemble -> return only one instr, so we need to iterate over the string gadget, using the size of the ins disassembled
	while(index < len(opCodeStr)):
		asm = disassembler.disassemble(opCodeStr[index:],index+address)
		if( asm is None ):
			raise AnalysisException("BARF unable to translate %s instructions %s in gadget %s" % (arch, "\\x"+ "\\x".join("{:02x}".format(ord(c)) for c in opCodeStr[index:]), "\\x"+ "\\x".join("{:02x}".format(ord(c)) for c in opCodeStr)))
		ins.append(asm)
		index=index+asm.size
	irsb = []
    	[irsb.append( r ) for i in ins for r in ir_translator.translate(i)]
	return (irsb,ins)	
