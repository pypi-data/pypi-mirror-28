...
# Custom simplifiers using Z3 tactics 
def customSimplify( condZ3 ):
...
class Cond:
	...
	Represents a condition on registers and/or memory 
		...
		(self.simplifiedHard) - (Bool) True <=> self.z3 have been simplified using the hard simplification function 
	...
	def __init__(self, condObject):
		...
		self.simplifiedSoft = condObject.simplifiedSoft
		self.simplifiedHard = condObject.simplifiedHard
	...
	def __init__(self, cond, left, right, z3 = None, cleaned = False, checked = False ):
		...
		self.right = right
		self.z3 = z3
		self.simplifiedSoft = False
		self.simplifiedHard = False
	...
	def simplifyZ3(self, hard=False):
		"""
		...
		Parameters:
			hard - (bool) Must be set to true if the simplification must use a Z3 solver and check() function
			/!\ Notice that a 'hard simplification' is way more expensive than a 'soft simplification' but can
			return better results 
		"""
		if( self.simplifiedHard == True ):
			return self.z3 
		elif( self.simplifiedSoft == True ):
			if( hard == True ):
				self.z3 = hardSImplify(self.z3)
				if( self.z3 == BoolSort().cast(True)):
					self.setTrue()
				elif( self.z3 == BoolSort().cast(False)):
					self.setFalse()
				self.simplifiedHard = True
			return self.z3 
		else:
			if( hard == False ):
				self.z3 = customSimplify(self.toZ3())
				self.simplifiedSoft = True
			else:
				self.z3 = hardSImplify(self.z3)
				if( self.z3 == BoolSort().cast(True)):
					self.setTrue()
	...
	def setTrue(self):
		...
		self.z3 = BoolSort().cast(True)
		self.cleaned = self.simplifiedHard = self.simplifiedSoft = self.checked = True
