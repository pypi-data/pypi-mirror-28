...
from Expr import *
from Cond import *
from Database import revertRegNamesTable
		...
		for reg in self.nodes.keys():
			...
			if( not( isinstance(node, MEMNode) and not isinstance(node,ITENode))):
				...
				if( self.lastMod.get(node.reg.num) != None ):
					self.lastMod[node.reg.num] = max( self.lastMod[node.reg.num], int(reg.split('_')[1]))
				else:
					self.lastMod[node.reg.num] = 0
...
class MEMNode(Node):
	...
	def getDependencies( self, num, addr, size, gadgetDep ):
		...
		for a in sorted( self.outgoingArcs, reverse=False):
			...
			higher = Op( "Add", [addr, ConstExpr(size, addr.size)] )
			lower = Op( "Sub", [addr, ConstExpr( a.size, addr.size )])
