...
class BaseForcaster(object):
    def __init__(self, dimensions=None, field_map={}, exclude=[]):
        ...
        self.exclude = exclude + [self.mid_field, self.payment_field]
    ...
    #     df = self.data
    #     # X, X_test, Y, Y_test = model_selection.train_test_split(df[self.dimensions], df[self.payment_field], test_size=self.test_rate, random_state=1)
...
class DecisionTreeForcaster(BaseForcaster):
    ...
    def train(self, df, Y=None):
        if Y is None:
            Y = df[self.payment_field]
            X = df[self.dimensions]
        else:
            X = df
        self.tree.fit(X, Y)

    def predict(self, df, need_categorical=False):
        if need_categorical:
...
class DecisionTreeClassifier(DecisionTreeForcaster):
    ...
    def normalize(self):
        ...
        df = self.data
        df[self.payment_field] = pd.Categorical(df[self.payment_field]).codes
...
#     def predict(self, df):
#         return pd.DataFrame(
#             [(row.gid, self.predict_object(row)[0], row.pay_amount) for index, row in df.iterrows()],
