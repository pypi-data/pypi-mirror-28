...
settings = ChainMap(load(DEFAULT_SETTINGS_LOCATION), load(open(USER_SETTINGS_LOCATION)) if exists(USER_SETTINGS_LOCATION) else {})

if "plugins" not in settings["cli"] or not settings["cli"]["plugins"]:
