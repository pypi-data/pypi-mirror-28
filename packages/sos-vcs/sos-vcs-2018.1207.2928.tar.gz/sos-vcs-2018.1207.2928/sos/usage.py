...
MARKER = r"/###/"  # type: str  # line 1


def usage(appname, version: 'str', short: 'bool'=False):  # line 4
    print("{marker} {appname}{version}".format(marker=MARKER, appname=appname, version="" if not short else " (PyPI: %s)" % version))  # line 5
    if not short:  # line 6
        print("""
...
Usage: {cmd} <command> [<argument>] [<option1>, ...]        When operating in offline mode, or command is one of "help", "offline", "version"
  ...
  Common options:
    ...
    --verbose                    Enable verbose output, including show compression ratios""".format(appname=appname, cmd="sos", CMD="SOS"))  # line 85
    Exit(code=0)  # line 86
