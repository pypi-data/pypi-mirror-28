...
def dump(argument: 'str', options: '_coconut.typing.Sequence[str]'=[]):  # line 799
    ...
    with zipfile.ZipFile(argument, "w", compression) as fd:  # line 810
        ...
        totalsize = 0  # type: int  # line 813
        start_time = time.time()  # type: float  # line 814
        for dirpath, dirnames, filenames in os.walk(repopath):  # TODO use index knowledge instead of walking to avoid adding stuff not needed?  # line 815
            for filename in filenames:  # line 816
                newtime = time.time()  # type: float  # TODO alternatively count bytes and use a threshold there  # line 817
                abspath = os.path.join(dirpath, filename)  # type: str  # line 818
                relpath = os.path.relpath(abspath, repopath)  # type: str  # line 819
                totalsize += os.stat(abspath).st_size  # line 820
                if progress and newtime - timer > .1:  # line 821
                    sys.stdout.write("Dumping %s@%6.2f MiB/s %s\r" % (PROGRESS_MARKER[int(counter.inc() % 4)], totalsize / (MEBI * (time.time() - start_time)), filename))  # line 821
                    sys.stdout.flush()  # line 821
                    timer = newtime  # line 821
                fd.write(abspath, relpath.replace(os.sep, "/"))  # write entry into archive  # line 822

def config(arguments: 'List[str]', options: 'List[str]'=[]):  # line 824
    command, key, value = (arguments + [None] * 2)[:3]  # line 825
    if command not in ["set", "unset", "show", "list", "add", "rm"]:  # line 826
        Exit("Unknown config command")  # line 826
    local = "--local" in options  # type: bool  # line 827
    m = Metadata(os.getcwd())  # type: Metadata  # loads layerd configuration as well. TODO warning if repo not exists  # line 828
    c = m.c if local else m.c.__defaults  # type: configr.Configr  # line 829
    if command == "set":  # line 830
        if None in (key, value):  # line 831
            Exit("Key or value not specified")  # line 831
        if key not in (["defaultbranch"] + ([] if local else CONFIGURABLE_FLAGS) + CONFIGURABLE_LISTS):  # line 832
            Exit("Unsupported key for %s configuration %r" % ("local " if local else "global", key))  # line 832
        if key in CONFIGURABLE_FLAGS and value.lower() not in TRUTH_VALUES + FALSE_VALUES:  # line 833
            Exit("Cannot set flag to '%s'. Try on/off instead" % value.lower())  # line 833
        c[key] = value.lower() in TRUTH_VALUES if key in CONFIGURABLE_FLAGS else (value.strip() if key not in CONFIGURABLE_LISTS else safeSplit(value, ";"))  # TODO sanitize texts?  # line 834
    elif command == "unset":  # line 835
        if key is None:  # line 836
            Exit("No key specified")  # line 836
        if key not in c.keys():  # line 837
            Exit("Unknown key")  # line 837
        del c[key]  # line 838
    elif command == "add":  # line 839
        if None in (key, value):  # line 840
            Exit("Key or value not specified")  # line 840
        if key not in CONFIGURABLE_LISTS:  # line 841
            Exit("Unsupported key %r" % key)  # line 841
        if key not in c.keys():  # add list  # line 842
            c[key] = [value]  # add list  # line 842
        elif value in c[key]:  # line 843
            Exit("Value already contained, nothing to do")  # line 843
        c[key].append(value)  # line 844
    elif command == "rm":  # line 845
        if None in (key, value):  # line 846
            Exit("Key or value not specified")  # line 846
        if key not in c.keys():  # line 847
            Exit("Unknown key %r" % key)  # line 847
        if value not in c[key]:  # line 848
            Exit("Unknown value %r" % value)  # line 848
        c[key].remove(value)  # line 849
    else:  # Show or list  # line 850
        if key == "flags":  # list valid configuration items  # line 851
            print(", ".join(CONFIGURABLE_FLAGS))  # list valid configuration items  # line 851
        elif key == "lists":  # line 852
            print(", ".join(CONFIGURABLE_LISTS))  # line 852
        elif key == "texts":  # line 853
            print(", ".join([_ for _ in defaults.keys() if _ not in (CONFIGURABLE_FLAGS + CONFIGURABLE_LISTS)]))  # line 853
        else:  # line 854
            out = {3: "[default]", 2: "[global] ", 1: "[local]  "}  # type: Dict[int, str]  # line 855
            c = m.c  # always use full configuration chain  # line 856
            try:  # attempt single key  # line 857
                assert key is not None  # line 858
                c[key]  # line 858
                l = key in c.keys()  # type: bool  # line 859
                g = key in c.__defaults.keys()  # type: bool  # line 859
                print("%s %s %r" % (key.rjust(20), out[3] if not (l or g) else (out[1] if l else out[2]), c[key]))  # line 860
            except:  # normal value listing  # line 861
                vals = {k: (repr(v), 3) for k, v in defaults.items()}  # type: Dict[str, Tuple[str, int]]  # line 862
                vals.update({k: (repr(v), 2) for k, v in c.__defaults.items()})  # line 863
                vals.update({k: (repr(v), 1) for k, v in c.__map.items()})  # line 864
                for k, vt in sorted(vals.items()):  # line 865
                    print("%s %s %s" % (k.rjust(20), out[vt[1]], vt[0]))  # line 865
                if len(c.keys()) == 0:  # line 866
                    info("No local configuration stored")  # line 866
                if len(c.__defaults.keys()) == 0:  # line 867
                    info("No global configuration stored")  # line 867
        return  # in case of list, no need to store anything  # line 868
    if local:  # saves changes of repoConfig  # line 869
        m.repoConf = c.__map  # saves changes of repoConfig  # line 869
        m.saveBranches()  # saves changes of repoConfig  # line 869
        Exit("OK", code=0)  # saves changes of repoConfig  # line 869
    else:  # global config  # line 870
        f, h = saveConfig(c)  # only saves c.__defaults (nested Configr)  # line 871
        if f is None:  # line 872
            error("Error saving user configuration: %r" % h)  # line 872
        else:  # line 873
            Exit("OK", code=0)  # line 873

def move(relPath: 'str', pattern: 'str', newRelPath: 'str', newPattern: 'str', options: 'List[str]'=[]):  # line 875
    ''' Path differs: Move files, create folder if not existing. Pattern differs: Attempt to rename file, unless exists in target or not unique. '''  # line 876
    force = '--force' in options  # type: bool  # line 877
    soft = '--soft' in options  # type: bool  # line 878
    if not os.path.exists(relPath.replace(SLASH, os.sep)) and not force:  # line 879
        Exit("Source folder doesn't exist. Use --force to proceed anyway")  # line 879
    m = Metadata(os.getcwd())  # type: Metadata  # line 880
    matching = fnmatch.filter(os.listdir(relPath.replace(SLASH, os.sep)) if os.path.exists(relPath.replace(SLASH, os.sep)) else [], os.path.basename(pattern))  # type: List[str]  # find matching files in source  # line 881
    matching[:] = [f for f in matching if len([n for n in m.c.ignores if fnmatch.fnmatch(f, n)]) == 0 or len([p for p in m.c.ignoresWhitelist if fnmatch.fnmatch(f, p)]) > 0]  # line 882
    if not matching and not force:  # line 883
        Exit("No files match the specified file pattern. Use --force to proceed anyway")  # line 883
    if not m.track and not m.picky:  # line 884
        Exit("Repository is in simple mode. Simply use basic file operations to modify files, then execute 'sos commit' to version the changes")  # line 884
    if pattern not in m.branches[m.branch].tracked:  # line 885
        for tracked in (t for t in m.branches[m.branch].tracked if os.path.dirname(t) == relPath):  # for all patterns of the same source folder  # line 886
            alternative = fnmatch.filter(matching, os.path.basename(tracked))  # type: _coconut.typing.Sequence[str]  # find if it matches any of the files in the source folder, too  # line 887
            if alternative:  # line 888
                info("  '%s' matches %d files" % (tracked, len(alternative)))  # line 888
        if not (force or soft):  # line 889
            Exit("File pattern '%s' is not tracked on current branch. 'sos move' only works on tracked patterns" % pattern)  # line 889
    basePattern = os.path.basename(pattern)  # type: str  # pure glob without folder  # line 890
    newBasePattern = os.path.basename(newPattern)  # type: str  # line 891
    if basePattern.count("*") < newBasePattern.count("*") or (basePattern.count("?") - basePattern.count("[?]")) < (newBasePattern.count("?") - newBasePattern.count("[?]")) or (basePattern.count("[") - basePattern.count("\\[")) < (newBasePattern.count("[") - newBasePattern.count("\\[")) or (basePattern.count("]") - basePattern.count("\\]")) < (newBasePattern.count("]") - newBasePattern.count("\\]")):  # line 892
        Exit("Glob markers from '%s' to '%s' don't match, cannot move/rename tracked matching files" % (basePattern, newBasePattern))  # line 896
    oldTokens = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 897
    newToken = None  # type: _coconut.typing.Sequence[GlobBlock]  # line 897
    oldTokens, newTokens = tokenizeGlobPatterns(os.path.basename(pattern), os.path.basename(newPattern))  # line 898
    matches = convertGlobFiles(matching, oldTokens, newTokens)  # type: _coconut.typing.Sequence[Tuple[str, str]]  # computes list of source - target filename pairs  # line 899
    if len({st[1] for st in matches}) != len(matches):  # line 900
        Exit("Some target filenames are not unique and different move/rename actions would point to the same target file")  # line 900
    matches = reorderRenameActions(matches, exitOnConflict=not soft)  # attempts to find conflict-free renaming order, or exits  # line 901
    if os.path.exists(newRelPath):  # line 902
        exists = [filename[1] for filename in matches if os.path.exists(os.path.join(newRelPath, filename[1]).replace(SLASH, os.sep))]  # type: _coconut.typing.Sequence[str]  # line 903
        if exists and not (force or soft):  # line 904
            Exit("%s files would write over existing files in %s cases. Use --force to execute it anyway" % ("Moving" if relPath != newRelPath else "Renaming", "all" if len(exists) == len(matches) else "some"))  # line 904
    else:  # line 905
        os.makedirs(os.path.abspath(newRelPath.replace(SLASH, os.sep)))  # line 905
    if not soft:  # perform actual renaming  # line 906
        for (source, target) in matches:  # line 907
            try:  # line 908
                shutil.move(os.path.abspath(os.path.join(relPath, source).replace(SLASH, os.sep)), os.path.abspath(os.path.join(newRelPath, target).replace(SLASH, os.sep)))  # line 908
            except Exception as E:  # one error can lead to another in case of delicate renaming order  # line 909
                error("Cannot move/rename file '%s' to '%s'" % (source, os.path.join(newRelPath, target)))  # one error can lead to another in case of delicate renaming order  # line 909
    m.branches[m.branch].tracked[m.branches[m.branch].tracked.index(pattern)] = newPattern  # line 910
    m.saveBranches()  # line 911

def parse(root: 'str', cwd: 'str'):  # line 913
    ''' Main operation. Main has already chdir into VCS root folder, cwd is original working directory for add, rm. '''  # line 914
    debug("Parsing command-line arguments...")  # line 915
    try:  # line 916
        command = sys.argv[1].strip() if len(sys.argv) > 1 else ""  # line 917
        arguments = [c.strip() for c in sys.argv[2:] if not c.startswith("--")]  # type: Union[List[str], str, None]  # line 918
        if len(arguments) == 0:  # line 919
            arguments = [None]  # line 919
        options = [c.strip() for c in sys.argv[2:] if c.startswith("--")]  # line 920
        onlys, excps = parseOnlyOptions(cwd, options)  # extracts folder-relative information for changes, commit, diff, switch, update  # line 921
        debug("Processing command %r with arguments %r and options %r." % (("" if command is None else command), arguments if arguments else "", options))  # line 922
        if command[:1] in "amr":  # line 923
            relPath, pattern = relativize(root, os.path.join(cwd, arguments[0] if arguments else "."))  # line 923
        if command[:1] == "m":  # line 924
            if len(arguments) < 2:  # line 925
                Exit("Need a second file pattern argument as target for move/rename command")  # line 925
            newRelPath, newPattern = relativize(root, os.path.join(cwd, options[0]))  # line 926
        if command[:1] == "a":  # line 927
            add(relPath, pattern, options)  # line 927
        elif command[:1] == "b":  # line 928
            branch(arguments[0], options)  # line 928
        elif command[:2] == "ch":  # line 929
            changes(arguments[0], options, onlys, excps)  # line 929
        elif command[:3] == "com":  # line 930
            commit(arguments[0], options, onlys, excps)  # line 930
        elif command[:2] == "ci":  # line 931
            commit(arguments[0], options, onlys, excps)  # line 931
        elif command[:3] == 'con':  # line 932
            config(arguments, options)  # line 932
        elif command[:2] == "de":  # line 933
            delete(arguments[0], options)  # line 933
        elif command[:2] == "di":  # line 934
            diff(arguments[0], options, onlys, excps)  # line 934
        elif command[:2] == "du":  # line 935
            dump(arguments[0], options)  # line 935
        elif command[:1] == "h":  # line 936
            usage()  # line 936
        elif command[:2] == "lo":  # line 937
            log(options)  # line 937
        elif command[:2] == "li":  # TODO handle absolute paths as well, also for Windows? think through  # line 938
            ls(relativize(root, cwd if not arguments else os.path.join(cwd, arguments[0]))[1], options)  # TODO handle absolute paths as well, also for Windows? think through  # line 938
        elif command[:2] == "ls":  # TODO avoid and/or normalize root super paths (..)  # line 939
            ls(relativize(root, cwd if not arguments else os.path.join(cwd, arguments[0]))[1], options)  # TODO avoid and/or normalize root super paths (..)  # line 939
        elif command[:1] == "m":  # line 940
            move(relPath, pattern, newRelPath, newPattern, options[1:])  # line 940
        elif command[:2] == "of":  # line 941
            offline(arguments[0], options)  # line 941
        elif command[:2] == "on":  # line 942
            online(options)  # line 942
        elif command[:1] == "r":  # line 943
            remove(relPath, pattern)  # line 943
        elif command[:2] == "st":  # line 944
            status(options, onlys, excps)  # line 944
        elif command[:2] == "sw":  # line 945
            switch(arguments[0], options, onlys, excps)  # line 945
        elif command[:1] == "u":  # line 946
            update(arguments[0], options, onlys, excps)  # line 946
        elif command[:1] == "v":  # line 947
            usage(short=True)  # line 947
        else:  # line 948
            Exit("Unknown command '%s'" % command)  # line 948
        Exit(code=0)  # line 949
    except (Exception, RuntimeError) as E:  # line 950
        print(str(E))  # line 951
        import traceback  # line 952
        traceback.print_exc()  # line 953
        traceback.print_stack()  # line 954
        try:  # line 955
            traceback.print_last()  # line 955
        except:  # line 956
            pass  # line 956
        Exit("An internal error occurred in SOS. Please report above message to the project maintainer at  https://github.com/ArneBachmann/sos/issues  via 'New Issue'.\nPlease state your installed version via 'sos version', and what you were doing.")  # line 957

def main():  # line 959
    global debug, info, warn, error  # line 960
    logging.basicConfig(level=level, stream=sys.stderr, format=("%(asctime)-23s %(levelname)-8s %(name)s:%(lineno)d | %(message)s" if '--log' in sys.argv else "%(message)s"))  # line 961
    _log = Logger(logging.getLogger(__name__))  # line 962
    debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 962
    for option in (o for o in ['--log', '--verbose', '-v', '--sos'] if o in sys.argv):  # clean up program arguments  # line 963
        sys.argv.remove(option)  # clean up program arguments  # line 963
    if '--help' in sys.argv or len(sys.argv) < 2:  # line 964
        usage()  # line 964
    command = sys.argv[1] if len(sys.argv) > 1 else None  # type: _coconut.typing.Optional[str]  # line 965
    root, vcs, cmd = findSosVcsBase()  # root is None if no .sos folder exists up the folder tree (still working online); vcs is checkout/repo root folder; cmd is the VCS base command  # line 966
    debug("Found root folders for SOS|VCS: %s|%s" % (("-" if root is None else root), ("-" if vcs is None else vcs)))  # line 967
    defaults["defaultbranch"] = (lambda _coconut_none_coalesce_item: "default" if _coconut_none_coalesce_item is None else _coconut_none_coalesce_item)(vcsBranches.get(cmd, "trunk"))  # sets dynamic default with SVN fallback  # line 968
    if force_sos or root is not None or (("" if command is None else command))[:2] == "of" or (("" if command is None else command))[:1] in ["h", "v"]:  # in offline mode or just going offline TODO what about git config?  # line 969
        cwd = os.getcwd()  # line 970
        os.chdir(cwd if command[:2] == "of" else (cwd if root is None else root))  # line 971
        parse(root, cwd)  # line 972
    elif force_vcs or cmd is not None:  # online mode - delegate to VCS  # line 973
        info("SOS: Running '%s %s'" % (cmd, " ".join(sys.argv[1:])))  # line 974
        import subprocess  # only required in this section  # line 975
        process = subprocess.Popen([cmd] + sys.argv[1:], shell=False, stdin=subprocess.PIPE, stdout=sys.stdout, stderr=sys.stderr)  # line 976
        inp = ""  # type: str  # line 977
        while True:  # line 978
            so, se = process.communicate(input=inp)  # line 979
            if process.returncode is not None:  # line 980
                break  # line 980
            inp = sys.stdin.read()  # line 981
        if sys.argv[1][:2] == "co" and process.returncode == 0:  # successful commit - assume now in sync again (but leave meta data folder with potential other feature branches behind until "online")  # line 982
            if root is None:  # line 983
                Exit("Cannot determine VCS root folder: Unable to mark repository as synchronized and will show a warning when leaving offline mode")  # line 983
            m = Metadata(root)  # type: Metadata  # line 984
            m.branches[m.branch] = dataCopy(BranchInfo, m.branches[m.branch], inSync=True)  # mark as committed  # line 985
            m.saveBranches()  # line 986
    else:  # line 987
        Exit("No offline repository present, and unable to detect VCS file tree")  # line 987


# Main part
verbose = os.environ.get("DEBUG", "False").lower() == "true" or '--verbose' in sys.argv or '-v' in sys.argv  # imported from utility, and only modified here  # line 991
level = logging.DEBUG if verbose else logging.INFO  # line 992
force_sos = '--sos' in sys.argv  # type: bool  # line 993
force_vcs = '--vcs' in sys.argv  # type: bool  # line 994
_log = Logger(logging.getLogger(__name__))  # line 995
debug, info, warn, error = _log.debug, _log.info, _log.warn, _log.error  # line 995
if __name__ == '__main__':  # line 996
    main()  # line 996
