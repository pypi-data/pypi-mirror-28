...
from .helper    import active_channel, SHARED_DIRECT_EXCHANGE_NAME, SHARED_TOPIC_EXCHANGE_NAME, SHARED_SIGNAL_CONNECTION_LOSS
...
class Driver(object):
    ...
    def _do_publish(self, route, message, options, allowed_retry_count):
        with active_channel(self.url, self._on_connect if not allowed_retry_count else None, self._on_disconnect if not allowed_retry_count else None, self._on_error if not allowed_retry_count else None) as channel:
            try:
