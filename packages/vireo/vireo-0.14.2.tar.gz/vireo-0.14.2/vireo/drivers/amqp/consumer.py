...
from vireo.helper import log, debug_mode_enabled, inspect_object
from vireo.model  import Message, RemoteSignal

from .exception import NoConnectionError
from .helper    import active_connection, active_channel, fill_in_the_blank, debug_mode_enabled, \
                       SHARED_DIRECT_EXCHANGE_NAME, SHARED_TOPIC_EXCHANGE_NAME, \
                       SHARED_SIGNAL_CONNECTION_LOSS
...
class Consumer(threading.Thread):
    ...
    def _listen(self):
        with active_connection(self.url, self._on_connect, self._on_disconnect, self._on_error) as connection:
            ...
            def callback_wrapper(message): #(channel, method_frame, header_frame, body):
                inspect_object(message)
    ...
    def _handle_error_during_consumption(self, message_id, raw_message, error, execution_trace):
        with active_channel(self.url, self._on_connect, self._on_disconnect, self._on_error) as channel:
            error_info = {
    ...
    def _declare_shared_queue(self, channel):
        ...
        exchange_options = dict(
            ...
            type        = self.exchange_options.get('type', 'direct'),
    ...
    def _declare_topic_queue(self, channel):
        ...
        exchange_options = dict(
            ...
            type        = self.exchange_options.get('type', 'topic'),
            passive     = self.exchange_options.get('passive', False),
            durable     = self.exchange_options.get('durable', True),
            auto_delete = self.exchange_options.get('auto_delete', False),
        )
