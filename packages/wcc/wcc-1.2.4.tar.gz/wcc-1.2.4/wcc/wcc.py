...
class Wcc:
    ...
    def getFileInfo(file_url_original):
        ...
        for k in range(1,6):
            ...
            try:
                ...
                if 'content-length' not in resp.headers:
                    file_size = 0
                    print(resp.headers)
                    print("content-length not in headers")
                else:
        ...
        if mime_type == 'image/gif':
            ...
            extension = 'mp4'
        elif mime_type == 'application/octet-stream' and 'k.syasn.com' in file_url_original:
            #如果是下载左左视频的视频文件，它是MP4,显示的却是这样
            mime_type = 'video/mp4'
            extension = 'mp4'
    ...
    def ossUploadUrlFileMd5(file_url,bucket,file_oss_dir,myheaders=None):
            if bucket == "xdua-files":
                oss_url_pfx = "http://xdua-files.oss-cn-qingdao.aliyuncs.com/"
            elif bucket == "wikicivi-files":
                oss_url_pfx = "http://wikicivi-files.oss-cn-qingdao.aliyuncs.com/"
            else:
    ...
    def webget(url):
        ...
        cap["phantomjs.page.settings.resourceTimeout"] = 9000
    ...
    def asmArtbooks(self,www_from,max_ctime,max_tidfrm,incmode,thread_count = 1,tk = 1,asmconfigArray=None):
        ...
        for asmconf in asmconfigArray:
            ...
            while not stop:
                ...
                if len(pagebook)>0 and pagebook_valid == True:
    ...
    def getFurlInfo(file_url,bucket,myheaders=None):
        ...
        if filedatmd5 == None:
            ...
            return None
        fileurlmd5_localpath = bucket_dir+"/"+filedatmd5+"."+file_ext
        ...
        if mime_type == "image/gif":
            ...
            if Wcc.existsFile(bucket,file_gifmp4_osspath) == False:
                if Wcc.uploadFile(file_gifmp4_localpath, file_gifmp4_osspath,bucket) == False:
                    print("Error:动图上传失败 "+file_oss_path)
                    return None
        ...
        if mime_main == "image":
            ...
            im = Image.open(fileurlmd5_localpath).save(webp_localpath, "WEBP")
            if Wcc.existsFile(bucket,webp_localpath) == False:
                if Wcc.uploadFile(webp_localpath, webp_localpath,bucket) == False:
                    print("Error:动图上传失败 "+webp_localpath)
                    return None
            retinfo["webp"]  = oss_url_pfx+webp_localpath
        info = Furlinfo()
        info.url = file_url
    ...
    def getFurlInfoShell(url,ref):
        ...
        if resp_json == None:
            ...
            return None
        """
        {'result': {'webp': 'http://wikicivi-files.oss-cn-qingdao.aliyuncs.com/Images/e59489d6ab90dc774ec03e63b28154d8.webp', 'url': 'http://img.99mm.net/2018/2694/1-se.jpg', 'ossurl': 'http://wikicivi-files.oss-cn-qingdao.aliyuncs.com/Images/96a06c75ca3ff1bb4e3431c4ead5a764.jpg', 'datmd5': '96a06c75ca3ff1bb4e3431c4ead5a764', 'ext': 'jpg', 'w': 800, 'mime': 'image/jpeg', 'urlmd5': 'e59489d6ab90dc774ec03e63b28154d8', 'h': 1180, 's': 152085}, 'reason': 'Success', 'status': 0}
        """
        try:
    ...
    def make_fitem2(farray):
        try:
            ...
            if "http://files.wikicivi.com/" in file_url:
                file_urlmd5_localpath = file_url.replace("http://files.wikicivi.com/","")
                file_datmd5_localpath = file_urlmd5_localpath
            if "http://wikicivi-files.oss-cn-qingdao.aliyuncs.com/" in file_url:
                file_urlmd5_localpath = file_url.replace("http://wikicivi-files.oss-cn-qingdao.aliyuncs.com/","")
                file_datmd5_localpath = file_urlmd5_localpath
            elif "http://files.xdua.org/" in file_url:
                file_urlmd5_localpath = file_url.replace("http://files.xdua.org/","")
                file_datmd5_localpath = file_urlmd5_localpath
            elif "http://xdua-files.oss-cn-qingdao.aliyuncs.com/" in file_url:
                file_urlmd5_localpath = file_url.replace("http://xdua-files.oss-cn-qingdao.aliyuncs.com/","")
                file_datmd5_localpath = file_urlmd5_localpath
            else:
                file_urlmd5 = md5(file_url.encode(encoding='utf_8')).hexdigest()
                #如果src里有暗示文件的存放目录，那么使用这个目录
            ...
            if furlinfo["mime"] == "image/gif":
                assert "givurl" in furlinfo
                farray["mp4"] = furlinfo["givurl"]
                assert "sv" in furlinfo
    ...
    def MakeSetUserMsg(uid_inc,user_info):
        ...
        if "avatar" not in fields and fields['ustr'].startswith("dbn.cbt:"):
            fields['avatar'] = 'http://xdua-files.oss-cn-qingdao.aliyuncs.com/Avatar/dbnlogo.jpg' 
    ...
    def MakeAddUserMsg(user_info):
        ...
        if "avatar" not in user_info and user_info['ustr'].startswith("dbn.cbt:"):
            user_info['avatar'] = 'http://xdua-files.oss-cn-qingdao.aliyuncs.com/Avatar/dbnlogo.jpg' 
