...
from ..lib_interfaces import rook_pb2, ProtobufConfiguration
...
class NamespaceSerializer(object):
    ...
    def _dump_object_namespace(self, namespace, variant, log_errors):
        self._dump_python_object(namespace.obj, variant, 0, namespace.dump_config)
    ...
    def _dump_python_object(self, obj, variant, current_depth, config):
        if current_depth == config.max_depth:
            self._dump_max_depth(obj, variant)
        else:
            self._dump_base_object(obj, variant, current_depth, config)

            if self._is_primitive(obj):
                self._dump_primitive(obj, str(type(obj)), variant, config.max_string)
            elif isinstance(obj, (list, tuple, set, frozenset)):
                self._dump_list(obj, variant, current_depth, config)
            elif isinstance(obj, dict):
                self._dump_dictionary(obj, variant, current_depth, config)
            elif isinstance(obj, BaseException):
                self._dump_exception(obj, variant, current_depth, config)
    ...
    def _dump_base_object(self, obj, variant, current_depth, config):
        ...
        if hasattr(obj, '__dict__'):
            for key, value in six.iteritems(obj.__dict__):
                ...
                attribute.name = key
                self._dump_python_object(value, attribute.value, current_depth+1, config)
    ...
    def _dump_list(self, collection, variant, current_depth, config):
        ...
        if current_depth < config.max_collection_dump:
            ...
            for index, item in enumerate(collection):
                ...
                self._dump_python_object(item, variant_item, current_depth+1, config)

    def _dump_dictionary(self, collection, variant, current_depth, config):
        ...
        if current_depth < config.max_collection_dump:
            ...
            for key, value in six.iteritems(collection):
                ...
                self._dump_python_object(key, pair.first, current_depth+1, config)
                self._dump_python_object(value, pair.second, current_depth+1, config)

    def _dump_exception(self, exc, variant, current_depth, config):
        variant.variant_type = variant.VARIANT_OBJECT

        args = variant.attributes.add()
        args.name = "args"
        self._dump_python_object(exc.args, args.value, current_depth+1, config)
