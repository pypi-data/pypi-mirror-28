...
class WcclsDesktop:
	...
	def Login(self, login, password):
		...
		g = Grab()
		g.setup(debug=self.debug, log_dir="logging", debug_post=True, verbose_logging=True)
		info(f"reuse_cookies: {g.config['reuse_cookies']}")
		loginUrl = self.host + "/polaris/logon.aspx"
		loginUrl = loginUrl + "?src=https%3a%2f%2fcatalog.wccls.org%2fpolaris%2fpatronaccount%2fdefault.aspx%3fctx%3d1.1033.0.0.6&ctx=1.1033.0.0.6"
		g.go(loginUrl)

		g.cookies.save_to_file(join("logging", "cookies-1.json"))

		g.doc.set_input("ctl00$BodyMainContent$textboxBarcodeUsername", login)
		g.doc.set_input("ctl00$BodyMainContent$textboxPassword", password)
		g.doc.submit()

		g.cookies.save_to_file(join("logging", "cookies-2.json"))

		info(f"reuse_cookies: {g.config['reuse_cookies']}")

		if g.doc("//input[@value]").exists():
			error("Login failed")
			assert False, "Login failed"
		elif g.doc("//a[(Log Out)]").exists():
			info("WORKED!!!")
		else:
			assert False

	def LoginOld(self, login, password):
		...
		debug(viewState)
		debug(viewStateGenerator)
		debug(eventValidation)
	...
	def DebugPath(self, filename):
		from tempfile import gettempdir
		return join(gettempdir(), "log", filename) 

	def SaveDebugFile(self, filename, content):
		if not self.debug:
			return

		with open(self.DebugPath(filename), "wb") as theFile:
