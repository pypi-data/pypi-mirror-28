...
RNA_TO_AA = {v: k for k, vs in AA_TO_RNA.items() for v in vs}
DNA_TO_AA = {k.replace('U', 'T'): v for k, v in RNA_TO_AA.items()}
AA_TO_DNA = {k: [v.replace('U', 'T') for v in vs] for k, vs in AA_TO_RNA.items()}
...
def _getGzipData(ftp_conn, gz_fn):
    buff = io.BytesIO()
...
def _getOrganismRec(raw_data, organism_id):
...
def getOrganismFrequencies(organism_id, organism_class):
    ftp_conn = _getFtpConn('db_dir')
...
def updateCodonFreqDataset(additional_organisms=None):
