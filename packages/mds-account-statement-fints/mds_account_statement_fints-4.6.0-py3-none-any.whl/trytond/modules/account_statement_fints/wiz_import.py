...
class ImportByFinTSWizard(Wizard):
    ...
    def save_statement(self, statement, accnm, journal, datestart, dateend, amount):
        ...
        for i in statement:
            ...
            stm_date = i.data.get('entry_date', None)
            ...
            stm_description = '%s: %s' % (
                    remote_name,
                    i.data.get('purpose', i.data.get('transaction_details', '-/-')).replace('\n', ' ')
                )
            ...
            stm_obj.fints_data = self.get_fints_data(i.data)
