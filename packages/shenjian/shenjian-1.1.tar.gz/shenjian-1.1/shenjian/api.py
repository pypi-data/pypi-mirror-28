...
class Service(_Base):
    ...
    def get_crawler_list(self, page=1, page_size=50, deleted=False):
        return self._request("crawler/list", method='post', data={'page': page, 'page_size': page_size,'deleted':deleted})
...
class Crawler(_Base):
    ...
    def start(self, node=1, frequency=False, clear_data=False):
        data = {'node': node}
        if frequency:
