import pretty_output, os

from collections import OrderedDict

from kooki.jars import search_file, search_recipe, search_jar, get_search_paths
...
def process_document(document_origin):
    ...
    for extension_name, extension_path in extensions_raw.items():
        ...
        for s in splitted[:-1]:
            if not s in current:
                current[s] = Metadata()
            current = current[s]
        current[splitted[-1]] = caller(extension_path)
    ...
    if file_full_path:
        ...
        raise KookiException('''Cannot find template '{}' in the following directory:\n{}'''.format(document.template, directories_str))
    ...
    for metadata in document.metadata:
        ...
        if file_full_path:
            ...
            output_search_finish()
            raise KookiException('Missing metadata')
    ...
    if not recipe_full_path or not os.path.isdir(recipe_full_path):
        raise KookiException('Bad recipe: cannot find recipe \'{}\''.format(recipe))

    import imp
    try:
        fp, pathname, description = imp.find_module('recipe', [recipe_full_path])
    except ImportError:
        print('Bad recipe: cannot find recipe')
        return (None, None)

    recipe = imp.load_module('recipe', fp, pathname, description)
    recipe.recipe(document, extensions)
