...
class DocManager(DocManagerBase):
    ...
    def create_update_id_index_if_not(self, namespace):
        if namespace in self.nsIndexedUpdateId:
            return

        db, coll = self._db_and_collection(namespace)
        self.mongo[db][coll].create_index([('updateId', 1)])
        self.nsIndexedUpdateId.add(namespace)
    ...
    def update(self, document_id, update_spec, namespace, timestamp):
        ...
        self.create_update_id_index_if_not(namespace)
