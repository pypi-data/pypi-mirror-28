...
from . import helpers, escher_runner
...
def run(worker, script, work_directory, args):
    ...
    if config_rc.scripts and script in config_rc.scripts:
        # todo: fill-in environment variables.
        ser_envs = escher_runner.envs_serializer(config_rc.envs)
        shell, _s = True, config_rc.scripts[script].format(envs=ser_envs).strip()
    ...
    if worker == "local":
        ...
        if helpers.is_script(_s):
            ...
            # config_rc.envs here is redundant?
            my_env = environ.copy().update(config_rc.envs)
