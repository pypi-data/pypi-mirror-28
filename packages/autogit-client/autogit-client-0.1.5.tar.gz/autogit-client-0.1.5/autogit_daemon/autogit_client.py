...
SERVER_URL = "https://dashboard.autogit.io"
...
class AutoGitSetup(Daemon):
    ...
    def set_server(self, custom_server):
        if custom_server:
    ...
    def get_disk_size(self):
        return os.popen('df ' + self.DEFAULT_PATH + ' -h | grep /dev/ |awk \'{ print $2 }\'').read()

    def get_free_space(self):
        return os.popen('df ' + self.DEFAULT_PATH + ' -h | grep /dev/ |awk \'{ print $4 }\'').read()
        ...
        if self.checkPath() and self.current_data != self.info:
            ...
            post = requests.post("%s/worker/log/" % self.autogit_server,
                                 data=dict(logs=json.dumps(logs), repository=self.info['repo'],
                                           ...
                                           disk_size=self.get_disk_size(), disk_free_space=self.get_free_space(), 
