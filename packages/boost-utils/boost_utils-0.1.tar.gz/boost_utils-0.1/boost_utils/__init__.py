...
def pickler(some_object, filename, compress=False, compression_level=-1):
    ...
    log("Made a pickle jar on file:{0}".format(filename))
...
def check_file(name, size):
    """checks if a file exists and has minimum size - returns 0 when check fails """
    trace('checking file {0} with size {1}'.format(name, size))
