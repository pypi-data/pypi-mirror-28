...
class TLSSysLogHandler(logging.Handler):
    ...
    def __init__(self, address, timeout=3, facility=LOG_USER, ssl_kwargs={}):
        ...
        self.thread_should_terminate = threading.Event()

        self.sender_thread = TCPLoggingThread(address, timeout, self.queue, self.thread_should_terminate, ssl_kwargs)
    ...
    def close(self):
        ...
        try:
            ...
            self.sender_thread.join()  # Wait for the thread to terminate
            super().close()
        finally:
            self.release()
