...
def __wrap_v2( text: str, width: int ) -> Iterator[str]:
    """
    ...
    while i < l:
        ...
        if c == " " or c == "\n":
            if (i - start_line) < width:
                if c == " ":
                    # Still enough space, continue
                    ls = i
                else:
                    # Still enough space, print now
                    yield text[start_line:i]
                    start_line = i + 1
            else:
                # No more room
                # Backtrack to ls
...
def wrap( text, width = 70 ) -> Iterator[str]:
    ...
    :return:            Wrapped text
    """
    return __wrap_v2( text, width )


def __wrap_v1( text, width = 70, get_length = len ) -> Iterator[str]:
    """
    Old wrapping algorithm based on Regex.
    """
    if width <= 0:
        for line in str( text ).split( "\n" ):
            yield line
...
def format_array( a: Iterable, join = ", ", sort=False ) -> str:
    if sort:
        a = sorted(str(x) for x in a)
        
    return join.join( str( x ) for x in a )
