...
class SpotifyClient(AuthMixin, EmailMixin):
        ...
        try:
            ...
            if response.status_code == 429 or (response.status_code >= 500 and response.status_code < 600):
                raise SpotifyRateLimitException(
                    response=response, retry_after=int(response.headers.get('Retry-After', 0))
                )
    ...
    def artist_albums(self, artist_id, album_type=None, country=None, limit=20, offset=0):
        ...
        id = self._get_artist_id(artist_id)
        return self._get(
            API.ARTIST_ALBUMS.value.format(id=id), album_type=album_type, country=country, limit=limit, offset=offset
        )
    ...
    def user_playlist_add_tracks(self, user, playlist_id, tracks, position=None):
        ...
        return self._post(
            API.PLAYLIST_TRACKS.value.format(user_id=user, playlist_id=id), payload=payload, position=position
        )
    ...
    def user_playlist_replace_tracks(self, user, playlist_id, tracks):
        ...
        track_uris = map(self._get_track_uri, tracks)
        payload = {"uris": list(track_uris)}
        return self._post(API.PLAYLIST_TRACKS.value.format(user_id=user, playlist_id=id), payload=payload)
    ...
    def user_playlist_reorder_tracks(
        ...
        return self._put(API.PLAYLIST_TRACKS.value.format(user_id=user, playlist_id=id), payload=payload)
    ...
    def user_playlist_remove_all_occurrences_of_tracks(self, user, playlist_id, tracks, snapshot_id=None):
        ...
        if snapshot_id:
            payload["snapshot_id"] = snapshot_id
        return self._delete(API.PLAYLIST_TRACKS.value.format(user_id=user, playlist_id=id), payload=payload)
    ...
    def _get_id(self, type, result):
        if isinstance(result, str):
            ...
            if len(fields) >= 3:
                if type != fields[-2]:
                    logger.warning('Expected id of type %s but found type %s %s', type, fields[-2], result)
                return fields[-1]
            fields = result.split('/')
            if len(fields) >= 3:
                itype = fields[-2]
                if type != itype:
                    logger.warning('Expected id of type %s but found type %s %s', type, itype, result)
                return fields[-1]
