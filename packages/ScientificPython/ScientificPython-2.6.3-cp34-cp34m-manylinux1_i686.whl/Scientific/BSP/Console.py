...
def synchronize():
    ...
    while not done:
        put(ok, list(range(processorID)) + list(range(processorID+1, numberOfProcessors)))
        status = sync()
...
class ParallelConsole(InteractiveConsole):
    ...
    def push(self, line):
        ...
        source = "\n".join(self.buffer)
        put(source, list(range(1, numberOfProcessors)))
        sync()
    ...
    def mainloop(self):
        ...
        while 1:
            try:
                ...
                banner = ""
        put(None, list(range(1, numberOfProcessors)))
        sync()
