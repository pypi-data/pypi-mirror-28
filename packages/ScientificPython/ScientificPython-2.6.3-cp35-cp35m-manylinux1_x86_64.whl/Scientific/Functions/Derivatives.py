...
class DerivVar:
    ...
    def toOrder(self, order):
        ...
        if order == 0:
            return self.value
        return DerivVar(self.value, list(map(lambda x, o=order-1: x.toOrder(o),
                                        self.deriv)), order)
    ...
    def __getitem__(self, order):
        ...
        if order == 0:
            ...
            return list(map(lambda d, i=order-1: _indexDeriv(d,i), self.deriv))

    def __repr__(self):
        return repr(tuple(map(lambda n, x=self: x[n], list(range(self.order+1)))))

    def __str__(self):
        return str(tuple(map(lambda n, x=self: x[n], list(range(self.order+1)))))
    ...
    def __neg__(self):
        return DerivVar(-self.value,[-a for a in self.deriv], self.order)
    ...
    def __mul__(self, other):
        ...
        return DerivVar(self.value*other.value,
                        _mapderiv(lambda a,b: a+b,
                                  list(map(lambda x,f=o1: f*x, self.deriv)),
                                  list(map(lambda x,f=s1: f*x, other.deriv))),
                        max(self.order, other.order))
    __rmul__ = __mul__
    ...
    def __div__(self, other):
        ...
        return DerivVar(_toFloat(self.value)/other.value,
                        _mapderiv(lambda a,b: a-b,
                                  list(map(lambda x,f=o1i: x*f, self.deriv)),
                                  list(map(lambda x,f=s1*pow(o1i, 2): f*x,
                                      other.deriv))),
                                  max(self.order, other.order))
    ...
    def __pow__(self, other, z=None):
        ...
        if len(other.deriv) > 0:
            ...
            return DerivVar(pow(self.value, other.value),
                            list(map(lambda x,f=ps1: f*x, self.deriv)),
                            max(self.order, other.order))
    ...
    def _mathfunc(self, f, d):
        ...
        return DerivVar(f(self.value), list(map(lambda x, f=fd: f*x, self.deriv)),
                        self.order)
    ...
    def arctan2(self, other):
        ...
        return DerivVar(Numeric.arctan2(self.value, other.value),
                        _mapderiv(lambda a,b: a-b,
                                  list(map(lambda x,f=o1: x*f, self.deriv)),
                                  list(map(lambda x,f=s1: x*f, other.deriv))),
                                  max(self.order, other.order))
