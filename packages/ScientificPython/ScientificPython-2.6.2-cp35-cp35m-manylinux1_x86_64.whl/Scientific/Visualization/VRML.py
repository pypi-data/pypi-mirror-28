...
class SceneFile:
    ...
    def uniqueName(self):
        ...
        return 'i' + repr(self.name_counter)
...
class ShapeObject(VRMLObject):
    ...
    def writeToFile(self, file):
        ...
        if trans_flag and rot_flag:
            file.writeString('Transform{translation ' + repr(vector[0]) + ' ' + \
                             repr(vector[1]) + ' ' + repr(vector[2]) +  \
                             ' rotation ' + repr(axis[0]) + ' ' + repr(axis[1]) +
                             ' ' + repr(axis[2]) + ' ' + repr(angle) + '}\n')
        elif trans_flag:
            file.writeString('Translation{translation ' + repr(vector[0]) + ' ' + \
                             repr(vector[1]) + ' ' + repr(vector[2]) +  '}\n')
        elif rot_flag:
            file.writeString('Rotation{rotation ' + repr(axis[0]) + ' ' + \
                             repr(axis[1]) + ' ' + repr(axis[2]) + ' ' + \
                             repr(angle) + '}\n')
...
class Sphere(ShapeObject):
    ...
    def writeSpecification(self, file):
        file.writeString('Sphere{radius ' + repr(self.radius) + '}\n')
...
class Cube(ShapeObject):
    ...
    def writeSpecification(self, file):
        file.writeString('Cube{width ' + repr(self.edge) + \
                         ' height ' + repr(self.edge) + \
                         ' depth ' + repr(self.edge) + '}\n')
...
class Cylinder(LinearOrientedObject):
    ...
    def writeSpecification(self, file):
        ...
        file.writeString(' radius ' + repr(self.radius) + \
...
class Cone(LinearOrientedObject):
    ...
    def writeSpecification(self, file):
        ...
        file.writeString(' bottomRadius ' + repr(self.radius) + \
                         ' height ' + repr(self.height) + '}\n')
...
class Line(ShapeObject):
    ...
    def writeSpecification(self, file):
        file.writeString('Coordinate3{point [' + \
                         repr(self.points[0][0]) + ' ' + repr(self.points[0][1]) + \
                         ' ' + repr(self.points[0][2]) + ',' + \
                         repr(self.points[1][0]) + ' ' + repr(self.points[1][1]) + \
                         ' ' + repr(self.points[1][2])  + \
...
class PolyLines(ShapeObject):
    ...
    def writeSpecification(self, file):
        ...
        for p in self.points:
            s = s + repr(p[0]) + ' ' + repr(p[1]) + ' ' + repr(p[2]) + ','
        file.writeString(s[:-1] + ']}IndexedLineSet{coordIndex')
        file.writeString(repr(list(range(len(self.points)))+[-1]) + '}\n')
...
class Polygons(ShapeObject):
    ...
    def writeSpecification(self, file):
        ...
        for v in self.points[:-1]:
            file.writeString(repr(v[0]) + ' ' + repr(v[1]) + ' ' + repr(v[2]) + ',')
        v = self.points[-1]
        file.writeString(repr(v[0]) + ' ' + repr(v[1]) + ' ' + repr(v[2]) + \
                         ']}IndexedFaceSet{coordIndex[')
        for polygon in self.index_lists:
            for index in polygon:
                file.writeString(repr(index)+',')
            file.writeString('-1,')
        file.writeString(']}\n')
...
class Arrow(Group):
    ...
    def __init__(self, point1, point2, radius, **attr):
        ...
        cone = Cone(*(point2, junction, 0.75*cone_height), **attr)
        objects = [cone]
        if cylinder_height > 0.005*radius:
            cylinder = Cylinder(*(point1, junction, radius), **attr)
