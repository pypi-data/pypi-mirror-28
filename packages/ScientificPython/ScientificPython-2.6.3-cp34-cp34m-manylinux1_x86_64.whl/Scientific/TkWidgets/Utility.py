...
import tkinter, tkinter.dialog, tkinter.filedialog
...
class FilenameEntry(tkinter.Frame):
    ...
    def __init__(self, master, text, pattern = '*', must_exist = True,
        ...
        entry = tkinter.Entry(*(self,), **newattr)
        entry.pack(side=tkinter.RIGHT, expand=1, fill=tkinter.X)
        entry.icursor("end")

    def browse(self):
        if self.must_exist:
            file = tkinter.filedialog.LoadFileDialog(self).go(pattern=self.pattern)
        else:
            file = tkinter.filedialog.SaveFileDialog(self).go(pattern=self.pattern)
        if file:
    ...
    def get(self):
        ...
        if self.must_exist and not os.path.exists(filename):
            tkinter.dialog.Dialog(self, title='File not found',
...
class FloatEntry(tkinter.Frame):
    ...
    def __init__(self, master, text, init = None, lower=None, upper=None,
        ...
        self.entry = tkinter.Entry(*(self,), **newattr)
    ...
    def get(self):
        ...
        try:
            ...
            tkinter.dialog.Dialog(self, title='Illegal value',
        ...
        if range_check != 0:
            ...
            if range_check < 0:
                text = text + 'smaller than ' + repr(self.lower) + '.'
            else:
                text = text + 'larger than ' + repr(self.upper) + '.'
            tkinter.dialog.Dialog(self, title='Value out of range', text=text,
                          bitmap='warning', default=0,
                          strings = ('Cancel',))
            raise ValueError
        return value
