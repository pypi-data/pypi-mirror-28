...
class TensorField(Interpolation.InterpolatingFunction):
    ...
    def __init__(self, rank, axes, values, default = None,
        ...
        if check:
            ...
            if len(values.shape) != 3 + rank:
                raise ValueError('Values must have rank ' + repr(rank))
    ...
    def __call__(self, *points):
        ...
        value = Interpolation.InterpolatingFunction.__call__(*(self, ) + points)
        if self.rank == 0:
            return value
        elif self.rank == 1:
            return Vector(value)
    ...
    def derivative(self, variable):
        ...
        if period is None:
            ...
            u = N.take(self.values, list(range(2, len(self.axes[variable])))+[0, 1],
                       axis=variable)
            l = self.values
