...
class DataSet(object):
    ...
    def _setupSimilarities(self, items, similarities,
        ...
        elif isinstance(similarities, collections.Callable):
            for i in range(self.nitems):
    ...
    def _setupRIndices(self):
        ...
        for i in range(self.nsimilarities):
            indices.append([])
        for i in range(self.nitems):
            for k1, index1 in list(self.index[i].items()):
                for k2, index2 in list(self.index[i].items()):
                    if k2 != k1:
    ...
    def _setupAIndices1(self):
        ...
        for i in range(self.nitems):
            for k, index in list(self.index[i].items()):
