...
class Polynomial:
    ...
    def __add__(self, other):
        ...
        index = tuple([slice(0, d) for d in self.coeff.shape] + \
                      (dim-self.dim)*[0])
        coeff1[index] = self.coeff
        coeff2 = Numeric.zeros_st(shape, other.coeff)
        index = tuple([slice(0, d) for d in other.coeff.shape] + \
                      (dim-other.dim)*[0])
    ...
    def derivative(self, variable=0):
        ...
        if n == 1:
            return Polynomial(Numeric.zeros(*self.dim*(1,)))
...
def _fitPolynomial(order, points, values):
    ...
    if type(order) != type(()):
        order = (order,)
    order = tuple([n+1 for n in order])
