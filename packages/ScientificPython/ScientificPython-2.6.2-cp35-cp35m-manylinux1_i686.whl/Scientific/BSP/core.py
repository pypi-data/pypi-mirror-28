...
import pickle, operator, sys, types
...
if world is not None:
    ...
    _type_tags_i = {}
    for key, value in list(_type_tags.items()):
    ...
    def _send(obj, destinations):
        ...
        if type(obj) is N.arraytype:
            ...
            if _debug_flag:
                ...
                sys.stdout.flush()
            if tag == 2:
                send_data = pickle.dumps(send_data, 1)
            else:
    ...
    def _receive(source, tag):
        if tag == 2:
            if _debug_flag:
                ...
                sys.stdout.flush()
            data = pickle.loads(world.receiveString(source, tag)[0])
            return data
        else:
            if _debug_flag:
            ...
            if typecode is None:
                raise ValueError("Invalid tag " + repr(tag))
...
class ParValue(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        for a in args:
            ...
            params.append(p)
        kw = {}
        for key, data in list(kwargs.items()):
            p, v = _getValue(data)
            kw[key] = p
            valid = valid and v
        if valid:
    ...
    def broadcast(self, from_pid=0):
        ...
        if processorID == from_pid:
            if self.valid:
                put(self.value, list(range(numberOfProcessors)))
            else:
    ...
    def accumulate(self, operator, zero):
        ...
        if self.valid:
            ...
            data = ParValue(zero)
        data = data.get(ParValue(list(range(processorID+1))))
    ...
    def alltrue(self):
        ...
        if processorID == 0:
            ...
            put(combined, list(range(numberOfProcessors)))
        return sync()[0]
    ...
    def map(self, function):
        ...
        return ParValue(list(map(function, self.value)))
...
class ParMessages(ParValue):
    ...
    def processorIds(self):
        ...
        @rtype: L{ParValue}
        """
        return ParValue([x[0] for x in self.value])
    ...
    def data(self):
        ...
        @rtype: L{ParValue}
        """
        return ParValue([x[1] for x in self.value])
...
class ParTuple(ParValue):
    ...
    def __init__(self, *args):
        ...
        @type args: C{tuple}
        """
        self.value = [pv.value for pv in args]
        self.valid = reduce(operator.and_, [pv.valid for pv in args])
...
class ParClass(object):
    ...
    def __init__(self, local_class):
        ...
        class _Wrapper(object):
            ...
            is_parvalue = 1
            def __repr__(self):
                if '__repr__' in self.attributes:
    ...
    def _collectAttributes1(self, klass, attrib_dict):
        for key in list(klass.__dict__.keys()):
...
class ParBase(object):
    ...
    def broadcast(self, data, from_pid=0):
        ...
        """
        if processorID == from_pid:
            put(data, list(range(numberOfProcessors)))
