...
class ForumsScenario(BaseScenario):
    ...
    def execute_scenario(self):
        self.collect_forums(self.website_url)
        pass

    def collect_forums(self, url):
        collect_forums_urls_step = URLCollectorStep([url], '-', self.website_url, self.forum_xpath)
        forum_urls = collect_forums_urls_step.execute_step()
        for forum_url in forum_urls:
            self.collect_forums(forum_url)
        collect_threads_urls_step = URLCollectorStep([url], '-', self.website_url, self.thread_xpath)
        thread_urls = collect_threads_urls_step.execute_step()
        collect_posts_step = DieselElcatStep(thread_urls, '=', self.post_xpath, self.post_info_xpaths)
        res = collect_posts_step.execute_step()
...
class DieselElcatStep(MultipleExtractByXpathStep):

    def get_next_page_url(self):
        next_page_xpath = './/li[@class="next"]/a/@href'
        next_page_url = self.tree.xpath(next_page_xpath)
