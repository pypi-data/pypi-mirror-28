...
from keyup.oscodes import exit_codes
from keyup.help_menu import menu_body
from keyup.statics import local_config
from keyup.script_utils import stdout_message, convert_dt_time, debug_mode
from keyup import configuration, logd, __version__
...
def authenticated(profile):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidClientTokenId':
            ...
            logger.exception('%s: Unknown Boto3 problem. Error: %s' %
...
def parse_awscli():
    ...
    try:
        if os.path.exists(awscli_file):
            # parse config
            config.read(awscli_file)
        else:
            logger.info(
                'awscli credentials file [%s] not found. Abort' %
                awscli_file
            )
            raise OSError
    except Exception as e:
...
def precheck():
    ...
    return False
    #
    # NOTE: CONFIG SCRIPT @ STARTUP DISABLED -- REVISIT
    #else:
    #    # no pre-existing config files
    #    r = configuration.init(cfg_path)
    #    return r
    ...
    if r['ResponseMetadata']['HTTPStatusCode'] == 200:
        ...
        if quiet:
            ...
            if stage:   # active rotation
                title = (Colors.BOLD + '\n    Access Key List:  ' + Colors.RESET + stage + '\n\n')
            else:
...
class SetLogging():
    ...
    Returns:
        TYPE: bool, Success | Failure
    """
    def __init__(mode, disable=False):
        global logger
...
def main(operation, profile, auto, debug, user_name=''):
    ...
    try:
        ...
        # -- 2 keysets exist --------------------------------------------------
            ...
            if result:
                # assemble key
                parsed, output_file, access_key, secret_key = configure_keyset(new_keys, profile, surrogate='')
                # log success
                logger.info(
...
def package_version():
    ...
    print(
        ...
        Colors.BOLD + __version__ + Colors.RESET
        )
    print(
        Colors.LTGRAY +
        '\n  Copyright Blake Huber.  Distributed under GPL v3.\n' + Colors.RESET
        )
    sys.exit(exit_codes['EX_OK']['Code'])
...
def option_configure(debug=False, path=None):
    ...
    if os.path.isfile(path):
        ...
        debug_mode(
            'local config file does not yet exist ',
            {'CONFIG_PATH': path}, debug, halt=True
            )
...
def init_cli():
    ...
    if len(sys.argv) == 1:
        ...
        r = option_configure(local_config['PROJECT']['CONFIG_PATH'], args.debug)
        return r
    else:
