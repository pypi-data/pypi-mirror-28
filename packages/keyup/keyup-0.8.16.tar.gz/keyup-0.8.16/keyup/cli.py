...
def authenticated(profile):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidClientTokenId':
            ...
            logger.info(
                '%s: Expired temporary credentials detected for profile user (%s). Exit. [Code: %d]'
...
def clean_config():
    ...
    Args:
        ...
        :config (configParser obj): GLOBAL object representing parsed awscli
         credentials file
    Returns:
        Success (clean config) | Failure, TYPE (bool)
        ...
        if 'aws_security_token' in parsed_config[profile].keys():
            ...
            if authenticated(profile):
             ...
             Key refresh prohibited. Exit (Code: %d)
                ...
                logger.info('Status of temporary credentials is: ACTIVE.' % profile)
