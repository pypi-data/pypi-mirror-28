...
class SharingDefinition(object):
    ...
    def __init__(self, uid, object_type, pub_access, usergroup_accesses=list()):
        self.uid = uid
    ...
    def __str__(self):
        return u"{} {} {} {}".format(self.object_type, self.uid, self.public_access, ' / '.join([json.dumps(x.to_json()) for x in self.usergroup_accesses]))
    ...
    for i, obj in enumerate(data[object_type], 1):
        ...
        if args.keep:
            ...
            if obj.get('userGroupAccesses'):
                existing.usergroup_accesses = list(set(UserGroupAccess(uid=x['id'], access=x['access']) for x in
