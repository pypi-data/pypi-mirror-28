...
def _get_spec(test_api_path=None):
    ...
    self.test_api_path is a path to the test dss path if the functionality is being tested.
...
def _make_name(http_method, path_split):
    """Name an endpoint."""
    # If the dss needs file/write/{generic} functionality, will become put-file-write
    name = [http_method]
...
def generate_python_bindings(test_api_path=None):
    """
    Generate classes for each endpoint that allow for easy parser integration and dss endpoint interaction.

    :param test_api_path: The swagger dss specification used to generate parser. If path is None, use base endpoint.
    """
    ...
    # Write the initial dss functions to dss init file because composite_commands rely on these.
    ...
    added_imports = ['"""This file is autogenerated according to HCA dss spec. Don\'t modify."""']
