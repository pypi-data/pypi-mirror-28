...
class SoundcloudAPI:
    ...
    def search(self, query, limit=10, offset=0, *, raw=False):
        ...
        results = get_obj_from(url + urlencode(params))
        if not raw:
            tracks_and_playlists = []
            for thing in results['collection']:
                if thing['kind'] == 'track':
                    obj = Track(obj=thing, client=self)
                elif thing['kind'] is 'playlist':
                    obj = Playlist(obj=thing, client=self)
                else:
                    continue
                tracks_and_playlists.append(obj)
            return tracks_and_playlists
        return results
