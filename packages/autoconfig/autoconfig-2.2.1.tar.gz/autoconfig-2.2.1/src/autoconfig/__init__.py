...
class _StdoutWrapper(object):
    ...
    statements.  This is useful if you only use print statements for
...
def _setup_logging(p):
    ...
    if console == 'systemd':
        ...
        try:
            # You can supply your own colors in your logging section
            # with [logging] color_<level>=setting
            # I use this with a Solarized theme on macOS:
            #
            # color_debug=bg_cyan
            # color_warning=red
            # color_error=bg_red
    ...
    if 'colorlog' in sys.modules:
        val = d.get('color_print')
        if val:
            from colorlog.escape_codes import parse_colors
            esc = parse_colors(val)
    sys.stdout = _StdoutWrapper(esc)
