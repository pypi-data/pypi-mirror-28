...
class UserFactor(object):
    ...
    def get_public_uploaded_factor(self):
        result = self._get_uploaded_factor()
        return filter(lambda item: True if item['pool_name'] == 'public' else False, result)

    def submit_public_factor(self, factor_name, chinese_name, chinese_description, category, factor_data_file_path,
                       function_list="winsorize,neutralize,standardize", universe="A"):
        """
        创建一个新的海量因子，并上传因子数据，触发因子报告计算
        ...
        """
        return self._submit_factor(factor_name, chinese_name, chinese_description, category, factor_data_file_path,
                                   function_list, universe, factor_type='public')
    ...
    def update_public_factor_data(self, factor_name, factor_data_file_path):
        """
        更新海量因子数据，并触发因子报告计算
        :param factor_name: 因子名
        ...
        """
        return self._update_factor_data(factor_name, factor_data_file_path, factor_type='public')
