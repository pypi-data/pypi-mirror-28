...
def create_dataset(ctx, account_uid, home):
    ...
    url = '{}/datasets'.format(conn.URL, account_uid)
    r = conn.get(url)
    ...
    dataset = None
    if Dataset.exists_remote(dataset_uid, remote_list):
...
def datasets_clone(ctx, path, directory, include, exclude):
    ...
    r = conn.get('{}/datasets'.format(conn.URL, account_uid))
    if r.status_code == 200:
        remote_list = r.json()
    else:
        print('Error: {}'.format(r.status_code))
    ...
    if not Dataset.exists_remote(dataset_uid, remote_list):
        click.echo('There is no dataset {}/{} on the server'.format(account_uid, dataset_uid))
        return
    ...
    # git clone
    git = GitWrapper()
    if git.lfs_clone(home, account_uid, dataset_uid, ext='_dataset', include=include, exclude=exclude) == 0:
...
def datasets_download(ctx, path, directory):
    ...
    datasets_clone(ctx, path, '.onepanel_download', include='*', exclude='')
