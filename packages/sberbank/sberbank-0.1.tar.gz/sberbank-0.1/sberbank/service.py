...
class BankService:
    ...
    def pay(self, amount, success_url=None, **kwargs):
        ...
        page_view = kwargs.get('page_view', self.merchant.get('page_view', self.__default_page_view))
        ...
        try:
            r = requests.post('{}/register.do'.format(self.__default_gateway_address), data=data)
        ...
        if r.status_code != 200:
            payment.status = Status.FAILED
            payment.save()
            log = BankLog(request_type=LogType.CREATE, bank_id=payment.bank_id, payment_id=payment.uid, response_text=r.text)
            log.save()
        ...
        if response.get('errorCode') and response.get('errorCode') != 200:
            ...
            payment.status = Status.FAILED
            payment.save()
            log = BankLog(request_type=LogType.CREATE, bank_id=payment.bank_id, payment_id=payment.uid, response_json=response)
            log.save()
            raise ProcessingException(payment.uid)
    ...
    def get_status(self, payment_id):
        ...
        if payment.status == Status.PENDING:
            ...
            try:
                r = requests.post('{}/getOrderStatus.do'.format(self.__default_gateway_address), data)
            except (requests.ConnectTimeout, requests.ConnectionError, requests.HTTPError):
                payment.status = Status.FAILED
                payment.save()
                raise NetworkException(payment.uid)
            ...
            try:
                ...
                payment.status = Status.FAILED
                payment.save()
                log = BankLog(request_type=LogType.CHECK_STATUS, bank_id=payment.bank_id, payment_id=payment.uid, response_text=r.text)
                log.save()
                raise ProcessingException(payment.uid)

            log = BankLog(request_type=LogType.CHECK_STATUS, bank_id=payment.bank_id, payment_id=payment.uid, response_json=response)
            log.save()
