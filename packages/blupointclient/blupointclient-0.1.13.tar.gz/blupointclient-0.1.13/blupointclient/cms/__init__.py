...
class components_base(object):
    ...
    async def _post(self, url, body, headers=None, api='management'):
        if api == 'delivery':
            api = self.DELIVERY_API

        _body = json.dumps(body, default=json_helpers.bson_to_json)

        url = urllib.parse.quote(url, safe=':?&=/')

        url = "{0}/domains/{1}{2}".format(api, self.DOMAIN_ID, url)
    ...
    def prepare_delivery_endpoint(self, module, **kwargs ):
        query_string = "?"
        keys = ['method', 'limit', 'skip', 'sort', 'select', 'types', 'paths', 'q']
        for key in keys:
            if kwargs.get("method", ''):
                query_string += key + '=' + kwargs.get(key, '')
