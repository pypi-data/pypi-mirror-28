...
cjk_magnify_re = re.compile('(?:{}|⋏)+'.format(cjkur.regex_string()))
...
class HtmlGenerator:
    ...
    def html_item_links_substitutions(self, tag):
    ...
    def generate_association_html(self, ass, anki=False):
        htmlsub = self.html_substitutions
        def callback(e):
            if isinstance(e, MnemonicRef):
                ...
                EL.extend_with_children_of(e_a1, htmlsub(e.item))
                EL.extend_with_children_of(e_a2, htmlsub(display_label))
                return ['(', e_a1, ')', e_a2]
            else:
                return EL.unwrap(htmlsub(e))
...
'''
    ...
    def add_extra_item_info(self, item, body):
        ...
        self.html_item_links_substitutions(div)
    ...
    def generate_items_page(self, items):
        ...
        for item in items:
            ...
            EL.extend_with_children_of(h3, htmlsub(item))
            ...
            for label, lassocs in sorted(ms.i2l2a[item].items()):
                ...
                h3 = ET.SubElement(body, 'h3')
                EL.extend_with_children_of(h3, htmlsub(item+': '+label))
    ...
    def write_anki_files(self):
        ...
        media_list.append(css_fn)
        with open(os.path.join(
                self.media_output_directory, css_fn), 'wt') as handle:
        ...
        for item_group, items in tqdm_(self.item_group_dict.items()):
            ...
            with open(os.path.join(
                    self.media_output_directory, filename), 'wt') as handle:
                handle.write(self.generate_items_page(items))

    def write_anki_csv(self, file):
        def _html(s):
            return ET.tostring(self.html_substitutions(s), 'unicode')
