...
TGT_DIR = './targets'
if not os.path.exists(TGT_DIR): os.mkdir(TGT_DIR)

INITAL_WORDS = './initial_words.txt'
if not os.path.exists(INITAL_WORDS):
    with open(INITAL_WORDS, 'w', encoding='utf-8') as f:
        f.write(' ')
...
class Embedding:
    def __init__(self, input_files=None, initial_words_file=None, dim=50, expanding_time=50, method='skip-gram', self_define_dictionary=None, update_corpus=False):
        ...
        initial_words_file = initial_words_file or INITAL_WORDS

        if not os.path.exists(initial_words_file):
            raise NameError('please put your initial words in initial_words.txt')

        src_dir = SRC_DIR
        input_files = input_files or ["{}/{}".format(src_dir, f) for f in os.listdir(src_dir)]

        if len(input_files) == 0: raise TypeError('source files is None, please put your files into ./source_files')
