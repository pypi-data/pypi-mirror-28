...
class MultiLanguageService(BaseService):
    ...
    lang_switch_command_keywords = [u"/language", u"/sprache"]
    u"""
    List of command triggers that can be used to list or change the language
    """

    lang_switch_aliases = {
        u"en": [u"en", u"english", u"englisch"],
    ...
    # noinspection PyMethodMayBeStatic
    def is_applicable_to_multi_language(self, message):
        u"""
        ...
        for keyword in self.lang_switch_command_keywords:
            if body.startswith(keyword + u" ") or body == keyword:
                return True
        return False

    def handle_message_multi_language(self, message):
        u"""
        ...
        lang_switch_dict = {
            u"@{title}": {
                u"en": u"Language Change",
                u"de": u"Sprachwechsel"
            },
            u"@{success_message}": {
                u"en": u"Successfully changed language to",
                u"de": u"Sprache erfolgreich geändert zu"
            },
            u"@{fail_message}": {
                u"en": u"Failed to switch to language",
        ...
        if len(params) == 1 and params[0] in self.lang_switch_command_keywords:
            ...
            self.reply(self.translate(u"@{title}", language, lang_switch_dict),
                       language, message)
        ...
        # Example Message: '/language english'
            ...
            title = self.translate(u"@{title}", language, lang_switch_dict)

            if found_language:
                self.reply(title, self.translate(u"@{success_message}: " +
                                                 language, language,
                                                 lang_switch_dict), message)
            else:
                self.reply(title, self.translate(u"@{fail_message}: " +
                                                 params[1], language,
                                                 lang_switch_dict), message)
    ...
    def get_stored_language_preference(self, contact):
        u"""
