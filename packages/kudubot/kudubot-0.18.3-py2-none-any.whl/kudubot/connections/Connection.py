...
class Connection(object):
    ...
    def __init__(self, services,
        ...
        self.database_file_location = \
            os.path.join(self.database_file_location, self.identifier + u".db")
        self.config_file_location = \
            os.path.join(self.config_file_location, self.identifier + u".conf")
        ...
        try:
            ...
            for service in services:
                try:
                    ...
                    self.logger.error(
                        u"Service " + service.define_identifier() +
                        u" failed to load due to error:" +
    ...
    def apply_services(self, message, break_on_match = True):
        ...
        if self.authenticator.is_blacklisted(message.sender) \
            ...
            self.logger.info(u"Contact is blacklisted. Not applying services")

        self.logger.debug(
            u"Applying services to " + repr(message.message_body) + u"."
        )
        ...
        for service in self.services:
            try:
                ...
                self.logger.error(
                    u"Service " + service.identifier +
                    u" failed in executing message " + message.message_body +
                    u" with exception " + unicode(e.args) +
    ...
    def listen_in_separate_thread(self):
        ...
        self.logger.info(
            u"Starting daemon thread for connection " + self.identifier + u"."
        )
