...
class HelperService(MultiLanguageService):
    ...
    def handle_message_helper(self, message):
        u"""
        ...
        dictionary = {
            u"@help_message_title":
                {u"en": u"Help Message for " + self.identifier,
                 u"de": u"Hilfnachricht für " + self.identifier},
            u"@syntax_message_title":
                {u"en": u"Syntax Message for " + self.identifier,
        ...
        if body.startswith(self.define_command_name(language)):
            ...
            if body in help_keywords:
                ...
                title = self.translate(u"@help_message_title",
                                       language, dictionary)
                try:
                    body = self.define_help_message(language)
                except KeyError:
                    body = u"NOT_DEFINED_FOR_LANG"
                    self.logger.error(u"Missing language definition: " +
                                      language)
                ...
                title = self.translate(u"@syntax_message_title",
                                       language, dictionary)
                try:
    ...
    def is_applicable_to_helper(self, message):
        u"""
        ...
        for entry, values in dictionary.items():
            for lang, keyword in values.items():
                if params[0] == u"/" + keyword:
                    language = lang
                elif params[1] == keyword:
                    language = lang

        help_command = self.translate(u"@help_command", language, dictionary)
        syntax_command = self.translate(u"@syntax_command", language,
                                        dictionary)

        if len(params) == 2 and u"/" + help_command == params[0] \
                and params[1] == identifier:
            # /help identifier
            return True

        elif len(params) == 2 and u"/" + syntax_command == params[0] \
                and params[1] == identifier:
            # /syntax identifier
            return True
            ...
            return message.message_body.lower() in [
                command + u" " + help_command,
                command + u" " + syntax_command
            ]

    def starts_with_command_keyword(self, message, language,
