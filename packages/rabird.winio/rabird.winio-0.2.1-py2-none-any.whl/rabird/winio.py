...
__author__ = u"""Hong-She Liang"""
__email__ = u'starofrainnight@gmail.com'
__version__ = u'0.2.1'
...
class tagPhysStruct(ctypes.Structure):
    _fields_ = [
        (u'dwPhysMemSizeInBytes', ctypes.c_ulonglong),
        (u'pvPhysAddress', ctypes.c_ulonglong),
        (u'PhysicalMemoryHandle', ctypes.c_ulonglong),
        (u'pvPhysMemLin', ctypes.c_ulonglong),
        (u'pvPhysSection', ctypes.c_ulonglong),
    ]
    _pack_ = 1
...
class tagPortStruct(ctypes.Structure):
    _fields_ = [
        (u'wPortAddr', ctypes.c_ushort),
        (u'dwPortVal', ctypes.c_ulong),
        (u'bSize', ctypes.c_ubyte),
    ]
    _pack_ = 1
...
class WinIO(object):
    ...
    def __stop_driver(self):
        ...
        hService = win32service.OpenService(
            self.hSCManager, u'WINIO', win32service.SERVICE_ALL_ACCESS)
        try:
    ...
    def __get_port_value(self, wPortAddr, bSize):
        if bSize not in [1, 2, 4]:
            raise InvalidArgumentError(
                u'Argument "bSize" only accept 1, 2, 4. Current value : %s!' % (unicode(bSize)))
    ...
    def map_phys_to_lin(self, PhysStruct):
        if (not win32file.DeviceIoControl(
                ...
                buffer(PhysStruct),
                buffer(PhysStruct))):
            raise DeviceIoControlError(u"Failed on IOCTL_WINIO_MAPPHYSTOLIN")
    ...
    def unmap_physical_memory(self, PhysStruct):
        if (not win32file.DeviceIoControl(
                ...
                buffer(PhysStruct),
                None)):
            raise DeviceIoControlError(u"Failed on IOCTL_WINIO_UNMAPPHYSADDR")
    ...
    def get_phys_long(self, pbPhysAddr):
        ...
        dwPhysVal = struct.unpack(u'@L', dwLinAddrRaw)
    ...
    def set_phys_long(self, pbPhysAddr, dwPhysVal):
        ...
        pdwLinAddr = self.map_phys_to_lin(PhysStruct)
        if (pdwLinAddr == 0):
            raise ValueError(u"Linear address invalid!")
    ...
    def __is_64bit_os(self):
        if u'PROCESSOR_ARCHITEW6432' in os.environ:
            if os.environ[u'PROCESSOR_ARCHITEW6432'] == u'AMD64':
                return True

        if u"64" in os.environ[u"PROCESSOR_ARCHITECTURE"]:
            return True
    ...
    def __get_driver_file_path(self):
        ...
        if self.__is_64bit_os():
            result = os.path.join(winio_binary_data_dir, u'WinIo64.sys')
        else:
            result = os.path.join(winio_binary_data_dir, u'WinIo32.sys')
    ...
    def __initialize(self):
        ...
        hDriver = None
        try:
            hDriver = win32file.CreateFile(ur"\\.\WINIO",
