...
def process_export_user_id(conninfo, credentials, input_user_id):
    user_id = users.get_user_id(conninfo, credentials, input_user_id)
    return str(user_id.data)

def parse_nfs_export_restrictions_file(conninfo, credentials, path):
    with open(path) as f:
        contents = f.read()
        try:
            restrictions = json.loads(contents)
    ...
    for r in restrictions['restrictions']:
        ...
        try:
            ...
            if (user.get('id_type') == 'NFS_UID') ^ ('map_to_group' in r):
                raise ValueError('Restriction should either have both of '
                    'map_to_nfs_user_id and map_to_nfs_group_id or neither.')

            restriction['map_to_user'] = \
                {
                    'id_type': user.get('id_type'),
                    'id_value': process_export_user_id(
                        conninfo, credentials, user.get('id_value'))
                }
...
class NFSAddExportCommand(qumulo.lib.opts.Subcommand):
    ...
    def main(conninfo, credentials, args):
        if args.restrictions:
            restrictions = parse_nfs_export_restrictions_file(conninfo,
                credentials, args.restrictions)
        else:
