...
def _get_preserved_time():
    ...
    """
    months = Settings.query.join(SettingsValues, Settings.id_settings == SettingsValues.settings_id_settings)\
        .filter(Settings.settings_name == 'MONTHS_PRESERVED').with_entities(SettingsValues.value).one_or_none()

    if months is None:
        return 4
    else:
        return int(months.value)
...
def get_events(user_id):
    ...
    usr_events = Events.query \
        ...
        .join(ReaderSettings, ReaderSettings.id_reader == Events.reader_settings_id_reader) \
    ...
    if usr_events is not None:
        ...
        for event in usr_events:
            content = event.first_name + ' ' + event.last_name + '<p class="small">(' + str(event.location_name) + ')</p>'
            val = {
                'id': event.id_event,
