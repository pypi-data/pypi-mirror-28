...
class Trafo:
    ...
    @classmethod
    def from_dict(cls, trafodata, trafoschema, verify=True): #TODO: avoid passing trafoschema - load it from here
        trafo = trafodata
        return cls(verify=verify, **trafo)
        trafo = collections.OrderedDict()
        if isinstance(trafodata, list):
            trafo["actions"] = trafodata
        elif isinstance(trafodata, basestring):
            trafo["actions"] = [{"print": trafodata}]
        else:
            trafo.update(trafodata)
        try:
