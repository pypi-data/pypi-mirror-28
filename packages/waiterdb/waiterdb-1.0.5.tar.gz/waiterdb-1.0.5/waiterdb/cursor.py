...
import logging
import collections
from waiterdb.types import TypeHelper
...
class Cursor(object):
    ...
    def _set_signature(self, signature):
        ...
        for column in self._signature['columns']:
            self._columns.append(column['columnName'])
        for column in signature['columns']:
            field_name, rep, cast_from = TypeHelper.from_class(column['columnClassName'])

            self._column_data_types.append(cast_from)
        #TODO
    ...
    def _transform_row(self, row):
        ...
        for i, column in enumerate(row):
            # cast_from = self._column_data_types[i]
            # if cast_from is not None:
            #     value = cast_from(column)
