...
class BusinessSalesPrediction(SnsVisualization):
    ...
    def sum_mutil_dealer(self, sales_file_ori):
        ...
        df.drop(['省', '市', '区/县', '营业部', '现有经销商/物流公司名称', '客户形态', '现有经销商/物流公司名称',
        ...
        df2 = df.groupby(['具体描述', '客户形态对应的商圈等级']).sum()  # 合并一商圈多经销商销量数据
...
def main(work_dir, province, sales_file_ori, sales_file_proc, ocm_file, feature_file):
    if not sales_file_proc:
    ...
    for sale_tag in ['整体平均千箱', '高价平均千箱']:
        ...
        AnalyzeGrading.analyze_grading(tag, '%s_综合.xlsx' % tag, '客户形态对应的商圈等级', '预估销量', '整体预估销量', province)
