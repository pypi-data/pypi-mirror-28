...
class SystemTests(unittest.TestCase):
    ...
    def check_call(
            self, method, uri, auth=True, data=None, include_example=False):
        headers = self.default_headers if auth else None
        response = self.client.open(
            uri, method=method, headers=headers, data=data)

        actual_status_code = int(response.status.split(" ")[0])
        self.assertEqual(self._expected_status, actual_status_code)

        if (
                not self._expected_contents or
                isinstance(self._expected_contents, str)):
