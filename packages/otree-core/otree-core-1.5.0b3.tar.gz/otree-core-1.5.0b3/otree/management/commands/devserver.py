...
ADVICE_DELETE_TMP = (
    "Try deleting the folder {}. If that doesn't work, "
...
if is_sqlite:
    ADVICE_DELETE_DB = (
        'Try deleting the file db.sqlite3 in your project folder.'
    )
else:
    ADVICE_DELETE_DB = 'Try deleting your database.'
...
class Command(runserver.Command):
    ...
    def handle_migrations(self):
        ...
        try:
            ...
            # could be due to corrupted migrations files,
            # or
            ExceptionClass = type(exc)
            tb = sys.exc_info()[2]
            raise ExceptionClass(
                '{} - {}'.format(exc, ADVICE_DELETE_TMP)
            ).with_traceback(tb) from None

        try:
            call_command('migrate', '--noinput')
        except (OperationalError, migrations_excs.InconsistentMigrationHistory) as exc:
            ExceptionClass = type(exc)
            tb = sys.exc_info()[2]
            raise ExceptionClass(
                '{} - {}'.format(exc, ADVICE_DELETE_DB)
            ).with_traceback(tb) from None
