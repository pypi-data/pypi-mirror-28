...
class Distribution:
                ...
                if self.__primary_key != None and value.empty is False :
                    ...
                    if check.empty is False:
                        if  self.__primary_key not in check.columns and self.__primary_key not in value.columns:
                            self.end()
                            raise ValueError('if you want to check repeat values,please write current column but not "{}" and mode must be "hdfs".'.format(self.__primary_key))
                        else:
                            if len(set(check[self.__primary_key])&set(value[self.__primary_key])) !=0:
                                ...
                                raise ValueError("the new value is repeated by {} with primary key '{}'.".format(key,self.__primary_key))
                            else:
        ...
        if self.__mode == 'a':
            if key in self.__KEYS:
                ...
                if check.empty is False and value.empty is False:
                    ...
                    raise ValueError('check length %s,value length %s,not update' % (len(check), len(value)))

            else:
                raise ValueError("{} not in keys".format(key))
        ...
        if self.__mode == 'a':
            if key in self.__KEYS:
                ...
                if check.empty is False :
                    ...
                    raise ValueError('check length %s,not update' % (len(check)))
            else:
                raise ValueError("{} not in keys".format(key))
        else:
            raise ValueError("the mode is '%s',you only can choose 'a'." % self.__mode)
    ...
    def read(self,key):
        ...
        craft = Craft(mode='a',store_path=json['position'][key_to_file],store_name=key_to_file)
        resp = craft.read(key)
        return resp
