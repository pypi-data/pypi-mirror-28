...
def binary_gather(obj_gather, comm, maxbchunk, logger):
    ...
    logger.debug("Rank: %d - gather #1" % rank)
    ...
    while continue_flag:
        ...
        logger.debug("Rank: %d - gather #2" % rank)
        msg_size = stop-start
        comm.gather( msg_size, root=0 )
        # get buffer size
        logger.debug("Rank: %d - gather #3" % rank)
        buf_size = comm.bcast(None, root=0)
        chunk = bytearray(buf_size)
        chunk[:msg_size] = obj_gather[start:stop]
        # gather chunk
        logger.debug("Rank: %d - gather #4" % rank)
        comm.Gather( chunk, None, root=0 )
        # get stopping flag
        logger.debug("Rank: %d - gather #5" % rank)
        continue_flag = comm.bcast(None, root=0)
        start = stop
            ...
            if cmd_str == "STOP": # Terminates
                ...
                for key, arg in zip(dmem_key_in_list, dmem_arg_in_list):
                    try:
                        ...
                        tmp[arg] = distr_mem[key]
                # Evaluate
                logger.debug("Rank: %d - Evaluate" % rank)
                out = func(**tmp)
                logger.debug("Rank: %d - Evaluate [DONE]" % rank)

                # Unpickle reduce object
                if reduce_obj is not None:
                    logger.debug("Rank: %d - Reduce" % rank)
