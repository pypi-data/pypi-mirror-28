...
class MPI_Pool_v2(object):
    ...
    def mod_import(self, import_set):
        ...
        """
        logger.debug("MPI_Pool.mod_import: broadcast [before]")
        bcast_tuple = ("IMPORT", import_set)
        bcast_tuple_dill = dill.dumps(bcast_tuple)
        self.comm.bcast(bcast_tuple_dill, root=MPI.ROOT)
        logger.debug("MPI_Pool.mod_import: broadcast [after]")
        # Gather any error
        logger.debug("MPI_Pool.mod_import: gather [before]")
        fval = self.comm.gather(None, root=MPI.ROOT)
        logger.debug("MPI_Pool.mod_import: gather [after]")
        # Check for exceptions
        fail = False
        for v in fval:
            if isinstance(v, tuple) and isinstance(v[0], Exception):
    ...
    def map(self, f, obj_scatter=None, obj_bcast=None,
        ...
        # Broadcast function and parameters
        bcast_tuple = ("MAP", obj_dill, f_in, obj_bcast,
    ...
    def map_alloc_dmem(self, f, obj_scatter=None, obj_bcast=None,
        ...
        bcast_tuple = (
            ...
            dmem_key_out_list, red_obj_dill)
        bcast_tuple_dill = dill.dumps(bcast_tuple)
        self.comm.bcast(bcast_tuple_dill, root=MPI.ROOT)
        logger.debug("MPI_Pool.eval: broadcast")
    ...
    def alloc_dmem(self, **kwargs):
        r""" Allocate data on distributed memory. ``kwargs`` defines the names used in the distributed memory dictionary.
        """
        logger.debug("MPI_Pool.alloc_dmem: bcast [before]")
        bcast_tuple = ("ALLOC_DMEM", kwargs)
        bcast_tuple_dill = dill.dumps(bcast_tuple)
        self.comm.bcast(bcast_tuple_dill, root=MPI.ROOT)
        logger.debug("MPI_Pool.alloc_dmem: bcast [after]")
        # Gather any error
        logger.debug("MPI_Pool.alloc_dmem: gather [before]")
        fval = self.comm.gather(None, root=MPI.ROOT)
        logger.debug("MPI_Pool.alloc_dmem: gather [after]")
        # Check for exceptions
        fail = False
        for v in fval:
            if isinstance(v, tuple) and isinstance(v[0], Exception):
    ...
    def get_dmem(self, *args):
        ...
        """
        logger.debug("MPI_Poll.get_dmem: bcast [before]")
        bcast_tuple = ("GET_DMEM", args)
        bcast_tuple_dill = dill.dumps(bcast_tuple)
        self.comm.bcast(bcast_tuple_dill, root=MPI.ROOT)
        logger.debug("MPI_Poll.get_dmem: bcast [after]")
        # Gather any error
        logger.debug("MPI_Pool.get_dmem: gather [before]")
        fval = self.comm.gather(None, root=MPI.ROOT)
        logger.debug("MPI_Pool.get_dmem: gather [after]")
        # Check for exceptions
        fail = False
        for v in fval:
            if isinstance(v, tuple) and isinstance(v[0], Exception):
    ...
    def pop_dmem(self, *args):
        ...
        Returns:
          (:class:`list` [``nprocs``]) -- list of tuples containing the data retrievered.
        """
        logger.debug("MPI_Poll.pop_dmem: bcast [before]")
        bcast_tuple = ("POP_DMEM", args)
        bcast_tuple_dill = dill.dumps(bcast_tuple)
        self.comm.bcast(bcast_tuple_dill, root=MPI.ROOT)
        logger.debug("MPI_Poll.pop_dmem: bcast [after]")
        # Gather any error
        logger.debug("MPI_Pool.pop_dmem: gather [before]")
        fval = self.comm.gather(None, root=MPI.ROOT)
        logger.debug("MPI_Pool.pop_dmem: gather [after]")
        # Check for exceptions
        fail = False
        for v in fval:
            if isinstance(v, tuple) and isinstance(v[0], Exception):
    ...
    def list_dmem(self):
        ...
        """
        logger.debug("MPI_Poll.list_dmem: bcast [before]")
        bcast_tuple = ("LIST_DMEM",)
        bcast_tuple_dill = dill.dumps(bcast_tuple)
        self.comm.bcast(bcast_tuple_dill, root=MPI.ROOT)
        logger.debug("MPI_Poll.list_dmem: bcast [after]")
        # Gather any error
        logger.debug("MPI_Pool.list_dmem: gather [before]")
        fval = self.comm.gather(None, root=MPI.ROOT)
        logger.debug("MPI_Pool.list_dmem: gather [after]")
        # Check for exceptions
        fail = False
        for v in fval:
            if isinstance(v, tuple) and isinstance(v[0], Exception):
    ...
    def clear_dmem(self):
        ...
        """
        logger.debug("MPI_Poll.clear_dmem: bcast [before]")
        bcast_tuple = ("CLEAR_DMEM",)
        bcast_tuple_dill = dill.dumps(bcast_tuple)
        self.comm.bcast(bcast_tuple_dill, root=MPI.ROOT)
        logger.debug("MPI_Poll.clear_dmem: bcast [after]")
        # Gather any error
        logger.debug("MPI_Pool.clear_dmem: gather [before]")
        fval = self.comm.gather(None, root=MPI.ROOT)
        logger.debug("MPI_Pool.clear_dmem: gather [after]")
        # Check for exceptions
        fail = False
        for v in fval:
            if isinstance(v, tuple) and isinstance(v[0], Exception):
