...
def get_output_list(git_folder=None, threshold_size=None):
    ...
    list_of_large_files = git_utility._get_all_file_with_info()
...
def console_output():
    ...
    '''
    if (len(sys.argv)) != 2:
        print '''Format should be:: cygitcheck "<GitFolderPath>"
         <ThresholdSize> \n GitFolderPath denotes where ".git" folder is
         present'''
        return
    args = sys.argv[1:]
...
def fat_html_output():
    ...
    template_path = pkg_resources.resource_filename(__name__,template_name)
    #template_path = '/'.join(('..', 'output_template', template_name))
