...
def send_firebse_message(api_key, sender_id, device_token, title, message, sound = None):
    fb = FirebaseAPI(firebase_api_key = api_key, sender_id = sender_id)
...
class FirebaseAPI(object):
    ...
    def init(self, firebase_api_key = None, sender_id = None):
        ...
        if self.firebase_api_key == None or self.send_id == None:
    ...
    def send_notification(self, payload):
        if type(payload) != dict or payload.get('to') == None or payload.get('notification'):
    ...
    def generate_payload(self, device_token, message, time_to_live = None, collapse_key = None):
        ...
        if type(message) != dict or message.get('title') == None or message.get('body'):
