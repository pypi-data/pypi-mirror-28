...
def api_model_jobs(request, model_name_slug, model_pk = None, model_uuid = None, admin = False):
    api = ModelJobAPI(request = request, admin = admin, model_name = model_name_slug, model_pk = model_pk, model_uuid = model_uuid, job_pk = None)
    api.run()
...
def api_model_job(request, model_name_slug, model_pk = None, model_uuid = None, job_uuid = None, admin = False):
    ...
    api = ModelJobAPI(request = request, admin = admin, model_name = model_name_slug, model_pk = model_pk, model_uuid=model_uuid, job_pk= job_uuid)
    api.run()
    response = api.format_data(api.data)
    return response
