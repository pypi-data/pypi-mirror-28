...
class Command(BaseCommand):
    ...
    def run(self):
        ...
        while start_time < arrow.utcnow().replace(minutes =+ self.MAX_TTL): # run for a maximum of 24 minutes and then die!
    ...
    def _check_and_update_job_status(self, job):
            ...
            # otherwise update the job status update job to "running status"
            try:
                job = self.JobManager.update(Serializer = self.JobSerializer, model = job, data = {'status' : 2})[0]
            except Exception as e:
                # in most likelihood, something broke, either job is missing or whatever.
                raise
