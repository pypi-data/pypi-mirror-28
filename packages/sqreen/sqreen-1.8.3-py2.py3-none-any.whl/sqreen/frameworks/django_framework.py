...
class DjangoRequest(BaseRequest):
    ...
    def messages_params(self):
        if self._messages_params is None:
            ...
            if storage is None:
                LOGGER.debug("Django messages framework is not enabled")
                self._messages_params = []
                return {}
            messages = getattr(storage, '_queued_messages', None)
            if not isinstance(messages, collections.Sequence):
                # Don't destroy messages.
                LOGGER.info("Skipping messages: expected a list, got {}",
                            messages.__class__.__name__)
                self._messages_params = []
                return {}
            self._messages_params = messages
        if not self._messages_params:
            return {}
        messages = [message.message for message in self._messages_params
