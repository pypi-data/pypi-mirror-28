...
class CoralDBClient(object):
    ...
    def getUserData(self, table, query="", sort="", fields="*", offset=0, limit=0, password=""):
        ...
        params = {'table': table, 'query': query, 'sort': sort, 'fields': fields, 'offset': offset, 'limit': limit,
                  'password': password}
        ret = []
        if limit <= 0:
            _offset = offset
    ...
    def setUserData(self, table, data=[], password=""):
        """
        setUserData: 保存用户数据
        :param table: 数据表名
        :param data: 待保存的数据或列表，每条数据必须有一个ID字段；
        :return:
        """
        if not table:
            raise Exception("table is required")
        ...
        params = {'table': table, 'data': data}
        rep = self.__request('setUserData', params)
        return CoralResultSet(rep['columns'], rep['values'])
