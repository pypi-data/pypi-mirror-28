...
AWS_ROLES = ('ReadOnly', 'Deployer', 'Administrator')
...
def kms_encrypt(config, external_credentials, kms_keyid, region, plain_text_blob):
    ...
    if external_credentials:
        ...
        if matching_roles:
            account_name = matching_roles[0].account_name
            kms = create_boto_session(token, account_id, matching_roles[0].role_name).client('kms', region)
        else:
