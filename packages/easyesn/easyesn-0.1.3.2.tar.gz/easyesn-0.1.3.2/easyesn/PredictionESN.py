...
class PredictionESN(BaseESN):
    ...
    def fit(self, inputData, outputData, transientTime="AutoReduce", transientTimeCalculationEpsilon = 1e-3, transientTimeCalculationLength = 20, verbose=0):
        ...
        if verbose > 0:
           bar = progressbar.ProgressBar(max_value=inputData.shape[0]*inputData.shape[1], redirect_stdout=True, poll_interval=0.0001)

        self._X = B.empty((1 + self.n_input + self.n_reservoir, totalLength))
        for i in range(inputData.shape[0]):
            self._X[:, i*partialLength:(i+1)*partialLength] = self.propagate(inputData[i], outputData[i], transientTime, verbose, bar=bar)
