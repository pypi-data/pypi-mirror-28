...
def ftp_decorator(func):
    ...
    def wrapper(self, *args, **kwargs):
        try:
            ...
            result = func(self, ftp, *args, **kwargs)
            return result
...
def convert_exc(exc):
    ...
    if "privileges" in msg:
        return PermissionError("no permission to access file/dir in ftp")
    elif "exists" in msg:
        return FileExistsError("file/dir already exist in ftp")
    elif "no such" in msg or "not a directory" in msg:
        return FileNotFoundError("file/dir not exists in ftp")
    else:
