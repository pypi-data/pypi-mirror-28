...
def serve_forever(debug=False):
    ...
    if debug:
        ...
        logger = logger.getLogger("release")
    # loading setting
    config_path = os.path.join(ETC_PATH, "web.conf")
    if not os.path.isfile(config_path):
        self.logger.error("configuration file %s not exist" % config_path)
    try:
        ...
        fs_buffer_size = config["fs_buffer_size"]
        ...
        self.logger.error("config file '%s' error: %s" %(config_path, format_exc()))
        raise
    try:
