...
@re_command( visibility = visibilities.HIGHLIGHT[visibilities.CLI] )
...
def follow_path( path: str, restrict: Type[T] = IVisualisable ) -> PathToVisualisable[T]:
    """
    ...
    last = selected_path.get_last()
    
    if not AnnotationInspector( restrict ).is_viable_instance( last ):
        raise ValueError( "This argument requires a «{}», but you have selected «{}», which is a «{}».".format( restrict, last, type( last ) ) )
