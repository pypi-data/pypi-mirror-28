...
from intermake.engine.constants import EThread, EDisplay
...
class GuiHost( PluginHost ):
    ...
    def __invoke_unmanaged( self, plugin: Plugin, args: ArgsKwargs, callback: Optional[Callable[[AsyncResult], None]], sync: bool ) -> Optional[object]:
        ...
        try:
            ...
            async_result = AsyncResult( plugin.name, result, None, traceback.format_exc(), None )
            return result
        except Exception as ex:
            async_result = AsyncResult( plugin.name, None, ex, traceback.format_exc(), None )
            raise
        finally:
            ...
            elif not self.__is_in_plugin():  # don't callback to the main thread if we are in another plugin's thread
