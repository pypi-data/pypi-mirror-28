...
class Project:
    ...
    def change_file_metadata(self, container_id, filename, modality, tags):
        ...
        :param modality: Modality identifier, or None if the file shouldn't have any modality
        :type modality: str or None

        :param tags: List of tags, or None if the filename shouldn't have any tags
        ...
        self._account.send_request(
            ...
            req_parameters={"container_id": container_id, "filename": filename, "tags": tags_str, "modality": modality}
        )
    ...
    def upload_file(self, file_path, subject_name, ssid="", date_of_scan="",
        ...
        :param ssid: the id of the timepoint
        :param date_of_scan: date of scan/creation of the file
        :param description: description of the file
        :param result: if result=True then the upload will be taken as an
                       offline analysis
        :param name: name of the file in the platform
        :param input_data_type: mri_brain_data:1.0 or gametection:1.0
    ...
    def start_analysis(
        ...
        :param in_container_id: The id of the container to get the data from.
                Must be None when passing a value for settings.
        ...
        :param settings: The input settings used to run the analysis.
                Use either settings or in_container_id. Input specification
                in the settings dict can be done by using the key 'input'.
        :type settings: Dict

        :param tags: The tags of the analysis.
        :type tags: String|List
        """
        logger = logging.getLogger(logger_name)

        if in_container_id is not None and settings is not None:
            raise ValueError("Pass a value for either in_container_id or "
        ...
        if in_container_id:
            if isinstance(in_container_id, dict):
                ...
                post_data['as_input'] = str(in_container_id)
        else:
            if isinstance(settings, dict):
                for key in settings:
                    post_data['as_' + key] = settings[key]
            else:
                raise ValueError('Settings must be of type dictionary!')
        ...
        if response["success"] == 0:
            ...
            if response["data_to_choose"]:
                ...
                for settings_key in response["data_to_choose"]:
                    ...
                    filters = response["data_to_choose"][settings_key]["filters"]
                    for filter_key in filters:
                        ...
                        choosen_files[settings_key][filter_key] = files_selection
            ...
            return self.__handle_start_analysis_response(new_post, response,
