...
def create(description, blueprint, machine, gpu, cheap, command):
    """Create a new job."""
    ...
    response = make_request('/spawner/jobs', method='POST', data=data)
...
def list():
    """List your jobs."""
    response = make_request('/spawner/jobs', method='GET')
    jobs = response.json()['jobs']
    data = []
    data.append(["", "Name", "Blueprint", "Command", "Machine type", "Status"])
    for index, job in enumerate(jobs):
        image = "{}/{}".format(
            job['specs']['repository'], job['specs']['blueprint']
        )
        data.append([
            index, job['name'],
            image,
            job['specs']['command'],
            job['specs']['machine_type'],
            job['status']
        ])
