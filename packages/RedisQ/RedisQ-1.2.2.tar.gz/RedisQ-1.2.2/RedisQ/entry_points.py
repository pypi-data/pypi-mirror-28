import click
import sys
import os
from RedisQ.scripts.DICOMpayload import *
from RedisQ.scripts.WSIpayload import *
from RedisQ.WatchDog import watchOn


@click.command()
@click.option('--worker', '-w', default='dicom', help="options: dicom, wsi")
@click.option('--retry', '-r', default=3, help="options: a int num to retry max")
@click.option('--host', '-h', default='127.0.0.1', help="options: server host")
@click.option('--port', '-p', default='6389', help="options: server port")
@click.option('--silence', '-s', default=True, type=bool, help="options: true or false to print")
@click.option('--path', '-p', default='', help="options: a path to your Algorithm")
def cli(worker, retry, host, port, silence, path):

    if path:
        sys.path.append(path)

    if not silence:
        sys.stdout = sys.__stdout__
    else:
        sys.stdout = open('./worker.log', 'a+')

    retry_count = -1

    if worker == 'dicom':
        while True:
            try:
                retry_count += 1
                conn = Redis(host=host, port=port)
                config = DICOMConfig()
                worker = GPUWorker(connection=conn, config=config, job_hash=HJ, sign_hash=HS,
                                   queue_list=LQ, working_list=LW, done_list=LD, fail_list=LF)
                print('polling...')
                worker.polling(config=config, payload=payload)(host, port)
            except KeyboardInterrupt as e:
                break
            except Exception as e:
                print(e)

            if retry_count >= retry:
                worker.working_list.pop_to(worker.fail_list)
                os.remove(worker.pid_file)

    elif worker == 'wsi':
        while True:
            try:
                retry_count += 1
                conn = Redis(host=host, port=port)
                config = WSIConfig()
                worker = GPUWorker(connection=conn, config=config, job_hash=HJ, sign_hash=HS,
                                   queue_list=LQ, working_list=LW, done_list=LD, fail_list=LF)
                print('polling...')
                worker.polling(config=config, payload=payload)
            except KeyboardInterrupt as e:
                break
            except Exception as e:
                print(e)

            if retry_count >= retry:
                worker.working_list.pop_to(worker.fail_list)
                os.remove(worker.pid_file)


@click.command()
@click.option('--api', '-a', default='127.0.0.1', help="options: api host")
@click.option('--path', '-p', default='.', help="options: the path you want to watch on")
def watchdog(path, api):
    watchOn(path, api)
