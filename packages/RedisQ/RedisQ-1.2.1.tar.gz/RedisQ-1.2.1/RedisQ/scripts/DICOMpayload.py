...
def payload(job, config):
    ...
    for image in response.json().get('results'):
        ...
        if raw_uri and id is None:
            raise Exception("can get resource information 'cause raw_uri or id is not in the response")
    ...
    if not files:
        print('prediction...')
        try:
            from DICOM.AlgorithmAPI import main_interface
            class algorith_thread(Thread):
                def run():
                    main_interface.predictLungCancer(dcm_dir=base,model_dir='DICOM/models/',tgt_dir=des,tmp_dir='/tmp/plc/',pci_id='1')
                    files = os.listdir(des)
                    ...
                    for file_name in files:
                        ...
                        if file_name.endswith('csv'):
                            with open('{des}/{file_name}'.format(des=des,file_name=file_name), 'r') as f:
                                ...
                                for row in reader:
                                    metadata.features['results'].append(row)
                        if file_name.endswith('txt'):
                            with open('{des}/{file_name}'.format(des=des, file_name=file_name), 'r') as f:
                                    result = f.read()
                                    metadata.fetures['total'] = result
                    ...
                    print('payload done')
                    print('polling...')
            algorith_thread.daemon = True
            algorith_thread.ident = 'DICOM_algorith'
            algorith_thread.start()
        except Exception as e:
            print(e)
            with open('./algorithm.log','a+') as f:
               f.write(str(e))
