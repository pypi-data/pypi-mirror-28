...
def payload(job, config):
    ...
    if not files:
        print('prediction...')
        try:
            from DICOM.AlgorithmAPI.main_interface import predictLungCancer
            predictLungCancer(dcm_dir=base,model_dir='DICOM/models/',tgt_dir=des,tmp_dir='/tmp/plc/',pci_id='1')
