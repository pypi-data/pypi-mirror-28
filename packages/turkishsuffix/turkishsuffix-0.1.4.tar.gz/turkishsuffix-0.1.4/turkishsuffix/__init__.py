...
class TurkishSuffix:
    def __init__(self):
        ...
        path = pkg_resources.resource_string(__name__, "turkishsuffix/config.yml")
...
class Suffix:
    ...
    def set_suffix(self, _type):
        ...
        for i, letter in enumerate(self._word[::-1]):
            if letter in turkishSuffix.hards:
                if first[0] in "cd":
                    ...
                    if turkishSuffix.hards.index(letter) < 4:
                        self._word = self._word[:-1] + turkishSuffix.softs[turkishSuffix.hards.index(letter)]
    ...
    def get_word(self, special):
        apostrophe = ""
        if special is True:
            apostrophe = "'"
        return self._word + apostrophe + self._suffix
