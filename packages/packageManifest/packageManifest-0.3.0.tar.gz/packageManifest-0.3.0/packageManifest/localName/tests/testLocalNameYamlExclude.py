...
from ..exclude import Exclude, YamlNotFound


class TestYamlExclude( unittest.TestCase ) :
    ...
    def testExcludeNotFound( self ) :
        ...
        with self.assertRaisesRegex( YamlNotFound, '^exclude not found in yaml data' ) :
            Exclude.from_yamlData( yamlData )


    def testIncludeFilesNotFound( self ) :
        yamlData = { 'exclude' : { 'directory' : 'one' } }

        with self.assertRaisesRegex( YamlNotFound, '^exclude must use files directive' ) :
            Exclude.from_yamlData( yamlData )


    def testExcludeOk( self ) :
        expectedPatterns = [ 'one', 't*' ]
        yamlData = { 'exclude' : { 'files' : expectedPatterns } }
