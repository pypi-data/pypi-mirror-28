...
from ..exclude import applyGlobalExclusions


class TestGlobalExclusions( unittest.TestCase ) :
    def testGlobalSubdirInclude( self ) :
        inclusions = [ 't*' ]
        inputList = [ 'one', 'two', 'three', 'four', 'subdir/one', 'subdir/two' ]
        expectedList = { 'one', 'four', 'subdir/one' }

        actualList = set( applyGlobalExclusions( inputList, inclusions ) )
    ...
    def testGlobalSubdirPathInclude( self ) :
        ...
        expectedList = { 'one', 'two', 'three', 'four', 'subdir/one' }
