...
class Files :
    ...
    def from_yamlData( cls, yamlData ) :
        ...
        elif 'files' not in yamlData :
            raise YamlNotFound( 'files not specified in yaml, {0}'.format( yamlData ) )
        else :
            thisFiles = cls()

            if not isinstance( yamlData[ 'files' ], list ) :
                raise YamlNotFound( 'files patterns must be a list' )

            thisFiles.patterns = yamlData[ 'files' ]
