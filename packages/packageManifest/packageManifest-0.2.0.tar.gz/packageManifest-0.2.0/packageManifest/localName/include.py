...
def applyInclusions( currentNames, inclusions ) :
    """
    Apply unix glob wildcard expressions to select elements from an iterable.

    eg.
       "*.py" implies select *.py files in the root directory
       "one/*.py" implies select *.py files in the "<root>/one" directory

    :param currentNames: The iterable to be filtered.
    :param inclusions: The iterable of inclusion globs.

    :return: Set of filtered paths.
    """
    filteredNames = set()
    for thisInclude in inclusions :
        namesMatchInclusion = set( fnmatch.filter( currentNames, thisInclude ) )

        filteredNames |= namesMatchInclusion
...
class Include( PathOperation ) :
    ...
    def apply( self, originalPathNames, currentPathNames ) :
        ...
        includedNames = applyInclusions( originalPathNames, [ self.includePattern ] )
        modifiedPathNames = currentPathNames | includedNames

        return modifiedPathNames
