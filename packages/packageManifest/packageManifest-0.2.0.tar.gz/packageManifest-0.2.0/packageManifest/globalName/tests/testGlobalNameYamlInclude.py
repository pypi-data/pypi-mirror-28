...
class TestGlobalNameYamlInclude( unittest.TestCase ) :
    ...
    def testIncludeNotFound( self ) :
        ...
        with self.assertRaisesRegex( YamlNotFound, '^global-include not found in yaml data' ) :
            Include.from_yamlData( yamlData )


    def testIncludeFilesNotFound( self ) :
        yamlData = { 'global-include' : { 'directory' : 'one' } }

        with self.assertRaisesRegex( YamlNotFound, '^global-include must use files directive' ) :
            Include.from_yamlData( yamlData )


    def testIncludeOk( self ) :
        expectedPatterns = [ 'one', 't*' ]
        yamlData = { 'global-include' : { 'files' : expectedPatterns } }
