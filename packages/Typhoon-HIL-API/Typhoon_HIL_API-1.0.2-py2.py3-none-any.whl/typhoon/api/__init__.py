...
SETTINGS_CONF_CONTENT = """
; This section contains configuration parameters that are relevant
; for the hil API
[hil]
; Server's response port. Used for receiveing requests from clients and
; sending response.
; The default value for this parameter is 51348, but if that port is
; already taken, new port will be assigned to the server and written here.
server_rep_port = 51348


; This section contains configuration parameters that are relevant
; for the Schematic API
[schematic_editor]
; Server's response port. Used for receiveing requests from clients and
; sending response.
; The default value for this parameter is 51357, but if that port is
; already taken, new port will be assigned to the server and written here.
server_rep_port = 51357


; This section contains configuration parameters that are relevant
; for the PV Generator API
[pv_generator]
; Server's response port. Used for receiving requests from clients and
; sending response.
; The default value for this parameter is 51357, but if that port is
; already taken, new port will be assigned to the server and written here.
server_rep_port = 52602


; This section contains configuration parameters for setting up various
; paths
[path]
; Path to the source folder. This parameter will only be used when
; running our software from the source (in this case, if the parameter
; is not set, or the path to the source is bad, the installation will be
; used)
src_path = c:\\t_sw
; Path to the python that will be used to start a server.
; Uncomment the line bellow and set appropriate path if the Python3
; you're using is not set as system's default. 
; python_path = c:\\Users\\SomeUser\\Envs\\typhoon36\\Scripts


; This section contains configuration parameters used for debugging
[debug]
; Turns the logging on if set to True. In logging is enabled it will
; create two files: server.log in the src_path and client.log in the
; directory from where the client is started.
; Should be False by default
logging_on = False
"""

#
# Generate settings.conf in %PROGRAM_DATA% on Windows family of OSes.
#
if sys.platform.startswith("win"):
    typhoon_dir = os.path.join(os.path.expandvars("%PROGRAMDATA%"), "typhoon")

    if not os.path.exists(typhoon_dir):
        os.makedirs(typhoon_dir)

    settings_conf = os.path.join(typhoon_dir, "settings.conf")

    if not os.path.exists(settings_conf):
        with io.open(file=settings_conf, mode="w") as f:
            f.write(SETTINGS_CONF_CONTENT)
            os.fsync(f.fileno())
