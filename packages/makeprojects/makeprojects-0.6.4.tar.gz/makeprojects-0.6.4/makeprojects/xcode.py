...
class XCBuildConfiguration(object):
	def __init__(self, configname, configfilereference, owner, sdkroot, installpath):
		...
		self.uuid = calcuuid('XCBuildConfiguration' + owner.pbxtype + owner.targetname + str(configname))

	def write(self, fp):
		fp.write('\t\t' + self.uuid + ' /* ' + str(self.configname) + ' */ = {\n')
...
class XCConfigurationList(object):
	...
	def write(self, fp):
		...
		for item in self.configurations:
			...
			fp.write('\t\t\t\t' + item.uuid + ' /* ' + str(item.configname) + ' */,\n')
		if default is None:
			default = 'Release'
		fp.write('\t\t\t);\n')
...
def generate(solution):
	...
	codefiles, includedirectories = solution.getfilelist([FileTypes.icns, \
		FileTypes.h, FileTypes.cpp, FileTypes.frameworks, FileTypes.exe, \
	...
	if solution.xcode.configfilename != None:
		configfilereference = xcodeprojectfile.addfilereference(solution.xcode.configfilename, FileTypes.xcconfig)
	else:
	...
	if solution.projecttype == ProjectTypes.library:
		if solution.platform == PlatformTypes.ios:
			...
			libextension = 'osx.a'
		outputfilereference = xcodeprojectfile.addfilereference('lib' + solution.projectname + idecode + libextension, FileTypes.library)
	else:
		if solution.projecttype == ProjectTypes.app:
			outputfilereference = xcodeprojectfile.addfilereference(solution.projectname + '.app', FileTypes.exe)
		else:
			outputfilereference = xcodeprojectfile.addfilereference(solution.projectname, FileTypes.exe)
	...
	if ioslibrary is True:
		devfilereference = xcodeprojectfile.addfilereference('lib' + solution.projectname + idecode + 'dev.a', FileTypes.library)
		simfilereference = xcodeprojectfile.addfilereference('lib' + solution.projectname + idecode + 'sim.a', FileTypes.library)
