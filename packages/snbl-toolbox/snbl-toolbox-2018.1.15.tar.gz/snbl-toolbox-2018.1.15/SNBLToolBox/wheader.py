    ...
    def run(self, folder: str, checked: list):
        ...
        if checked and os.path.isdir(folder):
            ...
            for i, cbf in enumerate(files):
                ...
                except cbfimage.CbfError as err:
                    QtCore.qWarning(f'Error: {err}')
                    values = [0] * len(checked)
                    if '!File name' in checked and values:
                        values[0] = os.path.basename(cbf)
                else:
                    values = [(hdr[key] if key in hdr else 0) for key in checked]
