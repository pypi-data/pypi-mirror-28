...
def parse(frame):
    if frame[3] == Packet.KEEPALIVE:  # Keepalive packet, not an ethernet frame
    ...
    if frame[14] == 0x06:  # ARP
        ...
        if frame[24] in [6, 17]:  # TCP & UDP
            offset = 15 + ihl * 4
            protocol = ' TCP' if frame[24] == 6 else ' UDP'
...
class VpnNode:
    ...
    def open(self):
        ...
        subprocess.check_call('ifconfig fond%d up' % self.i, shell=True)
        print('...up!')
        self.tap = f
