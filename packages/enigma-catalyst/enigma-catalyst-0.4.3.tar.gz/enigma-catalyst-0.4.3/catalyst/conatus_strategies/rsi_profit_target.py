...
from catalyst.exchange.utils.stats_utils import crossover, crossunder
...
def _handle_data_with_bbands(context, data):
    price = data.current(context.asset, 'close')
    log.info('got price {price}'.format(price=price))
    ...
    signal = None
    if rsi > context.RSI_OVERSOLD_BBANDS and crossover(prices, lower):
        signal = 'long'

    if rsi < context.RSI_OVERBOUGHT_BBANDS and crossunder(prices, upper):
        signal = 'short'

    _handle_buy_sell_decision(context, data, signal)
