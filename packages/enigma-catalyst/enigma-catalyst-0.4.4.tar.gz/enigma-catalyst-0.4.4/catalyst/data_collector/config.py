...
def update_config(exchange):
    ...
    try:
        ...
        msg = unicode(e.message).encode('utf-8', 'ignore')
        print(
    ...
    if new_config is not None:
        if config is not None:
            new_assets = list(
                set(new_config['assets']) - set(config['assets'])
            )
            if len(new_assets) > 0:
...
def apply_asset_bundle_dates(config):
