...
def select(ctx):
    ...
    for index, section in enumerate(config.sections()):
        ...
        echo('{index:<4} : {section_database:<40} http://{host}:{port}/?db={database}   {user}/{password}'.format(
            **kwargs))
    found = False
    while True:
        if found:
...
@click.pass_context
def func(ctx, model, function, domain, limit, id, param, ou, key):
    """Execute a function"""
...
def debug(ctx, model, metadata, menu, delete):
    ...
    models = model
    click.echo('Debug the models : %s' % ([x for x in models],))
    odoo = ctx.obj['action_login']()
...
def yaml_load(ctx, datas):
    ...
    if ctx.obj['workers'] > 0:
        threads = []
        for thread_i in range(ctx.obj['workers']):
            thread_tmp = Thread(target=__process_blocks, args=(ctx, blocks, context, odoo_context))
            thread_tmp.daemon = True
            thread_tmp.start()
            threads.append(thread_tmp)
        [thread_tmp.join() for thread_tmp in threads]
    else:
        __process_blocks(ctx, blocks, context, odoo_context)


def __process_blocks(ctx, blocks, context, odoo_context):
    for block in blocks:
    ...
    for attr in dir(fake):
        if not attr.startswith('_'):
            if name and (name not in attr and attr not in name):
                continue
            try:
                Log.code('{:.<30}{}'.format(attr, getattr(fake, attr)()))
            except:
                pass
