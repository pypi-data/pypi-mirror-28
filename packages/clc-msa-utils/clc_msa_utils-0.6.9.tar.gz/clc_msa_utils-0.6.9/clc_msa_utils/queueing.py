    ...
    def publish(self, message=None, properties=None, try_number=0):
        ...
        try:
            ...
            if try_number > 5:
                ...
                self._logger.info("An error occurred publishing to the {0} exchange {1}, on trying again on number {2}."
...
class QueueWorker:
    ...
    def kv_store(self):
        """
            Returs the KVStore created by this class.
            :return: KVStore
        """
    ...
    def _kv_store_on_reload(self, old_config, new_config):
        self._setup_publishers_and_consumers()

    def _setup_publishers_and_consumers(self, force=False):

        if force or self._kv_store.attribute_changed(self._rabbit_host_key,
