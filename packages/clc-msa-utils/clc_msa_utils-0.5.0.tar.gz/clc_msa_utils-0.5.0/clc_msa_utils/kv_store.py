...
class KVStore:
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(format="[%(threadName)s] %(asctime)s - %(levelname)s - %(name)s - %(message)s")
    ...
    def on_reload(self, on_reload_callback):
        ...
        self._on_reload = on_reload_callback
    ...
    def _read_config(self):
        ...
        # Notify the on-reload callback.
        if self._on_reload and callable(self._on_reload):
            if len(signature(self._on_reload).parameters) == 2:
                self._logger.debug("Notifying on_reload callback method: {0}".format(str(self._on_reload)))
                threading.Thread(target=self._on_reload(self._old_config, self._config)).start()
            else:
