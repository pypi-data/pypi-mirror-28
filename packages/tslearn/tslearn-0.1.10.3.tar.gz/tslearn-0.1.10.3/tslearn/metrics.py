...
def gak(s1, s2, sigma=1.):
    ...
    s2 = to_time_series(s2)
    gamma = 1. / (2 * sigma ** 2)
    return 10 ** (-soft_dtw(s1, s2, gamma=gamma) / gamma)  #cynormalized_gak(s1, s2, sigma)
...
def cdist_gak(dataset1, dataset2=None, sigma=1.):
    ...
    if dataset2 is None:
        ...
        dataset2 = to_time_series_dataset(dataset2)
    return numpy.array([[gak(s1, s2) for s2 in dataset2] for s1 in dataset1])# cycdist_gak(dataset1, dataset2, sigma, self_similarity=self_similarity)
