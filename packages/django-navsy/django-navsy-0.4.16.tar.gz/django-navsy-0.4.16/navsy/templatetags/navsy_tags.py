...
from navsy.utils import i18n_utils, split_words
...
def navsy_is(context, target, value):
    ...
    if isinstance(target, Page):
        ...
        return value if page_obj and page_obj == target else ''

    elif isinstance(target, Route):
        route_obj = data.get('navsy_route', None)
        return value if route_obj and route_obj == target else ''

    elif isinstance(target, basestring) and target:
        route_obj = data.get('navsy_route', None)
        if route_obj:
            targets = split_words(target)
...
@assignment_tag(takes_context=True)
def navsy_is_not(context, target, value):
    return value if not navsy_is(context, target, True) else ''


@assignment_tag(takes_context=True)
def navsy_is_child_of(context, target, value):
    ...
    if isinstance(target, Page):
        ...
        return value if page_obj.pk in page_target_parent_pks else ''

    elif isinstance(target, Route):
        route_target_data = cache.get_route_data(target.pk)
        page_target_data = route_target_data.get('page')
        page_target_obj = page_target_data.get('object')
        page_target_parent_pks = page_target_obj.get_parents_pks() if page_target_obj else []
        return value if page_target_obj and page_obj.pk in page_target_parent_pks else ''
        ...
        for target in targets:
            ...
            page_target_obj = page_target_data.get('object')
            page_target_parent_pks = page_target_obj.get_parents_pks() if page_target_obj else []
            if page_target_obj and page_obj.pk in page_target_parent_pks:
                return value
...
@assignment_tag(takes_context=True)
def navsy_is_parent_of(context, target, value):
    ...
    if isinstance(target, Page):
        page_target_obj = target
        return value if page_target_obj and page_target_obj.pk in page_parent_pks else ''
        ...
        for target in targets:
            ...
            page_target_data = route_target_data.get('page')
            page_target_obj = page_target_data.get('object')
            if page_target_obj and page_target_obj.pk in page_parent_pks:
                return value
