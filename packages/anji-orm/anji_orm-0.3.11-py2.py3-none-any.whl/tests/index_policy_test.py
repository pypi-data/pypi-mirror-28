...
class AbstractIndexPolicyTest(unittest.TestCase):
    ...
    def test_simple_index_creation(self, table_name, index_name):
        ...
        self.assertEqual(target_query.build(), result_query.build())

    @given(
        function_name,
        function_name,
        st.lists(function_name, min_size=1, average_size=2)
    )
...
class SingleIndexPolicyTest(unittest.TestCase):
    ...
    @given(
        st.lists(function_name, min_size=1, average_size=3)
    )
...
class GreedyIndexPolicyTest(unittest.TestCase):
    ...
    def test_greedy_build_index(self, index_list):
        buided_index_list = GreedyIndexPolicy.build_secondary_index_list(index_list)
        target_index_list = []
        ...
        self.assertEqual(target_index_list, buided_index_list)
