...
def wlseparate_ref_voting(arr, knn_lst, class_file, threshold, n_classes=3,
    ...
    vt = np.zeros([arr.shape[0], len(knn_lst)])

    chunk_size = 200000
    n_chunks = np.ceil(arr.shape[0] / chunk_size).astype(int)

    pids = np.arange(arr.shape[0], dtype=int)

    chunk_ids = chunk(pids, n_chunks)

    for cid in chunk_ids:

        d_base, idx_base = set_nbrs_knn(arr, arr[cid], np.max(knn_lst),
                                        return_dist=True)
