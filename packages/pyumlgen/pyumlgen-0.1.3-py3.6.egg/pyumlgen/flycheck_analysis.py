...
class PythonMethod:
    def __init__(self, fun: object):
        ...
        print(vars(inspect.getmodule(fun)))
        types = typing.get_type_hints(fun, vars(inspect.getmodule(fun)))
        ...
        if isinstance(syntax, ast.AnnAssign):
            if check_self_attr(syntax.target):
                ob = eval(compile(ast.Expression(syntax.annotation), "<hint>", "eval"),
                          globals(),
                          {**globals(), **vars(inspect.getmodule(klass))})  # pylint: disable=eval-used
