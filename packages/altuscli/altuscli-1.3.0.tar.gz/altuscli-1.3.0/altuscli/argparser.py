...
from altuscli import ARGPARSE_DASH_ENCODING
from altuscli.compat import six


LOG = logging.getLogger('altuscli.argparser')
...
HELP_BLURB = (
    ...
    "  altus help\n"
    "  altus <command> help\n"
    "  altus <command> <subcommand> help\n"
)
USAGE = (
    "altus [options] <command> <subcommand> [<subcommand> ...] [parameters]\n"
    "%s" % HELP_BLURB
)
...
class CLIArgParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        ...
        if action.choices is not None and value not in action.choices:
            visible_choices = [c for c in action.choices if self._is_visible_choice(c)]
            msg = ['Invalid choice, valid choices are:\n']
            for i in range(len(visible_choices))[::self.ChoicesPerLine]:
                current = []
                for choice in visible_choices[i:i + self.ChoicesPerLine]:
                    current.append('%-40s' % choice)
                msg.append(' | '.join(current))
            possible = get_close_matches(value, visible_choices, cutoff=0.8)
            if possible:
    ...
    def _is_visible_choice(self, choice):
        return True
...
class ServiceArgParser(CLIArgParser):
    ...
    def _is_visible_choice(self, choice):
        operation = self._operations_table[choice]
        if hasattr(operation, 'is_hidden'):
            return not self._operations_table[choice].is_hidden
        return True
