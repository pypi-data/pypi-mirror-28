...
class Coronagraphy(ImagingApPhot):
    ...
    def __init__(self, instrument, webapp=False, config={}, **kwargs):
        ...
        **kwargs: keyword/value pairs
        ...
        if not webapp:
            try:
                config["do_weighted_dithers"]
            except KeyError:
                config["do_weighted_dithers"] = True
        else:
            config["do_weighted_dithers"] = True
    ...
    def _make_dither_weights(self, exposure_list):
        ...
        # If self.do_contrast is set to True, set the unocculted dither. Otherwise, get the scaled coronagraphy dither
        if self.do_contrast:
            self.dither_weights = [1]
            exposure_list = [exposure_list[2]]
        else:
            if self.do_weighted_dithers:
