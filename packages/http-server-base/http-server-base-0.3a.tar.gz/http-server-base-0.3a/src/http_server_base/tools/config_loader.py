...
class ConfigLoader:
    ...
    __MAIN_CONFIG_NOT_FOUND_EXIT_CODE = 3
    ...
    def __find_element_in_dict_object(dict_object:dict, key_path:str):
        try:
            ...
            return ConfigLoader.__DEFAULT_OBJECT
    ...
    def __try_load(path:str) -> Union[Dict[str, Any], object]:
        try:
            ...
            logger.error(f"Error while loading configuration: File not found.")
            result = ConfigLoader.__DEFAULT_OBJECT
        except json.JSONDecodeError:
            logger.error(f"Error while loading configuration: File is not JSON-serializable.")
            result = ConfigLoader.__DEFAULT_OBJECT
        except:
            logger.exception(f"Unhandled error while loading configuration:")
            result = ConfigLoader.__DEFAULT_OBJECT
        else:
            config_file.close()
            result = config
    ...
    @staticmethod
    def load_configs(main_config_path: str = None, config_paths: Union[None, List[str], Dict[str, str]] = None, soft_mode: bool = False):
        """
    ...
    def get_from_config(path:str, config_name:str='main', default:Union[Any, Callable[[], Any]]=None) -> Any:
        ...
        if (config_name in ConfigLoader.__active_configs):
            result = ConfigLoader.__find_element_in_dict_object(dict_object=ConfigLoader.__active_configs[config_name], key_path=path)
