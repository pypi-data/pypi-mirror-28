...
class HttpServerBase(Application):
    ...
    def __init__(self,
        ...
        listen_port = _settings.get('listen_port') or _settings.get('port')
    ...
    def simple_start_decorator_with_settings(cls, **settings):
        ...
        Example usage:
        @HttpServerBase.simple_start_decorator_with_settings(port=8123, static_files='static', debug=True)
        def my_func(server_app):
            server_app.logger.debug("I am going to be run")
        
        """
        return lambda func: cls.simple_start_decorator(func, **settings)
        
    @classmethod
    def simple_start_decorator(cls, pre_run_func:Callable[['HttpServerBase', Any], None], **settings) -> Callable[[Any], None]:
        """
    ...
    @classmethod
    def __simple_start_func(cls, pre_run_func:Union[Callable[['HttpServerBase'], None], None], **settings) -> Tuple[Union['HttpServerBase', None], Union[Exception, None]]:
        """
        ...
        server_application:HttpServerBase = cls(**settings)
    ...
    @classmethod
    def simple_start_server(cls, pre_run_func:Union[Callable[['HttpServerBase'], None], None] = None, **settings):
        """
