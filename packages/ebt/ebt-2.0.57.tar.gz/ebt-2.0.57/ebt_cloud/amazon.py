...
class S3(object):
    ...
    def list_bucket(self, bucket_name):
        ...
        for item in bucket.list():
            item.last_modified_dt = get_localzone().normalize(datetime.datetime.strptime(item.last_modified, '%Y-%m-%dT%H:%M:%S.%fZ').replace(tzinfo=pytz.utc).astimezone(get_localzone()))
