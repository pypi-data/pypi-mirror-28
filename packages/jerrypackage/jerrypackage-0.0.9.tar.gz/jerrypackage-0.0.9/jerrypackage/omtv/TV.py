...
class TV:
    ...
    def __generate_season_object_recursive__(self, url, season_to_generate=-1, from_season = -1):
        next_url = None
        ...
        for div_tag in div_tags:
            ...
            if a_tag is not None:
                ...
                for season in season_TODO:
                    if (season_to_generate == -1 and from_season <= season) or season_to_generate == season:
        ...
        if len(seasons) == 0 or seasons != [n for n in range(1, max(seasons) + 1)]:  # 没有取全
            if next_url is not None:
                if season_to_generate == -1:
                    self.__generate_season_object_recursive__(next_url, season_to_generate, from_season)
                else:
                    self.__generate_season_object_recursive__(next_url, season_to_generate, from_season)
            else:
    ...
    def generate_season_object(self, season):
        ...
        self.__generate_season_object_recursive__(self.tv_url, season, -1)
        # self.latest_season = -1               #don't change
        print("End : ", self.tv_name)

    def generate_season_object_from(self, season):
        print("Generate Season object from: ", self.tv_name, " Season: ", season)
        self.__generate_season_object_recursive__(self.tv_url, -1, season)
        if len(self.season_object_dict.keys()) > 0:
            self.latest_season = max([int(n) for n in self.season_object_dict.keys()])
