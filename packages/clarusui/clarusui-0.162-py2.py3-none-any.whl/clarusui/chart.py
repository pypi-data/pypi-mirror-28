...
class Histogram(Chart):
    ...
    def _get_options(self):
        ...
        if binSize is not None:
            hist_options['xbins'] = dict(size=binSize, start=self.rangeStart, end=self.rangeEnd)
        return hist_options
...
class DistChart(Chart):
    def __init__(self, response, **options):
        self.binSize = options.pop('binSize', 1.)
    ...
    def _get_plot_data(self):
        ...
        return ff.create_distplot(data, groupLabels, bin_size=self.binSize)                 
