...
def create_nice_cx_from_pandas(df, source_field=None, target_field=None,
    ...
    if source_field and target_field:
        for index, row in df.iterrows():
            ...
            # ADD NODES
            # =============
            my_nicecx.create_node(id=str(row[source_field]), node_name=str(row[source_field]), node_represents=str(row[source_field]))
            my_nicecx.create_node(id=str(row[target_field]), node_name=str(row[target_field]), node_represents=str(row[target_field]))
            ...
            if edge_interaction:
                if row.get(edge_interaction):
                    my_nicecx.create_edge(id=index, edge_source=str(row[source_field]),
                                          edge_target=str(row[target_field]),
            ...
            for sp in source_node_attr:
                ...
                if type(row[sp]) is float and math.isnan(row[sp]):
                    ...
                    attr_type = ATTRIBUTE_DATA_TYPE.INTEGER
                my_nicecx.set_node_attribute(str(row[source_field]), sp, str(row[sp]), type=attr_type)
            ...
            for tp in target_node_attr:
                ...
                if type(row[tp]) is float and math.isnan(row[tp]):
                    ...
                    attr_type = ATTRIBUTE_DATA_TYPE.INTEGER
                my_nicecx.set_node_attribute(str(row[target_field]), tp, str(row[tp]), type=attr_type)
