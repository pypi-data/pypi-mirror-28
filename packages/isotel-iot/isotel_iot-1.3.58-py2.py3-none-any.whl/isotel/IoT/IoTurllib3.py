...
class RequestHandler:
    ...
    def get(self, url, timeout):
        ...
        #f = urllib.request.urlopen(req, timeout=timeout, context=ssl_context)

        #return _decode_resp(f)
    ...
    def _handle_request(self, method, url, data=None, timeout=10):
        ...
        #if method == "POST":  
        #    c.request(method, path, data, headers=headers)
        #else:    
        ...
        try:
            ...
            #retry with new connection if necessary
            if reuse:
                self.close_connection()
