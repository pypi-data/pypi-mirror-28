...
class Device:
    ...
    def __init__(self, server, device, unit=True, advanced=False, development=False):
        ...
        try :
            ...
            if 'label' not in status or status['label'] is None:
                raise ValueError("device " + self.device + " not yet loaded")
            self.device = str(status['label']).replace(" ", "_")
        except Exception as e:
            raise ValueError("device " + self.device + " not available")
