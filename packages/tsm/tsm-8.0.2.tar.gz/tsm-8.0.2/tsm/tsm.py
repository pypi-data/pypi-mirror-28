...
def save_csv(filename,data,use_quotes=False,double_to_single=True,file_mode='w',enc='utf-8'): #this assumes a list of lists wherein the second-level list items contain no commas
...
def get_top_hashtags(tweets_data,nodes_data='',min_ct=10,rtl_ht=False):
    ...
    for cid in clust_uniq:
        if nodes_data != '':
            splitprep = [' ' + re.sub(ht_pattern,' ',t[1]).replace(u'\u200F','') + ' ' for t in tweets if '#' in t[1] and node_dict[t[0]] == cid] #fills in the list tweets with hashtags, lowercased, space-padded, cleaned and only if a hashmark exists in the tweet
        else:
            splitprep = tuple([' ' + re.sub(ht_pattern,' ',t).replace(u'\u200F','') + ' ' for t in tweets if '#' in t])
...
def get_top_links(tweets_data,nodes_data='',min_ct=10,domains_only=False,remove_3ld=False,remove_trailing_chars=['#','?'],remove_domains=[]):
    tweets = [i for i in load_data(tweets_data) if len(i) > 0 and i[0].strip() != '']
    if nodes_data != '':
        nodes = [i for i in load_data(nodes_data) if i[0].strip() != '']
        if nodes_data[0][0] == 'name':
    ...
    for cid in clust_uniq:
        if nodes_data != '':
            splitprep = [' ' + re.sub(link_pattern,'',t[1]).replace(u'\u200F','') + ' ' for t in tweets if 'http://' in t[1] and node_dict[t[0]] == cid] #fills in the list tweets with hyperlinks, lowercased, space-padded, cleaned and only if 'http://' exists in the tweet
        else:
            splitprep = [' ' + re.sub(link_pattern,'',t).replace(u'\u200F','') + ' ' for t in tweets if 'http://' in t]
        ...
        if len(remove_domains) > 0:
            final = [hl for hl in final if not any(d in hl for d in remove_domains)]
