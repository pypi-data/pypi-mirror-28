...
class RedshiftReportGeneratorFactory(ReportGeneratorFactory):
    def __init__(self, *args, **kwargs):
        ...
        self.generic_storage = self._create_generic_storage()

    def _create_generic_storage(self):
        boto_client = boto3.resource(
            's3',
            aws_access_key_id=self.aws_credential.aws_access_key,
            aws_secret_access_key=self.aws_credential.aws_secret_access_key
        )
        return generic_storage.get_storage('S3', **{
    ...
    def _create_sa_engine(self, connection_dialect, *args, **kwargs):
        ...
        logger.info('Connecting to %s', connection_dialect)
