...
class development_tree(object):
    ...
    def get_s0(self, t0=0.0, dt=0.1, tn=2e2, eps=1e-5):

        t = np.arange(t0, tn, dt)
        nt = t.shape[0]

        s = np.random.uniform(-1, 1, size=self.n)
        for gene, sign in self.tfs[self.stem_cell_type].iteritems():
            s[gene] = sign
        for it, ti in enumerate(t):
...
def simulate_scRNAseq_data(dev_tree,
    ...
    args = ((icell, cell, copy.copy(dev_tree), nt[icell]) + kwds
