...
from newrelic_plugins_agent.plugins import base
...
class MongoDB(base.Plugin):
    ...
    def add_server_repl_datapoints(self, stats):
        """Add all of the data points about replication status

        :param dict stats: The stats data to add

        """
        self.add_gauge_value('Repl/state', 'state', stats.get('myState', 0));
    ...
    def get_and_add_replication_stats(self):
        LOGGER.debug('Fetching Replication stats')
        client = self.connect()
        if not client:
            return
        if self.config.get('admin_username'):
            client.admin.authenticate(self.config['admin_username'],
                                   self.config.get('admin_password'))
        self.add_server_repl_datapoints(client.admin.command('replSetGetStatus'))
        client.close()
    ...
    def poll(self):
        ...
        self.get_and_add_replication_stats()
        self.finish()
